    ...
    def _get_dynamic_routing_edge_list(self, context, gateway_network_id):
        ...
        for port in gateway_ports:
            ...
            if edge_binding['edge_type'] == nsxv_constants.SERVICE_EDGE:
                ...
                for edge_router in edge_router_list:
                    if edge_router['edge_id'] == edge_id:
                        edge_router['router_id_list'].append(router_id)
                        create_new = False
                        break
                ...
                if edge_bindings:
                    try:
                        ...
                        with excutils.save_and_reraise_exception():
                            LOG.error(_LE("Failed to update BGP peer"
        ...
        for bgp_speaker_id in bgp_speaker_ids:
            ...
            if edge_bindings:
                ...
                try:
                    ...
                    with excutils.save_and_reraise_exception():
                        LOG.error(_LE("NSXv: Failed to delete BGP peer "
                                      "for %s"), binding['edge_id'])
