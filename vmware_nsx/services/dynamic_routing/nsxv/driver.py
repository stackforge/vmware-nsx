...
class NsxvBGPDriver(bgp_db.BgpDbMixin):
    ...
    def _get_dynamic_routing_edge_list(self, context, gateway_network_id):
        ...
        for port in gateway_ports:
            ...
            if edge_binding['edge_type'] == nsxv_constants.SERVICE_EDGE:
                create_new = True
                for i in xrange(len(edge_router_list)):
                    if edge_router_list[i]['edge_id'] == edge_binding['edge_id']:
    ...
    def _query_fips_and_tenant_subnets(self, context, gateway_network_id, router_ids):
        ...
        fip_ports = self._core_plugin.get_ports(context, filters=filters,
                                                fields=fields)
        fips = [{'id': p['id'], 'ip_address': p['fixed_ips'][0]['ip_address']} for p in fip_ports]
    ...
    def update_bgp_speaker(self, context, bgp_speaker_id, bgp_speaker):
        ...
        for network_id in gateway_network_ids:
            ...
            if ((new_fip_policy is not None and new_fip_policy != old_fip_policy) or
                    (new_network_policy is not None and new_network_policy != old_network_policy)):
                for edge_router_config in edge_router_list:
                    ...
                    fips, subnets = self._query_fips_and_tenant_subnets(
                            context, network_id, edge_router_config['router_id_list'])
    ...
    def add_gateway_network(self, context, bgp_speaker_id, network_info):
        ...
        try:
            for edge_router_config in edge_router_list:
                ...
                fips, subnets = self._query_fips_and_tenant_subnets(
                        context, gateway_network_id, edge_router_config['router_id_list'])
