...
class NSXvBgpDriver(object):
    ...
    def _register_callbacks(self):
        super(NSXvBgpDriver, self)._register_callbacks()
        registry.subscribe(self._shared_router_after_migrate_callback,
                           shrd_rtr.SHARED_ROUTER,
                           events.AFTER_CREATE)
        registry.subscribe(self._shared_router_before_migrate_callback,
                           shrd_rtr.SHARED_ROUTER,
        ...
        for bgp_speaker_id in bgp_speaker_ids:
            ...
            for binding in bgp_bindings:
                ...
                try:
                    ...
                    LOG.exception("Failed to delete BGP neighbour on edge "
                                  "'%s'", binding['edge_id'])
                else:
    ...
    def add_gateway_network(self, context, bgp_speaker_id, network_info):
        ...
        for edge_id, edge_router_config in edge_router_dict.items():
            ...
            try:
                ...
                LOG.exception("Failed to configure BGP on edge '%s'.",
                              edge_id)
            else:
                password = cfg.CONF.nsxv.edge_appliance_password
                nbr = gw_bgp_neighbour(prot_router_id,
                                       speaker['local_as'],
                                       password)
                nbrs.append(nbr)
    ...
    def _start_bgp_on_edge(self, context, edge_id, speaker, bgp_peers,
        ...
        default_routes = [static_route('0.0.0.0/0', peer['peer_ip'])
                          for peer in bgp_peers]
        # TODO(roeyc): LOCK EDGE
        try:
            self._nsxv.add_bgp_speaker_config(edge_id, prot_router_id,
                                              local_as, enabled_state, ecmp,
                                              default_routes, bgp_neighbours,
                                              prefixes, redis_rules)
        except vcns_exc.VcnsApiException:
    ...
    def _stop_bgp_on_edges(self, context, bgp_bindings, speaker_id):
        ...
        for bgp_binding in bgp_bindings:
            ...
            try:
                ...
                LOG.exception("Failed to delete BGP config on edge %s.",
                              bgp_binding['edge_id'])
            else:
    ...
    def _shared_router_after_migrate_callback(self, resource, event, trigger,
                                              **kwargs):
        ...
        for speaker in speakers:
            self.enable_bgp_on_router(speaker, router['id'])

    def _shared_router_before_migrate_callback(self, resource, event, trigger,
                                               **kwargs):
        context = kwargs['context']
        router = kwargs['router']
        ext_net_id = router.gw_port['network_id']
        ...
        speakers = self._plugin._bgp_speakers_for_gateway_network(context,
                                                                  ext_net_id)
        for speaker in speakers:
            self.disable_bgp_on_router(speaker, router['id'], gw_ip)
    ...
    def disable_bgp_on_router(self, speaker, router_id, gw_ip):
        context = n_context.get_admin_context()
