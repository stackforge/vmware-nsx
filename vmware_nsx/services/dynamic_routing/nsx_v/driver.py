...
def redistribution_rule(connected, static, prefix_name=None, action='permit'):
...
def bgp_neighbour(bgp_peer):
    bgp_filter = {'bgpFilter': [{'direction': 'in', 'action': 'deny'}]}
    nbr = _bgp_neighbour(bgp_peer['peer_ip'], bgp_peer['remote_as'],
                         bgp_peer['password'])
    nbr['bgpNeighbour'].update(bgpFilters=bgp_filter)
    return nbr


def _bgp_neighbour(ip_address, remote_as, password=None):
    nbr = dict(ipAddress=ip_address, remoteAS=remote_as, password=password)
...
class NSXvBgpDriver(object):
        ...
        for port in gateway_ports:
            ...
            if edge_id not in edge_router_dict:
                edge_router_dict[edge_id] = {'no_snat_routers': [],
                                             ...
                                             protocol_router_id,
                                             'static': static}
            if not router[l3.EXTERNAL_GW_INFO]['enable_snat']:
    ...
    def _get_prefixes_and_redistribution_rules_from_subnets(self, subnets,
                                                            static):
        prefixes = []
        redis_rules = []
    ...
    def update_bgp_speaker(self, context, bgp_speaker_id, bgp_speaker):
        ...
        LOG.info("%(action)s BGP route redistribution on edges: %(edges)s.",
                 {'edges': edge_ids,
                  'action': 'Enabling' if new_enabled_state else 'Disabling'})
        for edge_id in edge_ids:
            # TODO(roeyc): LOCK EDGE
            try:
    ...
    def delete_bgp_speaker(self, context, bgp_speaker_id):
        # TBD(roeyc): LOCK BGP SPEAKER
        bgp_bindings = nsxv_db.get_nsxv_bgp_speaker_bindings(context.session,
                                                             bgp_speaker_id)
        for bgp_binding in bgp_bindings:
            # TODO(roeyc): LOCK EDGE
            try:
                self._nsxv.delete_bgp_speaker_config(bgp_binding['edge_id'])
            except vcns_exc.VcnsApiException:
                LOG.warning("Failed to delete BGP config on edge '%s'.",
                            bgp_binding['edge_id'])
            else:
    ...
    def update_bgp_peer(self, context, bgp_peer_id, bgp_peer):
        ...
        bgp_speaker_ids = self._plugin.get_bgp_speakers_by_bgp_peer(
            context, bgp_peer_id)
        ...
        for bgp_speaker_id in bgp_speaker_ids:
            # TBD(roeyc): LOCK SPEAKER
            bgp_bindings = nsxv_db.get_nsxv_bgp_speaker_bindings(
                context.session, bgp_speaker_id)
            nbr = bgp_neighbour(old_bgp_peer)
            for binding in bgp_bindings:
                try:
                    self._nsxv.update_bgp_neighbour(binding['edge_id'], nbr)
                except vcns_exc.VcnsApiException:
                    LOG.error("Failed to update BGP neighbor '%s' on edge "
                              "'%s'",
                              old_bgp_peer['peer_ip'], binding['edge_id'])

    def delete_bgp_peer(self, context, bgp_peer_id):
        bgp_speaker_ids = self._get_bgp_speakers_by_bgp_peer(context,
                                                             bgp_peer_id)
        bgp_peer_info = self._plugin.get_bgp_peer(context, bgp_peer_id)
        nbr = bgp_neighbour(bgp_peer_info)
        for bgp_speaker_id in bgp_speaker_ids:
            # TBD(roeyc): LOCK SPEAKER
            bgp_bindings = nsxv_db.get_nsxv_bgp_speaker_bindings(
                context.session, bgp_speaker_id)
            for binding in bgp_bindings:
                ...
                try:
                    ...
                    LOG.error("Failed to delete BGP neighbour on edge '%s'",
                              binding['edge_id'])
                else:
                    LOG.debug("Succesfully deleted BGP neighbor '%s' from %s",
                              bgp_peer_info['peer_ip'], binding['edge_id'])
    ...
    def add_gateway_network(self, context, bgp_speaker_id, network_info):
        ...
        for edge_id, edge_router_config in edge_router_dict.items():
            ...
            prot_router_id = edge_router_dict[edge_id]['protcol_router_id']
            try:
                self._start_bgp_on_edge(edge_id, speaker, bgp_peers,
                                        prot_router_id, subnets, static)
            except vcns_exc.VcnsApiException:
                LOG.error("Failed to configure BGP on edge '%s'.",
                          edge_id)
            else:
                nsxv_db.add_nsxv_bgp_speaker_binding(
                    context.session, edge_id, bgp_speaker_id, prot_router_id)
                nbrs.append(
                    _bgp_neighbour(prot_router_id, speaker['local_as']))
        for edge_gw in [peer['name'] for peer in bgp_peers]:
            self.add_bgp_neighbours(edge_gw, [nbrs])

    def _start_bgp_on_edge(self, edge_id, speaker, bgp_peers,
                           prot_router_id, subnets, static):
        enabled_state = speaker['advertise_tenant_networks']
        local_as = speaker['local_as']
        prefixes, redis_rules = (
            self._get_prefixes_and_redistribution_rules_from_subnets(
                subnets, static))
        default_rule = redistribution_rule(True, True, action='deny')
        redis_rules.append(default_rule)

        bgp_neighbours = [bgp_neighbour(bgp_peer) for bgp_peer in bgp_peers]
        # TODO(roeyc): LOCK EDGE
        self._nsxv.add_bgp_speaker_config(edge_id, prot_router_id, local_as,
                                          enabled_state,
                                          bgp_neighbours,
                                          prefixes,
                                          redis_rules)
        ...
        for bgp_binding in bgp_bindings:
            ...
            try:
                ...
                nsxv_db.delete_nsxv_bgp_speaker_binding(
                    context.session, bgp_binding['edge_id'])

    def enable_bgp_on_router(self, speaker, router):
        context = n_context.get_admin_context()
        edge_id, static = self._get_router_edge_info(context, router['id'])
        gw_ip = (
            router[l3.EXTERNAL_GW_INFO]['external_fixed_ips'][0]['ip_address'])
        ...
        if router.enable_snat:
            ...
            subnets = self._query_tenant_subnets(context, router['id'])
        ...
        if bgp_binding and subnets:
            ...
            prot_router_id = gw_ip
            self._start_bgp_on_edge(edge_id, speaker, bgp_peers,
                                    prot_router_id, subnets, static)
            for gw_edge_id in [peer['name'] for peer in bgp_peers]:
                nbr = _bgp_neighbour(prot_router_id, speaker['local_as'])
                self.driver.add_bgp_neighbours(gw_edge_id, [nbr])

    def disable_bgp_on_router(self, speaker, router_id, gw_ips):
        context = n_context.get_admin_context()
        edge_id, static = self._get_router_edge_info(
            context, router_id)
        routers_ids = (
            self._core_plugin.edge_manager.get_routers_on_same_edge(context,
                                                                    router_id))
        # TODO(roeyc): LOCK SPEAKER
        # TODO(roeyc): LOCK EDGE
        try:
            self._nsxv.delete_bgp_speaker_config(edge_id)
        except vcns_exc.VcnsApiException:
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to configure BGP on edge %s.", edge_id)

        if len(routers_ids) > 1:
            # Shared router, only remove prefixes and redistribution rules.
            bgp_bindings = nsxv_db.get_nsxv_bgp_speaker_bindings(
                context.session, speaker['id'])
            ...
            if bgp_bindings['protocol_router_id'] == gw_ips[0]:
                router = self._core_plugin._get_router(context, routers_ids[0])
                new_router_id = router.gw_port['fixed_ips'][0]['ip_address']
                bgp_bindings['protocol_router_id'] = new_router_id
                context.session.commit()
        else:
            nsxv_db.delete_nsxv_bgp_speaker_binding(context.session, edge_id)

    def advertise_subnet(self, speaker_id, router_id, subnet):
        context = n_context.get_admin_context()
        router = self._core_plugin.get_router(context, router_id)
        if router[l3.EXTERNAL_GW_INFO]['enable_snat']:
    ...
    def withdraw_subnet(self, speaker_id, router_id, subnet_id):
        context = n_context.get_admin_context()
        router = self._core_plugin.get_router(context, router_id)
        if router[l3.EXTERNAL_GW_INFO]['enable_snat']:
            # Do nothing, by default, only when advertisement is needed we add
