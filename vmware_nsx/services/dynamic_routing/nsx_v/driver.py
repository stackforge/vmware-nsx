...
def bgp_neighbour(bgp_peer):
    ...
    nbr = dict(ipAddress=bgp_peer['peer_ip'], remoteAS=bgp_peer['remote_as'],
               password=bgp_peer['password'], bgpFilters=bgp_filter)
...
class NSXvBgpDriver(object):
    ...
    def update_bgp_speaker(self, context, bgp_speaker_id, bgp_speaker):
        ...
        for edge_id in edge_ids:
            ...
            try:
                ...
                LOG.warning(_LE("Failed to update BGP on edge %s."), edge_id)
    ...
    def delete_bgp_speaker(self, context, bgp_speaker_id):
        ...
        for bgp_binding in bgp_bindings:
            ...
            try:
                ...
                LOG.warning(_LE("Failed to delete BGP config on edge '%s'."),
                            bgp_binding['edge_id'])
            else:
        ...
        for bgp_speaker_id in bgp_speaker_ids:
            ...
            for binding in bgp_bindings:
                try:
                    ...
                    LOG.error(_LE("Failed to update BGP neighbor '%s' "
                                  "on edge '%s'"),
                              old_bgp_peer['peer_ip'],
                              binding['edge_id'])
    ...
    def delete_bgp_peer(self, context, bgp_peer_id):
        ...
        for bgp_speaker_id in bgp_speaker_ids:
            ...
            for binding in bgp_bindings:
                # TODO(roeyc): LOCK EDGE
                try:
                    self._nsxv.delete_bgp_neighbour(binding['edge_id'], nbr)
                except vcns_exc.VcnsApiException:
                    LOG.error(_LE("Failed to delete BGP neighbour "
                                  "on edge '%s'"), binding['edge_id'])
                else:
    ...
    def add_bgp_peer(self, context, bgp_speaker_id, bgp_peer_info):
        ...
        for binding in bgp_bindings:
            # TODO(roeyc): LOCK EDGE
            try:
                self._nsxv.add_bgp_neighbour(binding['edge_id'], nbr)
            except vcns_exc.VcnsApiException:
                LOG.error(_LE("Failed to add BGP neighbour on '%s'"),
                          binding['edge_id'])
            else:
    ...
    def remove_bgp_peer(self, context, bgp_speaker_id, bgp_peer_info):
        ...
        for binding in bgp_bindings:
            try:
                ...
                LOG.exception(_LE("Failed to delete BGP neighbour on '%s'"),
                              binding['edge_id'])
            else:
    ...
    def add_gateway_network(self, context, bgp_speaker_id, network_info):
        ...
        for edge_id, edge_router_config in edge_router_dict.items():
            ...
            try:
                ...
                LOG.error(_LE("Failed to configure BGP on edge '%s'."),
                          edge_id)
            else:
    ...
    def _start_bgp_on_edge(self, edge_id, speaker, bgp_peers,
        ...
        # TODO(roeyc): LOCK EDGE
        self._nsxv.add_bgp_speaker_config(edge_id, router_id, local_as,
                                          enabled_state,
                                          bgp_neighbours,
                                          prefixes,
                                          redis_rules)
    ...
    def disable_bgp_on_router(self, speaker, router_id, gw_ips):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error(_LE("Failed to configure BGP on edge %s."), edge_id)
