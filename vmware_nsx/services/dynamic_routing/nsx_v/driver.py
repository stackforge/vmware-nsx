...
class NSXvBgpDriver(object):
    ...
    def _get_router_edge_info(self, context, router_id):
        ...
        if edge_binding['edge_type'] != nsxv_constants.SERVICE_EDGE:
            ...
            edge_binding = nsxv_db.get_nsxv_router_binding(context.session,
                                                           plr_id)
            if not edge_binding:
                # Distributed rotuer isn't bind to plr
                return None, None
    ...
    def add_bgp_peer(self, context, bgp_speaker_id, bgp_peer_info):
        ...
        for binding in bgp_bindings:
            try:
                ...
                nbr = gw_bgp_neighbour(binding['protocol_router_id'],
                                       speaker['local_as'],
                                       self.edge_password)
    ...
    def remove_bgp_peer(self, context, bgp_speaker_id, bgp_peer_info):
        ...
        for binding in bgp_bindings:
            try:
                ...
                LOG.error("Failed to delete BGP neighbour on '%s'",
                          binding['edge_id'])
            else:
                nbr = gw_bgp_neighbour(binding['protocol_router_id'],
                                       speaker['local_as'],
        ...
        if bgp_peer_obj.get('esg_id'):
            edge_gw = bgp_peer_obj['esg_id']
            try:
                self._nsxv.delete_bgp_neighbours(edge_gw, neighbours)
            except vcns_exc.VcnsApiException:
                LOG.error("Failed to delete BGP neighbour on GW Edge '%s'",
                          edge_gw)
        ...
        with locking.LockManager.get_lock('nsx-shared-router-pool'):
            ...
            for edge_id, edge_router_config in edge_router_dict.items():
                ...
                try:
                    ...
                    LOG.error("Failed to configure BGP speaker %s on edge "
                              "'%s'.", bgp_speaker_id)
                else:
    ...
    def _stop_bgp_on_edges(self, context, bgp_bindings, speaker_id):
        ...
        for gw_edge in gw_edges:
            try:
                self._nsxv.delete_bgp_neighbours(gw_edge, neighbours_to_remove)
            except vcns_exc.VcnsApiException:
                LOG.error("Failed to delete BGP neighbour on GW edge '%s'.",
                          gw_edge)
    ...
    def _before_router_edge_binding_delete_callback(self, resource, event,
                                                    trigger, **kwargs):
    ...
    def _after_router_edge_binding_create_callback(self, resource, event,
                                                   trigger, **kwargs):
        context = kwargs['context']
        router = kwargs['router']
        ext_net_id = router.gw_port['network_id']
