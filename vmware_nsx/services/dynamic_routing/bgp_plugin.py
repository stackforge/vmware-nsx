...
class BgpPluginBase(service_base.ServicePluginBase,
    ...
    def _get_esg_peer_info(self, context, bgp_peer):
        esg_id = nsxv_db.get_nsxv_bgp_peer_edge_binding(context.session,
                                                        bgp_peer['id'])
        if esg_id:
            bgp_peer['esg_id'] = esg_id
    ...
    def get_bgp_peer(self, context, bgp_peer_id, fields=None):
        ...
        if fields is None or 'esg_id' in fields:
            self._get_esg_peer_info(context, peer)
        return peer
    ...
    def router_gateway_callback(self, resource, event, trigger, **kwargs):
        ...
        for speaker in speakers:
            with locking.LockManager.get_lock(str(speaker.id)):
...
class NSXvBgpPlugin(BgpPluginBase):
    ...
    def _register_callbacks(self):
        ...
        registry.subscribe(
            self.nsx_driver._after_service_edge_create_callback,
            nsxv_constants.SERVICE_EDGE,
            events.AFTER_CREATE)
        registry.subscribe(
            self.nsx_driver._before_service_edge_delete_callback,
            nsxv_constants.SERVICE_EDGE,
