...
class NsxV3QosPlugin(qos_plugin.QoSPlugin):
    ...
    def create_policy(self, context, policy):
        ...
        try:
            policy_dict = super(NsxV3QosPlugin, self).create_policy(
                context, policy)
            ...
            with excutils.save_and_reraise_exception():
                ...
                LOG.exception(_LE('Failed to create qos-policy %s'),
                    policy['policy']['id'])
    ...
    def _get_bw_values_from_rule(self, bandwidth_limit_rule):
        ...
        if (attributes.is_attr_set(bw_data.get('max_kbps')) and
            int(bw_data['max_kbps']) < MAX_KBPS_MIN_VALUE):
            raise nsx_exc.NsxQosSmallBw()

        # 'None' value means we will keep the old value
        burst_size = peak_bandwidth = average_bandwidth = None

        if attributes.is_attr_set(bw_data.get('max_burst_kbps')):
            # translate kbps -> bytes
            burst_size = int(bw_data['max_burst_kbps']) * 128

        if attributes.is_attr_set(bw_data.get('max_kbps')):
            # translate kbps -> Mbps
