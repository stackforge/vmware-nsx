...
class NsxV3Driver(l2gateway_db.L2GatewayMixin):
    ...
    def _ensure_default_l2_gateway(self, resource, event,
        ...
        if not def_l2gw_name:
            ...
            return
        admin_ctx = nl_context.get_admin_context()
    ...
    def create_l2_gateway_connection_precommit(self, context, gw_connection):
        ...
        ports = self._core_plugin.get_ports(admin_ctx, filters=port_filters)
        for port in ports:
            ...
            if port.get('device_id'):
                ...
                for mapping in mappings:
                    ...
                    conn = self._get_l2_gateway_connection(admin_ctx, conn_id)
                    con_seg_id = self._get_conn_seg_id(admin_ctx, conn)
                    if conn and con_seg_id and int(con_seg_id) == int(seg_id):
                        ...
                        if conn_bridge_cluster == bridge_cluster:
                            msg = (_("Cannot create multiple connections with "
                                     "the same segmentation id %(seg_id)s for "
                                     "bridge cluster %(bridge)s") % {
                                'seg_id': seg_id,
