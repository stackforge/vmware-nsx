...
class EdgeLoadBalancerManager(base_mgr.EdgeLoadbalancerBaseManager):
    ...
    def _handle_subnet_gw_change(self, *args, **kwargs):
        ...
        lb_ports = self.core_plugin.get_ports(context, filters=filters)

        if lb_ports:
            for lb_port in lb_ports:
                if (lb_port['device_id'] and lb_port['device_id'].startswith(
                        'lbaas-')):
                    edge_bind = nsxv_db.get_nsxv_router_binding(
