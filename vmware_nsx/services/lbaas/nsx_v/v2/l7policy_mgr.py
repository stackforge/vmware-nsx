...
def policy_to_application_rule(policy):
    ...
    for rule in policy.rules:
        # DEBUG ADIT Based on: https://www.haproxy.com/doc/aloha/7.0/haproxy/
        #                      acls.html#sample-fetch-acl-derivatives
        if rule.provisioning_status == constants.PENDING_DELETE:
            # skip this rule as it is being deleted
            continue
        elif rule.type == lb_const.L7_RULE_TYPE_COOKIE:
            if rule.compare_type == lb_const.L7_RULE_COMPARE_TYPE_EQUAL_TO:
                rule_line = ('acl %(rule_id)s hdr_sub(cookie) '
                    '%(key)s=%(val)s' % {'rule_id': rule.id,
                                         'key': rule.key,
                                         'val': rule.value})
            elif rule.compare_type == lb_const.L7_RULE_COMPARE_TYPE_REGEX:
                rule_line = ('acl %(rule_id)s hdr_reg(cookie) '
                    '%(key)s=%(val)s' % {'rule_id': rule.id,
                                         'key': rule.key,
                                         'val': rule.value})
            else:
                # DEBUG ADIT - Add other compare methods
                pass
        if rule.type == lb_const.L7_RULE_TYPE_HOST_NAME:
            pass
        if rule.type == lb_const.L7_RULE_TYPE_PATH:
            pass
        if rule.type == lb_const.L7_RULE_TYPE_FILE_TYPE:
            pass
        if rule.type == lb_const.L7_RULE_TYPE_HEADER:
            pass
        rule_lines.append(rule_line)
        condition = condition + rule.id + ' '

        LOG.debug("DEBUG ADIT : Adding condition of rule %s", rule.id)
    if not rule_lines:
        condition = 'True'
    ...
    script = "\n".join(rule_lines) + "%(action)s if %(cond)s" % {
        'action': action, 'cond': condition}
    LOG.debug("DEBUG ADIT : Script: %s", script)
