...
class EdgeLBaaSv2ListenerManager(EdgeLBaaSv2BaseManager,
    ...
    def create(self, context, listener, certificate=None):
        default_pool = (listener.default_pool.to_dict()
                        if listener.default_pool else None)

        l7_policies = ([l7policy.to_dict() for l7policy in
                        listener.l7_policies] if listener.l7_policies
                       else None)

        super(EdgeLBaaSv2ListenerManager, self).base_create(
            context, listener, listener.id, listener.loadbalancer_id,
            listener.protocol, listener.protocol_port,
            listener.connection_limit, listener.description,
            default_pool, listener.default_tls_container_id,
            l7_policies, certificate=None)

    def update(self, context, old_listener, new_listener, certificate=None):
        default_pool = (new_listener.default_pool.to_dict()
                        if new_listener.default_pool else None)

        l7_policies = ([l7policy.to_dict() for l7policy in
                        new_listener.l7_policies] if new_listener.l7_policies
                       else None)

        super(EdgeLBaaSv2ListenerManager, self).base_update(
            context, new_listener, new_listener.id,
            new_listener.loadbalancer_id, new_listener.protocol,
            new_listener.protocol_port, new_listener.description,
            new_listener.connection_limit, l7_policies,
            old_listener.default_tls_container_id,
            new_listener.default_tls_container_id,
            default_pool, certificate=None)
