...
def get_lbaas_edge_id(context, plugin, lb_id, vip_addr, subnet_id, tenant_id):
    ...
    address_groups = [{'primaryAddress': ip_addr,
                       'secondaryAddresses': vip_addr,
                       'subnetPrefixLength': prefix_len}]
    edge_utils.update_internal_interface(
        plugin, context, resource_id,
        network_id, address_groups)

    gw_ip = subnet.get('gateway_ip')
    if gw_ip:
        plugin.edge_manager.update_routes(edge_id, gw_ip, [])
