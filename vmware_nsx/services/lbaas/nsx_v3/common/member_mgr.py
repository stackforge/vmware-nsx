...
class EdgeMemberManagerFromDict(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def create(self, context, member, member_obj=None):
        with locking.LockManager.get_lock(
            'member-%s' % str(member['pool']['loadbalancer_id'])):
            self._member_create(context, member, member_obj)

    def _member_create(self, context, member, member_obj):
                ...
                if lb_service:
                    ...
                    if vs_id:
                        try:
                            ...
                            self.failed_completion(context, member_obj)
        ...
        self.successful_completion(context, member_obj)

    def update(self, context, old_member, new_member, member_obj=None):
    ...
    def delete(self, context, member, member_obj=None):
        lb_id = member['pool']['loadbalancer_id']
        pool_id = member['pool']['id']
        pool_client = self.core_plugin.nsxlib.load_balancer.pool
        pool_binding = nsx_db.get_nsx_lbaas_pool_binding(
        ...
        self.successful_completion(context, member_obj, delete=True)
