...
class EdgeL7PolicyManagerFromDict(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def create(self, context, policy, policy_obj=None):
        ...
        if not binding:
            self.failed_completion(context, policy_obj)
        ...
        self.successful_completion(context, policy_obj)

    def update(self, context, old_policy, new_policy, policy_obj=None):
        rule_client = self.core_plugin.nsxlib.load_balancer.rule
    ...
    def delete(self, context, policy, policy_obj=None):
        vs_client = self.core_plugin.nsxlib.load_balancer.virtual_server
        rule_client = self.core_plugin.nsxlib.load_balancer.rule
        binding = nsx_db.get_nsx_lbaas_l7policy_binding(context.session,
                                                        policy['id'])
        ...
        self.successful_completion(context, policy_obj, delete=True)
