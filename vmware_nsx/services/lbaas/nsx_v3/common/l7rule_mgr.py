...
class EdgeL7RuleManagerFromDict(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def _update_l7rule_change(self, context, rule, rule_obj, delete=False):
        ...
        binding = nsx_db.get_nsx_lbaas_l7policy_binding(context.session,
                                                        policy_id)
        if not binding:
            self.failed_completion(context, rule_obj)
        ...
        self.successful_completion(context, rule_obj, delete=delete)

    def create(self, context, rule, rule_obj=None):
        self._update_l7rule_change(context, rule, rule_obj)

    def update(self, context, old_rule, new_rule, rule_obj=None):
        self._update_l7rule_change(context, new_rule, rule_obj)

    def delete(self, context, rule, rule_obj=None):
        self._update_l7rule_change(context, rule, rule_obj, delete=True)
