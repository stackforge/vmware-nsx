...
class EdgeLoadBalancerManagerFromDict(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def create(self, context, lb, lb_obj=None):
        if lb_utils.validate_lb_subnet(context, self.core_plugin,
                                       lb['vip_subnet_id']):
            self.successful_completion(context, lb_obj)
        else:
    ...
    def update(self, context, old_lb, new_lb, lb_obj=None):
    ...
    def delete(self, context, lb, lb_obj=None):
            ...
            try:
                ...
                if not vs_list:
                    try:
                        ...
                        self.failed_completion(context, lb_obj, delete=True)
                        msg = (_('Failed to delete lb service %(lbs)s from nsx'
                                 ) % {'lbs': lb_service_id})
                        raise n_exc.BadRequest(resource='lbaas-lb', msg=msg)
            nsx_db.delete_nsx_lbaas_loadbalancer_binding(
                context.session, lb['id'])
        self.successful_completion(context, lb_obj, delete=True)
