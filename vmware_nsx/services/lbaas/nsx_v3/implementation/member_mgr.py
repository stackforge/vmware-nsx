...
class EdgeMemberManagerFromDict(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def create(self, context, member, obj=None, completor=None):
        with locking.LockManager.get_lock(
            'member-%s' % str(member['pool']['loadbalancer_id'])):
            self._member_create(context, member, obj, completor)

    def _member_create(self, context, member, obj, completor):
    ...
    def update(self, context, old_member, new_member, obj=None,
               completor=None):
    ...
    def delete(self, context, member, obj=None, completor=None):
        lb_id = member['pool']['loadbalancer_id']
        pool_id = member['pool']['id']
        pool_client = self.core_plugin.nsxlib.load_balancer.pool
        pool_binding = nsx_db.get_nsx_lbaas_pool_binding(
        ...
        if completor:
            completor.successful_completion(context, obj, delete=True)
