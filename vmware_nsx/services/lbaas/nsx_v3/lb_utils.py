...
def get_nsx_rule_from_policy(nsxlib, policy):
    tags = {'scope': lb_const.LB_L7POLICY_TYPE,
            'tag': policy.id}
    result = nsxlib.search_by_tags(tags=tags, resource_type='LbRule')
    if not result['results']:
        msg = _('Cannot find LbRule on NSX backend for policy '
                '%(policy_id)s') % {'policy_id': policy.id}
        raise n_exc.BadRequest(resource='lbaas-l7policy', msg=msg)

    return result['results'][0]['id']


def get_nsx_vs_from_policy(context, policy):
    binding = nsx_db.get_nsx_lbaas_listener_binding(
        context.session, policy.listener.loadbalancer_id,
        policy.listener_id)
    if not binding:
        msg = _('Cannot find nsx lbaas listener binding for policy '
                '%(policy_id)s') % {'policy_id': policy.id}
        raise n_exc.BadRequest(resource='lbaas-l7policy', msg=msg)
    return binding['lb_vs_id']
