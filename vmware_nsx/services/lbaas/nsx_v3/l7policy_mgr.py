...
class EdgeL7PolicyManager(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def update(self, context, old_policy, new_policy):
        ...
        try:
            vs_id = lb_utils.get_nsx_vs_from_policy(context, old_policy)
            lb_rule_id = lb_utils.get_nsx_rule_from_policy(
                self.core_plugin.nsxlib, old_policy)
    ...
    def delete(self, context, policy):
        ...
        rule_client = self.core_plugin.nsxlib.load_balancer.rule
        try:
            vs_id = lb_utils.get_nsx_vs_from_policy(context, policy)
            rule_id = lb_utils.get_nsx_rule_from_policy(
                self.core_plugin.nsxlib, policy)
            ...
            with excutils.save_and_reraise_exception():
                self.lbv2_driver.l7policy.failed_completion(context, policy)
                LOG.error('Failed to delete L7policy %(policy)s: '
                          '%(error)s', {'policy': policy.id, 'error': e})
