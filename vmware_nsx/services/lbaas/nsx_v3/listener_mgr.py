...
class EdgeListenerManager(base_mgr.Nsxv3LoadbalancerBaseManager):
    ...
    def delete(self, context, listener):
        ...
        if binding:
            ...
            cert_tags = [{'scope': lb_const.LB_LISTENER_TYPE, 'tag': listener.id}]
            cert_list = self.core_plugin.nsxlib.search_by_tags(
                tags=cert_tags, resource_type='certificate_self_signed')
            if cert_list['results']:
                cert_id = cert_list['results'][0]['id']
                tm_client = self.core_plugin.nsxlib.trust_management
                tm_client.delete_cert(cert_id)
