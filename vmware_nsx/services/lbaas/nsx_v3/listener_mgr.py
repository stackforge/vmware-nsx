...
class EdgeListenerManager(base_mgr.LoadbalancerBaseManager):
    ...
    def create(self, context, listener, certificate=None):
        ...
        tags = lb_common.get_tags(self.core_plugin, listener.id,
    ...
    def delete(self, context, listener):
        ...
        if binding:
            vs_id = binding['lb_virtual_server_id']
            app_profile_id = binding['app_profile_id']
            try:
                vs_client.delete(vs_id)
            except nsx_exc.NsxResourceNotFound:
                LOG.error("virtual server not found on nsx: %s", vs_id)
            except nsxlib_exc.ManagerError:
                with excutils.save_and_reraise_exception():
                    self.lbv2_driver.listener.failed_completion(context, listener)
                    LOG.error('Failed to delete virtual server: %s', listener.id)
            try:
                ...
                with excutils.save_and_reraise_exception():
                    self.lbv2_driver.listener.failed_completion(context, listener)
                    LOG.error('Failed to delete application profile: %s',
                              app_profile_id)
        self.lbv2_driver.listener.successful_completion(
