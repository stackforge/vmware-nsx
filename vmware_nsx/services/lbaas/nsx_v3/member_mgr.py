...
class EdgeMemberManager(base_mgr.LoadbalancerBaseManager):
    ...
    def _create_lb_service(self, context, service_client, tenant_id,
        ...
        lb_name = utils.get_name_and_uuid(router['name'],
                                          router_id)
        tags = lb_common.get_tags(self.core_plugin, router_id,
    ...
    def create(self, context, member):
        ...
        if not lb_common.validate_lb_subnet(context, self.core_plugin,
                                            member.subnet_id):
        ...
        if binding:
                    ...
                    try:
                        ...
                        with excutils.save_and_reraise_exception():
                            self.lbv2_driver.member.failed_completion(context,
                                                                      member)
                            LOG.error('Failed to attach virtual server %s to '
                                      'lb service %s', vs_id, lb_service_id)
                else:
                    msg = ('Failed to get lb service to attach virtual '
            ...
            msg = ('Failed to get pool binding to add member %s' %
                   member['id'])
            raise nsx_exc.NsxPluginException(err_msg=msg)
        ...
        if pool_binding:
            ...
            try:
                ...
                network = lb_common.get_network_from_subnet(
                    context, self.core_plugin, member.subnet_id)
                if network.get('router:external'):
                    _, fixed_ip = self._get_info_from_fip(
                        context, member.address)
                else:
                    fixed_ip = member.address
