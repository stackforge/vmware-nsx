            ...
            for listener in new_lb.listeners:
                ...
                if binding:
                    ...
                    try:
                        ...
                        tags.append({'scope': lb_const.LB_LB_NAME,
                                     'tag': new_lb.name[:utils.MAX_TAG_LEN]})
                        tags.append({'scope': lb_const.LB_LB_TYPE,
                                     'tag': new_lb.id})
                        # Update tag on virtual server with new lb name
                        vs_client.update(vs_id, tags=tags)
                        # Update tag on application profile with new lb name
                        app_client.update(app_profile_id, tags=tags)
