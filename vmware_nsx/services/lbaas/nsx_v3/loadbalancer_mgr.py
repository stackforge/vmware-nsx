            ...
            for listener in new_lb.listeners:
                ...
                if binding:
                    ...
                    try:
                        ...
                        new_vs_tags = lb_utils.update_tag(
                            vs['tags'], lb_const.LB_LB_NAME, new_lb_name)
                        vs_client.update(vs_id, tags=new_vs_tags)
                        # Update tag on application profile with new lb name
                        app_profile = app_client.get(app_profile_id)
                        new_app_tags = lb_utils.update_tag(
                            app_profile['tags'], lb_const.LB_LB_NAME,
                            new_lb_name)
                        app_client.update(
                            app_profile_id, tags=new_app_tags,
