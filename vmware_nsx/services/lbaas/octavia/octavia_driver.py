        ...
        if obj_type == 'LoadBalancer':
            ...
            if 'listener' not in obj_dict:
                if obj_dict.get('listener_id'):
                    ...
                    # remove circular references from the listener
                    #if 'default_pool' in listener_obj:
                    #    del listener_obj['default_pool']
                    obj_dict['listener'] = listener_dict
                    if 'listeners' not in obj_dict:
                        # multiple listeners is not really supported yet
                        obj_dict['listeners'] = [listener_dict]
                else:
...
class NSXOctaviaDriverEndpoint(driver_lib.DriverLibrary):
    ...
    @log_helpers.log_method_call
    def update_loadbalancer_statistics(self, ctxt, statistics):
        return super(NSXOctaviaDriverEndpoint,
                     self).update_loadbalancer_statistics(statistics)
