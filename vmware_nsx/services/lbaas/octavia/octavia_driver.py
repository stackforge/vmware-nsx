...
class NSXOctaviaDriver(driver_base.ProviderDriver):
    ...
    def obj_to_dict(self, obj, is_update=False, project_id=None):
        ...
        if obj_type == 'Listener':
            ...
            # TODO(asarfaty): wait till we get it from Octavia, or read it
            # from the DB with the same session as Octavia use, since it was
            # not committed yet.
            #db_listeners = self.repositories.pool.get(
            #    db_apis.get_session(), id=obj_dict['id']).listeners
    ...
    def member_create(self, member):
        ...
        if not member_dict.get('subnet_id'):
            # The NSX driver does not support member with no subnet
            # TODO(asarfaty): use the parent vip_subnet_id instead
            LOG.warning("The NSX Octavia driver does not support member with "
                        "no subnet")
            raise exceptions.NotImplementedError()

        kw = {'member': member_dict}
...
class NSXOctaviaDriverEndpoint(object):
    ...
    # TODO(kobis): replace this with call to Octavia status update API
    def __init__(self):
        self.repo = {
            d_const.HEALTHMONITOR: repositories.HealthMonitorRepository(),
            d_const.LOADBALANCER: repositories.LoadBalancerRepository(),
            d_const.LISTENER: repositories.ListenerRepository(),
            d_const.MEMBER: repositories.MemberRepository(),
            d_const.POOL: repositories.PoolRepository(),
            d_const.L7POLICY: repositories.L7PolicyRepository(),
            d_const.L7RULE: repositories.L7RuleRepository()}

    @log_helpers.log_method_call
    def set_completion_state(self, ctx, obj_type, obj_id, success):
        status = o_const.ACTIVE if success else o_const.ERROR
        self.repo[obj_type].update(db_apis.get_session(), obj_id,
                                   provisioning_status=status)
        LOG.info("Setting status of %s %s to %s", obj_type, obj_id, status)
