...
class NsxpFwaasCallbacksV2(com_callbacks.NsxCommonv3FwaasCallbacksV2):
    ...
    def _get_rule_ips_group_id(self, router_id, rule_id, direction):
        return '%s-%s-%s' % (direction, rule_id, router_id)
    ...
    def _get_rule_cidr_group(self, domain_id, router_id, rule, is_source,
        ...
        if (rule.get(field) and
            ...
            # Adding the rotuer id to the group id since it must be separate
            # for each router
            group_id = self._get_rule_ips_group_id(
                router_id, rule['id'], direction_text)
    ...
    def _set_rules_order(self, fw_rules):
        # TODO(asarfaty): Consider adding nsxlib api for this
