...
class NsxpFwaasCallbacksV2(com_callbacks.NsxFwaasCallbacksV2):
    ...
    def _get_default_backend_rule(self, domain_id, router_id):
        ...
        to be added to the end of the rules list
        """
    ...
    def _translate_services(self, fwaas_rule):
        ...
        if l4_protocol in [nsx_constants.TCP, nsx_constants.UDP]:
            ...
            #         nsx_constants.ICMP_TYPE_NSSERVICE,
            #         protocol=nsx_constants.ICMPV4),
            #     self.nsx_firewall.get_nsservice(
            #         nsx_constants.ICMP_TYPE_NSSERVICE,
            #         protocol=nsx_constants.ICMPV6),
            # ]
    ...
    def _get_rule_cidr_group(self, domain_id, router_id, rule, is_source,
        ...
        if (rule.get(field) and
            ...
            self.nsxpolicy.group.create_or_overwrite_with_conditions(
                "Rule %s %s" % (rule['id'], direction_text),
                domain_id, group_id=group_id,
                description='%s for FW rule %s' % (group_ips, rule['id']),
    ...
    def _create_network_group(self, domain_id, router_id, neutron_net_id):
        ...
        expr = self.nsxpolicy.group.build_condition(
            ...
            cond_member_type='LogicalSwitch') # DEBUG ADIT add constant
        self.nsxpolicy.group.create_or_overwrite_with_conditions(
            "Segment %s" % neutron_net_id,
            domain_id,
    ...
    def _translate_rules(self, domain_id, router_id, segment_group,
        ...
        for rule in fwaas_rules:
            ...
            src_group = self._get_rule_cidr_group(
                domain_id, router_id, rule, is_source=True, is_ingress=is_ingress)
            if not is_ingress and not src_group:
                src_group = segment_group
            dest_group = self._get_rule_cidr_group(
                domain_id, router_id, rule, is_source=False, is_ingress=is_ingress)
    ...
    def _get_port_translated_rules(self, domain_id, router_id, neutron_net_id,
        ...
        """
        self._create_network_group(domain_id, router_id, neutron_net_id)
        port_rules = []
        ...
        port_rules.extend([
            self.nsxpolicy.gateway_policy.build_entry(
                ...
                self._get_random_rule_id(DEFAULT_RULE_ID + neutron_net_id + 'ingress'),
                action=nsx_constants.FW_ACTION_DROP,
                source_groups=[neutron_net_id],
                scope=[self.nsxpolicy.tier1.get_path(router_id)],
                direction=nsx_constants.IN),
            self.nsxpolicy.gateway_policy.build_entry(
                "Block port egress", domain_id, router_id,
                self._get_random_rule_id(DEFAULT_RULE_ID + neutron_net_id + 'egress'),
                action=nsx_constants.FW_ACTION_DROP,
                scope=[self.nsxpolicy.tier1.get_path(router_id)],
                dest_groups=[neutron_net_id],
    ...
    def update_router_firewall(self, context, nsxlib, router_id, router,
        ...
        if from_fw:
            ...
            if with_fw:
                ...
                if not sr_exists and exists_on_backend:
                    ...
                    self.core_plugin.delete_service_router(context, domain_id, router_id)
        ...
        if exists_on_backend:
            ...
            # DEBUG ADIT - is this the right place?
            LOG.error("DEBUG ADIT delete router gw policy")
            self.delete_router_gateway_policy(domain_id, router_id)

        if not with_fw:
            LOG.error("DEBUG ADIT delete router fw resources")
            # Do all the cleanup once the router has no more FW rules
            # TODO(asarfaty): Consider deleting the resources when the rule
            # or policy is removed or just try to delete all resources which
            # will delete only unused ones
            self._cleanup_router_fw_resources(domain_id, router_id)
    ...
    def _create_router_gateway_policy(self, context, domain_id, router_id,
        ...
        try:
            ...
            LOG.error("DEBUG ADIT _create_router_gateway_policy update policy"
                      "with rules %s", fw_rules)
        ...
        LOG.error("DEBUG ADIT _create_router_gateway_policy create policy "
                  "with rules %s", fw_rules)
    ...
    def delete_router_gateway_policy(self, domain_id, router_id):
        ...
        """
        LOG.error("DEBUG ADIT delete_router_gateway_policy %s %s", domain_id, router_id)
        try:
            self.nsxpolicy.gateway_policy.get(domain_id, map_id=router_id)
        except nsx_lib_exc.ResourceNotFound:
            return
    ...
    def _cleanup_router_fw_resources(self, domain_id, router_id):
        # Delete per rule & per network groups
        groups = self.nsxpolicy.search_by_tags(
            [{'scope': ROUTER_FW_TAG, 'tag': router_id}],
            self.nsxpolicy.group.entry_def.resource_type())['results']
        for group in groups:
            LOG.error("DEBUG ADIT deleting group %s", group)
