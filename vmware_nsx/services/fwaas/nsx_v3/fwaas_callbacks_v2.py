...
class Nsxv3FwaasCallbacksV2(com_callbacks.NsxFwaasCallbacksV2):
    ...
    def update_router_firewall(self, context, nsxlib, router_id,
        ...
        if from_fw:
            # fw action required
            if with_fw:
                # firewall exists in Neutron and sr is missing on backend -
                # create
        ...
        if with_fw and not exists_on_backend:
            LOG.error("Unable to update firewall, missing service router %s",
                      router_id)
