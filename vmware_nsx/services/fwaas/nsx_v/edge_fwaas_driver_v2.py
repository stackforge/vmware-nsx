...
class EdgeFwaasDriverV2(fwaas_base_v2.FwaasDriverBase):
    ...
    def __init__(self):
        LOG.debug("Loading FWaaS V2 NsxVDriver.")
        super(EdgeFwaasDriverV2, self).__init__()
    ...
    def _create_or_update_firewall_group(self, agent_mode, apply_list,
                                         firewall_group):
        # admin state down means default block rule firewall
        if not firewall_group['admin_state_up']:
            self.apply_default_policy(agent_mode, apply_list, firewall_group)
            return
        ...
        if not edges_map:
            ...
            LOG.warning("Cannot apply the firewall %(fw)s to any of the "
                        "routers %(rtrs)s",
                        {'fw': firewall_group['id'], 'rtrs': routers})
            return

        # Translate the FWaaS rules
        rules = self._translate_rules(firewall_group['firewall_rule_list'])
    ...
    def _delete_firewall_or_set_default_policy(self, apply_list,
                                               firewall_group,
                                               delete_fw=False):
        # get router-edge mapping
        context = n_context.get_admin_context()
        ...
        for router_info in apply_list:
            ...
            if info:
                self._set_rules_on_router_edge(
                    context, info['lookup_id'], neutron_id, info['edge_id'],
                    firewall_group['id'], rules, delete_fw=delete_fw)
    ...
    def delete_firewall_group(self, agent_mode, apply_list, firewall_group):
        ...
        """
        self._delete_firewall_or_set_default_policy(apply_list,
                                                    firewall_group,
                                                    delete_fw=True)
    ...
    def apply_default_policy(self, agent_mode, apply_list, firewall_group):
        ...
        """
        self._delete_firewall_or_set_default_policy(apply_list, firewall_group,
                                                    delete_fw=False)

    def get_firewall_translated_rules(self, firewall_group):
        if firewall_group['admin_state_up']:
            return self._translate_rules(firewall_group['firewall_rule_list'])
        return []
