...
class NsxFwaasCallbacks(firewall_l3_agent.L3WithFWaaS):
    ...
    def _get_router_ids_for_fw(self, context, fw, to_delete=False):
        ...
        LOG.error("DEBUG ADIT _get_router_ids_for_fw FW tenant: %s", fw['tenant_id'])
        ...
        if self._has_router_insertion_fields(fw):
            ...
            LOG.error("DEBUG ADIT _get_router_ids_for_fw ids: %s", ids)
            project_ids = [router['id'] for router in routers_in_proj
                           if router['id'] in ids]
            LOG.error("DEBUG ADIT _get_router_ids_for_fw project_ids: %s", project_ids)
    ...
    def should_apply_firewall_to_router(self, context, router_id):
        ...
        # if the router belongs to a different tenant than the FW - ignore it
        # DEBUG ADIT
        return True
