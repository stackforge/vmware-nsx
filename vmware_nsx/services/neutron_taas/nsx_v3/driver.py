...
class NsxV3Driver(base_driver.TaasBaseDriver,
    ...
    def delete_tap_flow_postcommit(self, context):
        ...
        dest_port_id = ts.get('port_id')
        if self._is_local_span(context, src_port_id, dest_port_id):
            self._delete_local_span(context, pm_session_mapping['port_mirroring_session_id'])
        else:
            self._delete_l3span(context, pm_session_mapping)
    ...
    def _delete_l3span(self, context, pm_session_mapping):
        tf = context.tap_flow
        src_port_id = tf.get('source_port')
    ...
    def _delete_local_span(self, context, pm_session_mapping_id):
        # Delete port mirroring session on the backend
        try:
            nsxlib.delete_port_mirror_session(pm_session_mapping_id)
        except nsx_exc.ManagerError:
            with excutils.save_and_reraise_exception():
                LOG.error(_LE("Unable to delete port mirror session %s "
                              "on NSX backend."), pm_session_mapping_id)
