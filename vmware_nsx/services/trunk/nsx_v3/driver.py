...
class NsxV3TrunkHandler(object):
    ...
    def _set_subports(self, trunk, subports):
        for subport in subports:
            # Update port with parent port for backend.
            pass

    def _unset_subports(self, subports):
        for subport in subports:
    ...
    def trunk_created(self, trunk):
        if trunk.sub_ports:
            self._set_subports(trunk.port_id, trunk.sub_ports)
        trunk.update(status=trunk_consts.ACTIVE_STATUS)

    def trunk_deleted(self, trunk):
        self._unset_subports(trunk.sub_ports)

    def subports_added(self, trunk, subports):
        self._set_subports(trunk.port_id, subports)
        trunk.update(status=trunk_consts.ACTIVE_STATUS)

    def subports_deleted(self, trunk, subports):
        self._unset_subports(subports)
        trunk.update(status=trunk_consts.ACTIVE_STATUS)
    ...
    def subport_event(self, resource, event, trunk_plugin, payload):
        if event == events.AFTER_CREATE:
            ...
            self.subports_deleted(payload.original_trunk, payload.subports)
