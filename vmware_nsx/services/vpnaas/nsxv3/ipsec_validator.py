...
class IPsecV3Validator(vpn_validator.VpnReferenceValidator):
    ...
    def validate_ipsec_site_connection(self, context, ipsec_site_conn):
        ...
        # regardless pf vpn/using the same vpn service (=TIER0) as the nsx
        # does not support it

        # TODO(asarfaty) validate no overlapping policy rules (of all tenants)
        # using the same vpn service (=TIER0)(?) as the nsx does not support it

        # It is not allowed to create separate connections (of all tenants)
        # on the same local address (tier1 router gw) and peer address
        ...
        for conn in connections:
            ...
            if srv_local == local_addr:
                # TODO(asarfaty): is it allowed for different TIER0 routers?
