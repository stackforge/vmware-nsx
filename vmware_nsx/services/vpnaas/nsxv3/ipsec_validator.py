...
class IPsecV3Validator(vpn_validator.VpnReferenceValidator):
    ...
    # DEBUG ADIT todo check which validation we really need
    ...
    def validate_auth_algorithm(self, policy_info):
        ...
        if encryption not in ipsec_utils.ENCRYPTION_ALGORITHM_MAP:
            msg = _("Unsupported encryption_algorithm: %(algo)s! please "
                    ...
                    "%(supported_algos)s") % {
                        'algo': encryption,
                        'supported_algos':
                        ipsec_utils.ENCRYPTION_ALGORITHM_MAP}
            raise nsx_exc.NsxVpnValidationError(details=msg)

    def validate_pfs(self, policy_info):
        pfs = policy_info['pfs']
    ...
    def validate_ike_policy(self, policy_info):
        ...
        self.validate_auth_algorithm(policy_info)
        self.validate_encryption_algorithm(policy_info)
        self.validate_pfs(policy_info)
