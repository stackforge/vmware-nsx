...
#TODO(asarfaty) support vpn-endpoint-group-create for local & peer cidrs too
class NSXv3IPsecVpnDriver(service_drivers.VpnDriver):
    ...
    def _create_local_endpoint_if_needed(self, context, connection,
        ...
        # TODO(asarfaty) search by tags or name when possible
        eps = self._nsx_vpn.local_endpoint.list()['results']
        for ep in eps:
            if (ep['local_address'] == local_addr and
                ep['ipsec_vpn_service_id'] == nsx_service_id):
                return ep['id']
    ...
    def _get_session_rules(self, context, connection, vpnservice_id):
    ...
    def update_ipsec_site_connection(self, context, old_ipsec_conn,
        ...
        # Note(asarfaty): local endpoints are resuable and will not be updated
        ...
        # update the tier1 logical router update_advertisement with a new flag
        # for vpn (here or when creating the service)
