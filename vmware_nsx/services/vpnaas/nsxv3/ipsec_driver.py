...
#TODO(asarfaty) add tags to all objects (also reused objects??)
class NSXv3IPsecVpnDriver(service_drivers.VpnDriver):
    ...
    def _get_session_rules(self):
        # DEBUG ADIT - not yet
        return [self._nsx_vpn.session.get_rule_obj()]
    ...
    def update_ipsec_site_connection(self, context, old_ipsec_conn,
        ...
        if update_all:
            self._update_session(mapping['session_id'], ipsec_site_conn, None)
        # DEBUG ADIT - update the session itself & rules!
