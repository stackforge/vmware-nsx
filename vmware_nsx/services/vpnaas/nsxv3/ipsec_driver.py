...
class NSXv3IPsecVpnDriver(service_drivers.VpnDriver):
    ...
    def _create_local_endpoint(self, connection, local_addr, nsx_service_id):
    ...
    def _create_local_endpoint_if_needed(self, context, connection,
                                         vpnservice_id):
        # The NSX allows only one local endpoint per local address
        # so we should reuse it.
        ...
        # TODO(asarfaty) from the backend and no DB
        local_ep_id = db.get_nsx_vpn_local_ep_mapping(
        ...
        local_ep_id = self._create_local_endpoint(
            connection, local_addr, nsx_service_id)
        db.add_nsx_vpn_local_ep_mapping(
            context.session, router_id, local_ep_id)
        return local_ep_id

    def _delete_local_endpoint(self, local_ep_id):
        # TODO(asarfaty): when the router is deleted/gw removed?
        self._nsx_vpn.local_endpoint.delete(local_ep_id)

    def _create_session(self, local_ep_id, peer_ep_id):
        #TODO(asarfaty): add rules for the specific subnets
        try:
            session = self._nsx_vpn.session.create(
                local_ep_id, peer_ep_id)
        except nsx_lib_exc.ManagerError as e:
    ...
    def _get_endpoints_from_session(self, session_id):
        session = self._nsx_vpn.session.get(session_id)
        return (
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_conn):
        ...
        try:
            ...
            local_ep_id = self._create_local_endpoint_if_needed(
                context, ipsec_site_conn, vpnservice_id)

            # Finally: create the session with policy rules
            session_id = self._create_session(local_ep_id, peer_ep_id)
            # DEBUG ADIT not yet with rules
    ...
    def delete_ipsec_site_connection(self, context, ipsec_site_conn):
        ...
        if not mapping:
            LOG.warning("Couldn't find nsx idd for vpn connection %s",
                      ipsec_site_conn['id'])
        ...
        # Do not delete the local endpint and service as they are reused
    ...
    def update_ipsec_site_connection(self, context, old_ipsec_conn,
        ...
        # TODO(asarfaty): check what other fields can change. peer cidrs?
    ...
    def _find_vpn_service(self, tier0_uuid):
        ...
        services = self._nsx_vpn.service.list()
        for srv in services:
            if srv.get('logical_router_id') == tier0_uuid:
                # if it exists but disabled: issue an error
                if not srv.get('enabled', True):
                    msg = _("NSX vpn service %s must be enabled") % srv['id']
                    raise nsx_exc.NsxPluginException(err_msg=msg)
                return srv['logical_router_id']
