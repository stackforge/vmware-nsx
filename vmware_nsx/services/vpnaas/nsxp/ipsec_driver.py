...
class NSXpIPsecVpnDriver(common_driver.NSXcommonIPsecVpnDriver):
    ...
    def _translate_cidr(self, cidr):
        return self._nsxpolicy.firewall_section.get_ip_cidr_reference(
            cidr,
    ...
    def _update_router_advertisement(self, context, vpnservice):
        ...
        #                                              router_id)
        #     self._nsxpolicy.logical_router.update_advertisement_rules(
    ...
    def _nsx_tags_for_reused(self):
        ...
        return self._nsxpolicy.build_v3_api_version_tag()
    ...
    def _create_ike_profile(self, context, connection):
        ...
        try:
            profile_id = self._nsx_vpn.ike_profile.create_or_overwrite(
                ...
                digest_algorithms=[ipsec_utils.AUTH_ALGORITHM_MAP[
                    ikepolicy['auth_algorithm']]],
                ike_version=ipsec_utils.IKE_VERSION_MAP[
                    ikepolicy['ike_version']],
                dh_groups=[ipsec_utils.PFS_MAP[ikepolicy['pfs']]],
    ...
    def _create_ipsec_profile(self, context, connection):
        ...
        tags = self._nsxpolicy.build_v3_tags_payload(
            ipsecpolicy, resource_type='os-vpn-ipsecpolicy-id',
            project_name=context.tenant_name)
        ...
        try:
            profile_id = self._nsx_vpn.tunnel_profile.create_or_overwrite(
                ...
                dh_groups=[ipsec_utils.PFS_MAP[ipsecpolicy['pfs']]],
    ...
    def _create_local_endpoint(self, context, connection, vpnservice):
        ...
        try:
            local_endpoint_id = self._nsx_vpn.local_endpoint.create_or_overwrite(
    ...
    def _delete_local_endpoint(self, resource, event, trigger, **kwargs):
        ...
        if port:
            self._nsx_vpn.local_endpoint.delete(router_id, router_id, router_id)
        ...
        if not services:
            try:
                ...
                LOG.error("Failed to delete VPN service %s: %s",
                          router_id, e)
        # DEBUG ADIT - check if service router should be deleted
