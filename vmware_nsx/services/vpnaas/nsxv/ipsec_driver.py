...
class NSXvIPsecVpnDriver(service_drivers.VpnDriver):
    ...
    def _update_firewall_rules(self, context, ipsec_site_connection,
        ...
        with locking.LockManager.get_lock(edge_id):
            fake_fw_rules = self._get_subnets_dnat_firewall_rules(context,
                                                                  router_db,
                                                                  router_id)
            ipsec_fw_rules = self._generate_ipsecvpn_firewall_rules(edge_id)
            fake_fw_rules.extend(ipsec_fw_rules)
    ...
    def delete_ipsec_site_connection(self, context, ipsec_site_conn):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to delete ipsec site connection "
    ...
    def _update_ipsec_config(self, edge_id, sites, enabled=True):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to update ipsec vpn "
                          "configuration with edge_id: %s", edge_id)
