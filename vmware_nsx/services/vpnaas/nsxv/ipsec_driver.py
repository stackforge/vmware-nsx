...
class NSXvIPsecVpnDriver(service_drivers.VpnDriver):
    ...
    def _get_subnets_dnat_firewall_rules(self, context, router, router_id):
        fake_fw_rules = []
        subnet_cidrs = (
            self._core_plugin._find_router_subnets_cidrs(context,
                                                         router['id']))
        ...
        dnat_rules = self._core_plugin._get_nat_rules(context, router)[1]
    ...
    def _update_firewall_rules(self, context, ipsec_site_connection,
        ...
        vpnservice = self.service_plugin._get_vpnservice(context,
                                                         vpnservice_id)
        router_db = self._core_plugin._get_router(context,
    ...
    def _update_status(self, context, vpn_service_id, ipsec_site_conn_id,
        ...
        ipsecvpn_driver = (self.service_plugin.
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_connection):
        LOG.info('Creating ipsec site connection %(conn_info)s.',
        ...
        try:
            LOG.info('Updating ipsec vpn configuration %(vse_sites)s.',
                     {'vse_sites': vse_sites})
            self._update_ipsec_config(edge_id, vse_sites, enabled=True)
        except vcns_exc.VcnsApiException:
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to create ipsec site connection "
        ...
        LOG.info('Updating ipsec vpn firewall')
        try:
    ...
    def delete_ipsec_site_connection(self, context, ipsec_site_conn):
        LOG.info('Deleting ipsec site connection %(site)s.',
        ...
        del_site, vse_sites = self._find_vse_site(context, edge_id,
                                                  ipsec_site_conn)
        if not del_site:
            LOG.exception("Failed to find ipsec_site_connection "
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception("Failed to delete ipsec site connection "
    ...
    def update_ipsec_site_connection(self, context, old_ipsec_conn,
                                     ipsec_site_connection):
        LOG.info('Updating ipsec site connection %(site)s.',
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception("Failed to create ipsec site connection "
                              "configuration with %(edge_id)s.",
                              {'edge_id': edge_id})
                # Update status to error
                self._update_status(context, vpnservice_id, ipsec_id,
    ...
    def create_vpnservice(self, context, vpnservice):
        LOG.info('Creating VPN service %(vpn)s', {'vpn': vpnservice})
