...
class EdgeVPNDriver(object):
    ...
    def _get_edge_id(self, context, ipsec_site_connection):
        ...
        if edge_binding['edge_type'] == nsxv_constants.VDR_EDGE:
            # get plr edge_id and enable edge
    ...
    def _convert_ipsec_conn(self, context, ipsec_site_connection):
        ...
        'enabled': True,
            ...
            'encryptionAlgorithm': nsxv_constants.ENCRYPTION_ALGORITHM_MAP.get(
    ...
    def _generate_new_firewall(self, edge_id, peersubnets):
        ...
        ipsec_vpn_fw_rules = edge_firewall_driver.get_firewall(context, edge_id)
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_connection):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                ...
                self.vpn_plugin.delete_ipsec_site_connection(context, conn_info.get("id"))
        """try:        
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE("Failed to update firewall rule configuration "
    ...
    def update_ipsec_site_connection(self, context, old_ipsec_site_id,
                                     ipsec_site_connection):
        LOG.debug('Updating ipsec site connection %s', ipsec_site_connection)
        conn_info = self.vpn_plugin.get_ipsec_site_connection(old_ipsec_site_id)
        edge_id = self._get_edge_id(context, conn_info)
        ...
        if ipsec_site_connection['peer_cidrs'] != old_site['peerSubnets']['subnets']:
            vse_sites = self._delete_site_dict(edge_id, old_site)
            ...
            try:
                ...
                ipsec_vpn_fw_rules.append({
                    ...
                    'enabled': True,
                    'source_vnic_groups': ["external"],
                    'destination_ip_address': ipsec_site_connection['peer_cidrs']})
                edge_utils.update_firewall(self.plugin.nsx_v, context, router_id,
                                           {'firewall_rule_list': ipsec_vpn_fw_rules},
                                            allow_external=False)
            except vcns_exc.VcnsApiException:
