...
class Nsxv3IpamDriver(subnet_alloc.SubnetAllocator, Nsxv3IpamBase):
    ...
    def __init__(self, subnetpool, context):
        ...
        self.nsxlib_ipam = resources.IpPool(self.get_core_plugin().nsxlib.client) 
    ...
    def get_subnet_request_factory(self):
        ...
        return Nsxv3SubnetRequestFactory
    ...
    def allocate_backend_pool(self, subnet_request):
        ...
        LOG.error("DEBUG ADIT ipam v3 driver - create!")
        if subnet_request.allocation_pools:
            ranges = [
        ...
        #nsx_pool_id = None
        try:
            LOG.error("DEBUG ADIT ipam v3 driver - create calling nsxlib")
            response = self.nsxlib_ipam.create(display_name=name, gateway_ip=subnet_request.gateway_ip, ranges=ranges)
            LOG.error("DEBUG ADIT ipam v3 driver - response = %s", response)
...
class Nsxv3IpamSubnet(ipam_base.Subnet, Nsxv3IpamBase):
...
class Nsxv3SubnetRequestFactory(ipam_req.SubnetRequestFactory, Nsxv3IpamBase):
    """Builds request using subnet info, including the network id"""

    @classmethod
    def get_request(cls, context, subnet, subnetpool):
        req = super(Nsxv3SubnetRequestFactory, cls).get_request(
