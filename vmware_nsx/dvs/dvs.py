...
te_cluster_rules_spec(self, client_factory, name, vm_group_name,
                                   host_group_name, operation="add"):
    ...
    def update_cluster_edge_failover(self, resource_id, vm_moids,
        ...
        cluster = session.invoke_api(
            vim_util, "get_object_property", self._session.vim, resource,  "owner")
        vms = [vim_util.get_moref(vm_moid, 'VirtualMachine') for
               vm_moid in vm_moids]
        client_factory = session.vim.client.factory
        config_spec = client_factory.create('ns0:ClusterConfigSpecEx')
        cluster_config = session.invoke_api(
            vim_util, "get_object_property", self._session.vim, cluster,  "configurationEx")
        # Create the VM groups
        config_spec.groupSpec = [
            self._create_vm_group_spec(client_factory, 'neutron-group-%s-%s' % (edge_id, index), [vm])
    ...
    def cluster_edge_delete(self, resource_id, edge_id):
        ...
        #            client_factory, group.name, group=group))
        #if groupSpec:
