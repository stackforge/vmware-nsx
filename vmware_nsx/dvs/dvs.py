...
class DvsManager(object):
    ...
    def _reconfigure_port_group(self, pg_moref,
      spec_update_calback, spec_update_data):
        # Get the current configuration of hte port group
        ...
        if len(pg_spec) == 0 or len(pg_spec[0].propSet[0]) == 0:
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE('Failed to ReconfigureDVPortgroup'))
    ...
    # Finding the port group usingthe exact moref of the network
    # update the spec using a callback and user data
