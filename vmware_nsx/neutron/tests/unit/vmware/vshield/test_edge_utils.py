...
from neutron.plugins.vmware.dbexts import nsxv_db
from neutron.plugins.vmware.vshield.common import constants as vcns_const
from neutron.plugins.vmware.vshield import edge_utils
from neutron.tests import base
from neutron.tests.unit.vmware import VCNS_DRIVER_NAME

PLUGIN_NAME = 'neutron.plugins.vmware.plugin.NsxVPlugin'
...
class EdgeUtilsTestCaseMixin(base.BaseTestCase):

    def setUp(self):
        super(EdgeUtilsTestCaseMixin, self).setUp()
        db.configure_db()
        nsxv_manager_p = mock.patch(VCNS_DRIVER_NAME, autospec=True)
...
class EdgeDHCPManagerTestCase(EdgeUtilsTestCaseMixin):
    ...
    def setUp(self):
        ...
        self.edge_manager = edge_utils.EdgeManager(self.nsxv_manager)
...
class EdgeManagerTestCase(EdgeUtilsTestCaseMixin):
    ...
    def setUp(self):
        ...
        self.default_edge_pool_dicts = {
            vcns_const.SERVICE_EDGE: {
                vcns_const.LARGE: {'minimum_pooled_edges': 1,
                                   'maximum_pooled_edges': 3},
                vcns_const.COMPACT: {'minimum_pooled_edges': 1,
                                     'maximum_pooled_edges': 3}},
            vcns_const.VDR_EDGE: {}}
    ...
    def _create_available_router_bindings(self, num, size=vcns_const.LARGE,
    ...
    def _create_creating_router_bindings(self, num, size=vcns_const.LARGE,
                                         edge_type=vcns_const.SERVICE_EDGE):
    ...
    def _create_error_router_bindings(self, num, status=plugin_const.ERROR,
                                      size=vcns_const.LARGE,
                                      edge_type=vcns_const.SERVICE_EDGE):
    ...
    def _create_error_router_bindings_at_backend(
        ...
        size=vcns_const.LARGE, edge_type=vcns_const.SERVICE_EDGE):
    ...
    def _create_deleting_router_bindings(self, num, size=vcns_const.LARGE,
                                         edge_type=vcns_const.SERVICE_EDGE):
    ...
    def test_get_backup_edge_bindings(self):
        ...
        pool_edges = (self._create_edge_pools(1, 2, 3, 4, 5) +
                      self._create_edge_pools(
                          1, 2, 3, 4, 5, size=vcns_const.COMPACT))
    ...
    def test_check_backup_edge_pool_with_min(self):
        ...
        router_bindings = [
            binding
            for binding in nsxv_db.get_nsxv_router_bindings(self.ctx.session)
            if binding['edge_id'] == None
            and binding['status'] == plugin_const.PENDING_CREATE]
        self.assertEqual(2, len(router_bindings))
        self.nsxv_manager.deploy_edge.assert_has_calls(
            [mock.call(mock.ANY, mock.ANY, internal_network=None,
                       jobdata=mock.ANY, wait_for_exec=True,
    ...
    def test_check_backup_edge_pools_with_empty_conf(self):
        pool_edges = (self._create_edge_pools(1, 2, 3, 4, 5) +
                      self._create_edge_pools(
                          1, 2, 3, 4, 5, size=vcns_const.COMPACT) +
                      self._create_edge_pools(
                          1, 2, 3, 4, 5, edge_type=vcns_const.VDR_EDGE))
        self._populate_vcns_router_binding(pool_edges)
    ...
    def test_check_backup_edge_pools_with_default(self):
        ...
        large_bindings = [
            binding
            for binding in router_bindings
            if (binding['appliance_size'] == vcns_const.LARGE and
                binding['edge_type'] == vcns_const.SERVICE_EDGE)]
        ...
        expect_compact_bindings = self._create_backup_router_bindings(
            ...
            error_status=plugin_const.PENDING_DELETE,
            error_at_backend_status=plugin_const.PENDING_DELETE,
            size=vcns_const.COMPACT)
        compact_bindings = [
            binding
            for binding in router_bindings
            if (binding['appliance_size'] == vcns_const.COMPACT and
                binding['edge_type'] == vcns_const.SERVICE_EDGE)]
        ...
        vdr_bindings = [
            ...
            for binding in router_bindings
            if (binding['appliance_size'] == vcns_const.LARGE and
                binding['edge_type'] == vcns_const.VDR_EDGE)]
    ...
    def test_check_backup_edge_pools_with_vdr(self):
        ...
        service_bindings = [
            binding
            for binding in router_bindings
            if binding['edge_type'] == vcns_const.SERVICE_EDGE]
    ...
    def test_allocate_large_edge_appliance_with_default(self):
        ...
        self.edge_manager._allocate_edge_appliance(
            self.ctx, 'fake_id', 'fake_name')
        edge_id = (EDGE_AVAIL + vcns_const.LARGE + '-' +
                   vcns_const.SERVICE_EDGE + '-edge-' + str(0))
        self.nsxv_manager.update_edge.assert_has_calls(
            [mock.call('fake_id', edge_id, 'fake_name', None,
                       appliance_size=vcns_const.LARGE, dist=False)])
    ...
    def test_allocate_compact_edge_appliance_with_default(self):
        ...
        self.edge_manager._allocate_edge_appliance(
            self.ctx, 'fake_id', 'fake_name',
            appliance_size=vcns_const.COMPACT)
        edge_id = (EDGE_AVAIL + vcns_const.COMPACT + '-' +
                   vcns_const.SERVICE_EDGE + '-edge-' + str(0))
        self.nsxv_manager.update_edge.assert_has_calls(
            [mock.call('fake_id', edge_id, 'fake_name', None,
                       appliance_size=vcns_const.COMPACT, dist=False)])
    ...
    def test_allocate_large_edge_appliance_with_vdr(self):
        ...
        edge_id = (EDGE_AVAIL + vcns_const.LARGE + '-' +
                   vcns_const.VDR_EDGE + '-edge-' + str(0))
        self.nsxv_manager.update_edge.assert_has_calls(
            [mock.call('fake_id', edge_id, 'fake_name', None,
                       appliance_size=vcns_const.LARGE, dist=True)])
