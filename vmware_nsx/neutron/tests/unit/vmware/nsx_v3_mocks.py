...
class FakeManager(client.NsxManagerBase):
    ...
    def create_resource(self, resource, data):
        if resource == 'logical-switches':
            lswitch = FAKE_SWITCH.copy()
            lswitch.update({'id': uuidutils.generate_uuid(),
                           'transport_zone_id': uuidutils.generate_uuid()})
            return lswitch
        elif resource == 'logical-ports':
            lport = FAKE_PORT.copy()
            lport.update({'id': uuidutils.generate_uuid(),
