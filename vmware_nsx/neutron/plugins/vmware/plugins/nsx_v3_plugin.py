...
class NsxV3Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _add_fip_nat_rules(self, logical_router_id, ext_ip, int_ip):
    ...
    def _delete_fip_nat_rules(self, logical_router_id, ext_ip, int_ip):
    ...
    def _add_static_routes(self, nsx_router_id, route):
        return nsxlib.add_static_route(nsx_router_id, route['destination'],
                                       route['nexthop'])

    def _delete_static_routes(self, nsx_router_id, route):
    ...
    def delete_router(self, context, router_id):
        ...
        nsx_router_id = nsx_db.get_nsx_router_id(context.session,
                                                 router_id)
        # TODO(berlin): remove router bindings here
        try:
            ...
            if 'routes' in r:
                ...
                old_routes, routes_dict = (
                    self._get_extra_routes_dict_by_router_id(
                        context, router_id))
                added, removed = n_utils.diff_list_of_dict(old_routes,
                                                         new_routes)
                nsx_router_id = nsx_db.get_nsx_router_id(context.session,
                                                         router_id)
                try:
                    for route in removed:
                        self._delete_static_routes(nsx_router_id, route)
                except nsx_exc.ResourceNotFound:
                    LOG.warning(_LW("Backend static rules for dest: %(dest)s, "
                                    "nexthop: %(nexthop)s on router "
                                    "%(router_id)s not found"),
                                {'dest': route.get('destination'),
                                 'nexthop': route.get('floating_ip_address'),
                                 'router_id': router_id})
                for route in added:
                    self._add_static_routes(nsx_router_id, route)
    ...
    def update_floatingip(self, context, fip_id, floatingip):
        old_fip_db = self._get_floatingip(context, fip_id)
        old_port_id = old_fip_db.fixed_port_id
        ...
        try:
            ...
            if old_fip_db.router_id:
                try:
                    old_nsx_router_id = nsx_db.get_nsx_router_id(
                        context.session, old_fip_db.router_id)
                    self._delete_fip_nat_rules(
                        old_nsx_router_id, old_fip_db.floating_ip_address,
                        old_fip_db.fixed_ip_address)
                except nsx_exc.ResourceNotFound:
                    LOG.warning(_LW("Backend NAT rules for fip: %(fip_id)s "
                                    "(ext_ip: %(ext_ip)s int_ip: %(int_ip)s) "
                                    "not found"),
                                {'fip_id': old_fip_db.id,
                                 'ext_ip': old_fip_db.floating_ip_address,
                                 'int_ip': old_fip_db.fixed_ip_address})
