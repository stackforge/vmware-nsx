...
class NsxV3Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def create_port(self, context, port):
        ...
        with context.session.begin(subtransactions=True):
            ...
            if port['port'].get('device_owner') == (nsx_constants.
                                                    BRIDGE_ENDPOINT):
                # NSX backend requires the vif id be set to bridge endpoint id
                # for ports plugged into a Bridge Endpoint.
                vif_uuid = port['port'].get('device_id')
                attachment_type = port['port'].get('device_owner')
            result = nsxlib.create_logical_port(
                lswitch_id=port['port']['network_id'],
                vif_uuid=vif_uuid, name=port['port']['name'], tags=tags,
