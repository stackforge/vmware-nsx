...
class NsxVPluginV2(agents_db.AgentDbMixin,
    ...
    def delete_network(self, context, id):
        ...
        if auto_del:
            ...
            if len(sids) > 0:
                edge_backend = (
                    self.edge_manager.get_dhcp_edge_binding_at_backend(
                        context, id))
                edge_type = (edge_backend.get('edge_type')
                             if edge_backend else None)
        ...
        if subnet['enable_dhcp']:
            ...
            if len(remaining_subnets) == 0:
                if is_multi_dhcp is True:
                    LOG.debug("Delete service container edge for network %s",
                              network_id)
                    self.edge_manager.unbind_multi_context_dhcp_edge(
                        context, network_id)
                elif is_multi_dhcp is False:
                ...
                address_groups = self._create_network_dhcp_address_group(
                    context, network_id)
                if is_multi_dhcp is True:
                    LOG.debug("Update service container edge for network %s",
                              network_id)
                    ...
                    LOG.warning(_LW("No backend edge for updating the "
