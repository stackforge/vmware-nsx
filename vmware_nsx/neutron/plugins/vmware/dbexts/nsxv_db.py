...
def add_neutron_nsx_section_mapping(session, neutron_id, ip_section_id,
                                    mac_section_id=None):
    with session.begin(subtransactions=True):
        mapping = nsxv_models.NeutronNsxSectionMapping(
...
def add_neutron_nsx_rule_mapping(session, neutron_id, nsx_rule_id):
    with session.begin(subtransactions=True):
        mapping = nsxv_models.NeutronNsxRuleMapping(neutron_id=neutron_id,
...
def add_neutron_nsx_port_vnic_mapping(session, neutron_id, nsx_id):
    with session.begin(subtransactions=True):
        mapping = nsxv_models.NeutronNsxPortVnicMapping(
...
def get_nsx_section(session, neutron_id):
    try:
        mapping = (session.query(nsxv_models.NeutronNsxSectionMapping).
...
def get_nsx_rule_id(session, neutron_id):
    try:
        mapping = (session.query(nsxv_models.NeutronNsxRuleMapping).
                   filter_by(neutron_id=neutron_id).
                   one())
...
def get_nsx_vnic_id(session, neutron_id):
    try:
        mapping = (session.query(nsxv_models.NeutronNsxPortVnicMapping).
                   filter_by(neutron_id=neutron_id).
                   one())
