...
def create_logical_router_port(router_id, resource_type,
                                cidr_length, ip_addresses, port_id):
    controller = _get_controller()
    ...
    body = {'id': router_id,
            ...
            'linked_logical_switch_port_id': port_id}
...
def delete_logical_router_port(port_id):
    controller = _get_controller()
    url = controller + "/api/v1/logical-router-ports/%s?detach=true" % port_id
    headers = {'Content-Type': 'application/json'}
    result = requests.delete(url, auth=auth.HTTPBasicAuth('admin', 'default'),
                             verify=False, headers=headers)
