...
#from vmware_nsx.neutron.tests.unit.vmware import nsx_v3_mocks as nsclient
...
# TBD(roeyc): support for other l4 services ?
...
def get_nsservice(resource_type, **properties):
    ...
    for k, p in list(properties.items()):
        if p is None:
            del properties[k]
...
def remove_nsgroup_member(nsgroup_id, target_id):
    nsgroup = read_nsgroup(nsgroup_id)
    members = [member['value'] for member in nsgroup.get('members', [])]
    for i, member_id in enumerate(members):
        if target_id == member_id:
            nsgroup['members'].remove(target_id)
            break
    return nsclient.update_resource('ns-groups/%s' % nsgroup_id, nsgroup)
...
def create_empty_section(display_name, description, applied_tos,
                         operation=(INSERT_BOTTOM, ), **tags):
    resource = 'firewall/sections'
    body = _build_section(display_name, description, applied_tos, tags)

    if operation[0] == INSERT_BEFORE:
        # Insert the section before the one is specified.
        resource += '?operation=%s&id=%s' % operation

    return nsclient.create_resource(resource, body)


def create_section_with_rules(display_name, description, applied_tos, rules,
                              operation=(INSERT_BOTTOM, ), **tags):
    resource = 'firewall/sections?action=create_with_rules'
    body = _build_section(display_name, description, applied_tos, tags)
    body['rules'] = rules
