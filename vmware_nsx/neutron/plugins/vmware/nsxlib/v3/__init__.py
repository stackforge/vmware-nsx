...
def delete_resource_by_values(resource, id='id', **kwargs):
    ...
    for res in resources_get['results']:
        ...
        if is_matched:
            ...
            delete_resource = resource + "/" + res[id]
...
def get_logical_router_port_by_ls_id(logical_switch_id):
    ...
    # TODO(berlin): here actually returns a resource with "result_count" and
    # "results"
...
def create_logical_router_port(logical_router_id,
    ...
    if address_groups:
        body['address_groups'] = address_groups
...
def delete_nat_rule_by_values(logical_router_id, **kwargs):
    resource = 'logical-routers/%s/nat/rules' % logical_router_id
    return delete_resource_by_values(resource, id='rule_id', **kwargs)
