...
def _create_firewall_rules(context, section_id, nsgroup_id,
...
def _save_sg_mappings(session, sg_id, nsgroup_id, section_id):
    with session.begin(subtransactions=True):
        session.add(
...
def _get_sg_rule_mapping(session, rule_id):
    return session.query(nsx_models.NeutronNsxRuleMapping).filter_by(
        neutron_id=rule_id).one()


def _get_sg_mappings(session, sg_id):
...
def _get_remote_nsg_mapping(context, remote_sg_id, rule_sg_id):
    remote_nsgroup_id = None
    # skip unnecessary db access when possible
    if remote_sg_id == rule_sg_id:
        remote_nsgroup_id = rule_sg_id
    elif remote_sg_id:
        remote_nsgroup_id, _ = _get_sg_mappings(context.session, rule_sg_id)
    return remote_nsgroup_id


def _update_lport_with_security_groups(context, lport_id, original, updated):
    added = set(updated) - set(original)
    removed = set(original) - set(updated)
    for sg_id in added:
        nsgroup_id, _ = _get_sg_mappings(context.session, sg_id)
