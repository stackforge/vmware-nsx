...
from neutron.plugins.vmware.vshield.tasks import constants as task_const
...
class EdgeManager(object):
    ...
    def _deploy_backup_edges(self, context, num,
        ...
        with context.session.begin(subtransactions=True):
            for router_id in router_ids:
                vcns_db.add_vcns_router_binding(
    ...
    def _clean_all_error_edge_bindings(self, context):
        router_bindings = vcns_db.get_vcns_router_bindings(context.session)
    ...
    def _get_physical_provider_network(self, context, network_id):
        phy_net = nsx_db.get_network_bindings(context.session, network_id)
    ...
    def _delete_dhcp_internal_interface(self, context, edge_id, vnic_index,
        ...
        bindings = vcns_db.get_vcns_router_bindings(context.session)
    ...
    def _allocate_edge_appliance(self, context, resource_id, name,
        ...
        if not available_router_binding:
            ...
            vcns_db.delete_vcns_router_binding(
    ...
    def update_dhcp_service_config(self, context, edge_id):
        ...
        edge_vnic_bindings = vcns_db.get_edge_vnic_bindings_by_edge(
        ...
        vcns_db.clean_edge_dhcp_static_bindings_by_edge(
            context.session, edge_id)
        for mac_address, binding_id in bindings_get.items():
            vcns_db.create_edge_dhcp_static_binding(context.session, edge_id,
    ...
    def create_dhcp_edge_service(self, context, network_id,
        ...
        resource_id = (vcns_const.DHCP_EDGE_PREFIX + network_id)[:36]
        dhcp_edge_binding = vcns_db.get_vcns_router_binding(context.session,
                                                            resource_id)
        ...
        if dhcp_edge_binding:
            ...
            if edge_id in conflict_edge_ids:
                old_binding = vcns_db.get_edge_vnic_binding(
                ...
                vcns_db.free_edge_vnic_by_network(context.session, edge_id,
                                                  network_id)
        ...
        # case 2: attach the subnet to a new edge and update vnic
            ...
            if available_edge_ids:
                ...
                self._allocate_dhcp_edge_appliance(context, resource_id)
                new_edge = vcns_db.get_vcns_router_binding(context.session,
                                                           resource_id)
                vcns_db.allocate_edge_vnic_with_tunnel_index(
                    context.session, new_edge['edge_id'], network_id)
    ...
    def update_dhcp_edge_service(self, context, network_id,
        ...
        resource_id = (vcns_const.DHCP_EDGE_PREFIX + network_id)[:36]
        edge_binding = vcns_db.get_vcns_router_binding(context.session,
                                                       resource_id)
        dhcp_binding = vcns_db.get_edge_vnic_binding(context.session,
    ...
    def delete_dhcp_edge_service(self, context, network_id):
        ...
        edge_binding = vcns_db.get_vcns_router_binding(context.session,
                                                       resource_id)
        if edge_binding:
            dhcp_binding = vcns_db.get_edge_vnic_binding(
            ...
            if dhcp_binding:
                ...
                vcns_db.free_edge_vnic_by_network(context.session,
                                                  edge_id,
                                                  network_id)
                try:
                    self._delete_dhcp_internal_interface(context, edge_id,
    ...
    def get_plr_by_tlr_id(self, context, router_id):
        lswitch_id = vcns_db.get_vcns_router_binding(
            context.session, router_id).lswitch_id
        if lswitch_id:
            edge_vnic_bindings = vcns_db.get_edge_vnic_bindings_by_int_lswitch(
                context.session, lswitch_id)
            if edge_vnic_bindings:
                for edge_vnic_binding in edge_vnic_bindings:
                    plr_router_id = vcns_db.get_vcns_router_binding_by_edge(
    ...
    def create_plr_with_tlr_id(self, context, router_id, router_name):
        ...
        # to a PLR
        tlr_edge_id = vcns_db.get_vcns_router_binding(
        ...
        tlr_vnic_index = self.nsxv_manager.add_vdr_internal_interface(
            ...
            type="uplink")
        vcns_db.create_edge_vnic_binding(
        ...
        plr_vnic_index = vcns_db.allocate_edge_vnic(
    ...
    def delete_plr_by_tlr_id(self, context, plr_id, router_id):
        ...
        tlr_binding = vcns_db.get_vcns_router_binding(
            context.session, router_id)
        lswitch_id = tlr_binding.lswitch_id
        tlr_edge_id = tlr_binding.edge_id
        plr_edge_id = vcns_db.get_vcns_router_binding(
            context.session, plr_id).edge_id
        plr_vnic_index = vcns_db.get_edge_vnic_binding(
            context.session, plr_edge_id, lswitch_id).vnic_index
        ...
        task.wait(task_const.TaskState.RESULT)
        vcns_db.free_edge_vnic_by_network(
        ...
        tlr_vnic_index = vcns_db.get_edge_vnic_binding(
        ...
        vcns_db.delete_edge_vnic_binding_by_network(
...
def delete_dhcp_service(context, nsxv_manager, network_id):
    ...
    task = None
    binding = vcns_db.get_dhcp_edge_network_binding(context.session,
                                                    network_id)
    if binding:
    ...
    if edge_id:
        ...
        for mac_address, binding_id in bindings_get.items():
            if mac_address in mac_address_list:
                vcns_db.create_edge_dhcp_static_binding(
                    context.session, edge_id,
                    mac_address, binding_id)
...
def delete_dhcp_binding(context, nsxv_manager, network_id, mac_address):
    edge_id = get_dhcp_edge_id(context, network_id)
    dhcp_binding = vcns_db.get_edge_dhcp_static_binding(
    ...
    if edge_id and dhcp_binding:
        ...
        vcns_db.delete_edge_dhcp_static_binding(
            context.session, edge_id, mac_address)
...
def update_routes(edge_manager, context, router_id, routes, nexthop=None):
    ...
    for route in routes:
        ...
        if route.get('external'):
            ...
            edge_routes.append({
                'vnic_index': vcns_db.get_edge_vnic_binding(
                    context.session, edge_id,
...
def update_routes_on_plr(edge_manager, context, plr_id, router_id,
    ...
    binding = vcns_db.get_vcns_router_binding(context.session, plr_id)
    edge_id = binding['edge_id']
    lswitch_id = vcns_db.get_vcns_router_binding(
        context.session, router_id).lswitch_id
    edge_vnic_binding = vcns_db.get_edge_vnic_binding(
...
def update_external_interface(
    ...
    binding = vcns_db.get_vcns_router_binding(context.session, router_id)
    net_bindings = nsx_db.get_network_bindings(context.session, ext_net_id)
    if not net_bindings:
...
def update_internal_interface(
    ...
    if not edge_vnic_binding:
        edge_vnic_binding = vcns_db.allocate_edge_vnic(
            context.session, edge_id, int_net_id)
...
def update_dnat_rules(nsxv_manager, context, router_id, dnat_rules):
    rtr_binding = vcns_db.get_vcns_router_binding(context.session, router_id)
    edge_id = rtr_binding['edge_id']

    for dnat_rule in dnat_rules:
        vnic_binding = vcns_db.get_edge_vnic_binding(
            context.session,
            edge_id,
...
def update_firewall(nsxv_manager, context, router_id, firewall,
                    allow_external=True):
    jobdata = {'context': context}
    edge_id = vcns_db.get_vcns_router_binding(
        context.session, router_id)['edge_id']
...
class NsxVCallbacks(object):
    ...
    def edge_deploy_started(self, task):
        ...
        if edge_id:
            ...
            if not dist:
                ...
                vcns_db.init_edge_vnic_binding(
                    context.session, edge_id)
        else:
    ...
    def edge_deploy_result(self, task):
        ...
        if task.status == task_const.TaskStatus.COMPLETED:
            ...
            vcns_db.update_vcns_router_binding(
                ...
                status=plugin_const.ERROR)
            if not dist:
                vcns_db.clean_edge_vnic_binding(
    ...
    def edge_delete_result(self, task):
        ...
        context = jobdata['context']
        try:
            vcns_db.delete_vcns_router_binding(context.session, router_id)
            if not dist:
                vcns_db.clean_edge_vnic_binding(context.session, edge_id)
