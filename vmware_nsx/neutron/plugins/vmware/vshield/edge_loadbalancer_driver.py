...
def convert_lbaas_app_profile(name, sess_persist, protocol):
    ...
    if sess_persist.get('type'):
        ...
        if (protocol != LB_PROTOCOL_HTTP and
            sess_persist['type'] != (
                LB_SESSION_PERSISTENCE_SOURCE_IP)):
...
def convert_lbaas_vip(vip, app_profile_id, pool_mapping):
    ...
    """
    pool_vse_id = pool_mapping['edge_pool_id']
    return {
        'name': 'vip_' + vip['id'],
...
def extract_resource_id(location_uri):
    ...
    return uri_elements[len(uri_elements) - 1]
...
def get_subnet_primary_ip(ip_addr, address_groups):
    ...
    for address_group in address_groups['addressGroups']:
        net_addr = '%s/%s' % (address_group['primaryAddress'],
                              address_group['subnetPrefixLength'])
        if netaddr.IPAddress(ip_addr) in netaddr.IPNetwork(net_addr):
            return address_group['primaryAddress']


def is_address_in_same_subnet(ip_addr, address_groups, action=None):
    """
    Lookup an address group with a matching subnet to ip_addr.
    If found, return True, else False.
    If action is specified, perform action on the address group which was
    found.
    """
    for address_group in address_groups['addressGroups']:
        net_addr = '%s/%s' % (address_group['primaryAddress'],
...
def add_address_to_address_groups(ip_addr, address_groups):
    ...
    is_address_in_same_subnet(ip_addr, address_groups, add_action)
...
def del_address_from_address_groups(ip_addr, address_groups):
    ...
    """
    def del_action(address_group):
        sec_addr = address_group.get('secondaryAddresses')
        if sec_addr and ip_addr in sec_addr['ipAddress']:
            sec_addr['ipAddress'].remove(ip_addr)

    is_address_in_same_subnet(ip_addr, address_groups, del_action)
...
class EdgeLbDriver(object):
    def __init__(self):
        ...
        self._lb_driver = None

    def _get_lb_driver(self):
        if not self._lb_driver:
            loaded_plugins = manager.NeutronManager().get_service_plugins()
            plugins = loaded_plugins[constants.LOADBALANCER]
            self._lb_driver = plugins.drivers['vmwareedge']

        return self._lb_driver
    ...
    def _get_lb_edge_id(self, context, subnet_id):
        ...
        attached_routers = self.callbacks.plugin.get_ports(
            neutron_context.get_admin_context(), filters=filters,
            fields=['device_id'])
    ...
    def _add_vip_as_secondary_ip(self, edge_id, vip):
            ...
            for vnic in vnics:
                if vnic['type'] == 'trunk':
                    for sub_interface in vnic.get('subInterfaces').get(
                        ...
                        if is_address_in_same_subnet(vip, address_groups):
                            add_address_to_address_groups(vip, address_groups)
        ...
        raise nsxv_exc.VcnsApiException
            ...
            for vnic in vnics:
                if vnic['type'] == 'trunk':
                    for sub_interface in vnic.get('subInterfaces').get(
                        ...
                        address_groups = sub_interface.get('addressGroups')
                        if is_address_in_same_subnet(vip, address_groups):
                            del_address_from_address_groups(vip,
                                                            address_groups)
                            self.vcns.update_interface(edge_id, vnic)
                            return
                else:
                    address_groups = vnic.get('addressGroups')
                    if is_address_in_same_subnet(vip, address_groups):
                        del_address_from_address_groups(vip, address_groups)
    ...
    def _update_pool_fw_rule(self, context, pool_id, edge_id,
        ...
        members = self._get_lb_driver().plugin.get_members(
            context,
    ...
    def create_pool(self, context, pool):
        ...
        try:
            ...
            self._get_lb_driver().create_pool_successful(
                context, pool, edge_id, edge_pool_id)

        except nsxv_exc.VcnsApiException:
            self._get_lb_driver().pool_failed(context, pool)
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE('Failed to create pool %s') % pool['id'])
    ...
    def update_pool(self, context, old_pool, pool, pool_mapping):
        ...
        try:
            self.vcns.update_pool(pool_mapping['edge_id'],
                                  ...
                                  edge_pool)
            self._get_lb_driver().pool_successful(context, pool)

        except nsxv_exc.VcnsApiException:
            self._get_lb_driver().pool_failed(context, pool)
            with excutils.save_and_reraise_exception():
                LOG.exception(_LE('Failed to update pool %s') % pool['id'])
    ...
    def delete_pool(self, context, pool, pool_mapping):
        ...
        if pool_mapping:
            try:
                ...
                self._get_lb_driver().pool_failed(context, pool)
                with excutils.save_and_reraise_exception():
                    LOG.exception(_LE('Failed to delete pool %s') %
                                  pool['id'])
        else:
            LOG.error(_LE('No mapping found for pool %s') % pool['id'])

        self._get_lb_driver().delete_pool_successful(context, pool)
    ...
    def create_vip(self, context, vip, pool_mapping):
        ...
        try:
            ...
            self._get_lb_driver().vip_failed(context, vip)
            with excutils.save_and_reraise_exception():
        ...
        try:
            ...
            edge_fw_rule_id = self._add_vip_fw_rule(edge_id, vip['id'],
                                                    vip['address'])
            self._get_lb_driver().create_vip_successful(
                context, vip, context, vip, edge_id, app_profile_id,
    ...
    def update_vip(self, context, old_vip, vip, vip_mapping):
        ...
        edge_vip = convert_lbaas_vip(context, vip, app_profile_id)
        try:
            self.vcns.update_vip(edge_id, edge_vip_id, edge_vip)
            self._get_lb_driver().vip_successful(context, vip)
        except nsxv_exc.VcnsApiException:
            self._get_lb_driver().vip_failed(context, vip)
            with excutils.save_and_reraise_exception():
    ...
    def delete_vip(self, context, vip, vip_mapping):
        ...
        if vip_mapping:
            ...
            try:
                ...
                with excutils.save_and_reraise_exception():
                    LOG.exception(
                        _LE('Failed to delete vip on edge: %s'),
                        edge_id)
            ...
            try:
                ...
                LOG.exception(
                    _LE('app profile not found on edge: %s'),
                    edge_id)
            except nsxv_exc.VcnsApiException:
                self._get_lb_driver().vip_failed(context, vip)
                with excutils.save_and_reraise_exception():
            ...
            self._get_lb_driver().delete_vip_successful(context, vip)
        else:
    ...
    def create_member(self, context, member, pool_mapping):
        ...
        try:
            ...
            self._update_pool_fw_rule(context, member['pool_id'],
                                      ...
                                      'add',
                                      member['address'])
            self._get_lb_driver().member_successful(context, member)

        except nsxv_exc.VcnsApiException:
            self._get_lb_driver().member_failed(context, member)
            with excutils.save_and_reraise_exception():
                LOG.exception(
                    _LE('Failed to create member on edge: %s'),
                    pool_mapping['edge_id'])
    ...
    def update_member(self, context, old_member, member, pool_mapping):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception(
                    _LE('Failed to update member on edge: %s'),
                    pool_mapping['edge_id'])
    ...
    def create_pool_health_monitor(self, context, health_monitor, pool_id,
        ...
        if mon_mappings:
            ...
            try:
                ...
                self._get_lb_driver().pool_health_monitor_failed(context,
                                                          health_monitor,
                                                          pool_id)
                with excutils.save_and_reraise_exception():
                    LOG.exception(
        ...
        self._get_lb_driver().create_pool_health_monitor_successful(
    ...
    def update_pool_health_monitor(self, context, old_health_monitor,
        ...
        self._get_lb_driver().pool_health_monitor_successful(context,
                                                      health_monitor,
                                                      pool_id)
    ...
    def delete_pool_health_monitor(self, context, health_monitor, pool_id,
        ...
        if mon_mapping:
            ...
            self._get_lb_driver().delete_pool_health_monitor_successful(
