...
class NsxV3Driver(l2gateway_db.L2GatewayMixin):
    ...
    def _validate_network(self, context, gw_connection):
        ...
        # If network type is None, the default network type for NSXv3 is VXLAN
        if network_type is None:
            return
        # If network is a provider network, verify whether it is of type VXLAN
        if network_type != nsx_utils.NsxV3NetworkTypes.VXLAN:
    ...
    def create_l2_gateway_connection(self, context, l2_gateway_connection):
        ...
        except (nsx_exc.ManagerError, db_exc.DBError):
            with excutils.save_and_reraise_exception():
