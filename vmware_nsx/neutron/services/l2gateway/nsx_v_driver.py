...
class NsxvL2GatewayDriver(l2gateway_db.L2GatewayMixin):
    ...
    def _validate_interface_list(self, interfaces):
        ...
        if len(interfaces) != 1:
            ...
            raise n_exc.InvalidInput(error_message=msg)
        if not self._vcns.validate_network(interfaces[0]['name']):
    ...
    def create_l2_gateway(self, context, l2_gateway):
        ...
        try:
            ...
            LOG.exception(_LE("Failed to create the L2 gateway"))
            return
    ...
    def create_l2_gateway_connection(self, context, l2_gateway_connection):
        ...
        virtualWire = nsx_db.get_nsx_switch_ids(context.session, network_id)
        l2gw_id = gw_connection.get('l2_gateway_id')
        ...
        try:
            self._vcns.create_bridge(device_name, bridge_dict)
        except Exception:
    ...
    def delete_l2_gateway_connection(self, context, l2_gateway_connection):
        ...
        device = self._get_device(context, l2gw_id)
        device_name = device.get('device_name')
        self._vcns.delete_bridge(device_name)
    ...
    def delete_l2_gateway(self, context, l2_gateway):
        ...
        rtr_binding = nsxv_db.get_nsxv_router_binding_by_edge(context.session,
                                                              edge_id)
        if rtr_binding:
            edge_manager = self._edge_manager()
