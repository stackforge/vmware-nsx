...
def add_nsx_extensions_to_parser(parser):
    if utils.is_nsx_v():
        # DHCP MTU
        parser.add_argument(
            '--dhcp-mtu',
        ...
        parser.add_argument(
            '--advanced-service-providers',
            metavar='<advanced-service-providers>',
            help=_("Advanced service providers")
        )
...
def _get_plugin_attrs(client_manager, parsed_args, is_create=True):
    ...
    if utils.is_nsx_v():
        ...
        # Advanced service providers
        if parsed_args.advanced_service_providers is not None:
            attrs['advanced_service_providers'] = (
                parsed_args.advanced_service_providers)
            parsed_args.advanced_service_providers = None
...
class NsxCreateSubnet(subnet.CreateSubnet):
    ...
    def get_parser(self, prog_name):
        ...
        add_nsx_extensions_to_parser(parser)
        return parser
