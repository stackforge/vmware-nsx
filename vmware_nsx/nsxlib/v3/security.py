...
class NsxLibNsGroup(utils.NsxLibApiBase):
    ...
    def update_on_backend(self, context, security_group,
        ...
        logging = (log_sg_allowed_traffic or
                   security_group[consts.SecurityConstants.LOGGING])
    ...
    def update_lport(self, context, lport_id, original, updated):
        ...
        for nsgroup_id in added:
            try:
                self.add_members(
                    nsgroup_id, consts.SecurityConstants.LOGICAL_PORT,
                    [lport_id])
        ...
        for nsgroup_id in removed:
            self.remove_member(
                nsgroup_id, consts.SecurityConstants.LOGICAL_PORT, lport_id)
    ...
    def init_default_section(self, name, description, nested_groups,
        ...
        block_rule = self.get_firewall_rule_dict(
            'Block All', action=consts.SecurityConstants.DROP,
            logged=log_sg_blocked_traffic)
        # TODO(roeyc): Add additional rules to allow IPV6 NDP.
        dhcp_client = self.get_nsservice(
            consts.SecurityConstants.L4_PORT_SET_NSSERVICE,
            l4_protocol=consts.SecurityConstants.UDP,
            source_ports=[67],
            destination_ports=[68])
        dhcp_client_rule_in = self.get_firewall_rule_dict(
            'DHCP Reply',
            direction=consts.SecurityConstants.IN,
            service=dhcp_client)

        dhcp_server = (
            self.get_nsservice(consts.SecurityConstants.L4_PORT_SET_NSSERVICE,
                               l4_protocol=consts.SecurityConstants.UDP,
                               source_ports=[68],
                               destination_ports=[67]))
        dhcp_client_rule_out = self.get_firewall_rule_dict(
            'DHCP Request',
            direction=consts.SecurityConstants.OUT,
            service=dhcp_server)
    ...
    def get_port_tag_expression(self, scope, tag):
        return {'resource_type': consts.SecurityConstants.NSGROUP_TAG_EXP,
                'target_type': consts.SecurityConstants.LOGICAL_PORT,
                'scope': scope,
                'tag': tag}
    ...
    def get_member_expression(self, target_type, target_id):
        return {
            'resource_type': consts.SecurityConstants.NSGROUP_SIMPLE_EXP,
            'target_property': 'id',
            'target_type': target_type,
            'op': consts.SecurityConstants.EQUALS,
            'value': target_id}
    ...
    def add_members(self, nsgroup_id, target_type, target_ids):
        ...
        try:
            return self._update_with_members(
                nsgroup_id, members, consts.SecurityConstants.ADD_MEMBERS)
    ...
    def remove_member(self, nsgroup_id, target_type,
        ...
        try:
            return self._update_with_members(
                nsgroup_id, members, consts.SecurityConstants.REMOVE_MEMBERS)
        except exceptions.ManagerError:
            if verify:
...
class NsxLibFirewallSection(utils.NsxLibApiBase):
    ...
    def _get_direction(self, sg_rule):
        return (
            consts.SecurityConstants.IN if sg_rule['direction'] == 'ingress'
            else consts.SecurityConstants.OUT
        )
    ...
    def _get_l4_protocol_name(self, protocol_number):
        ...
        if protocol_number == 6:
            return consts.SecurityConstants.TCP
        elif protocol_number == 17:
            return consts.SecurityConstants.UDP
        elif protocol_number == 1:
            return consts.SecurityConstants.ICMPV4
        else:
    ...
    def _decide_service(self, sg_rule):
        ...
        if l4_protocol in [consts.SecurityConstants.TCP,
                           consts.SecurityConstants.UDP]:
            # If port_range_min is not specified then we assume all ports are
            # matched, relying on the plugin to perform validation.
            source_ports = []
            ...
            if direction == consts.SecurityConstants.OUT:
        ...
        elif l4_protocol == consts.SecurityConstants.ICMPV4:
            return self.get_nsservice(
                consts.SecurityConstants.ICMP_TYPE_NSSERVICE,
                protocol=l4_protocol,
                icmp_type=sg_rule['port_range_min'],
                icmp_code=sg_rule['port_range_max'])
        elif l4_protocol is not None:
            return self.get_nsservice(
                consts.SecurityConstants.IP_PROTOCOL_NSSERVICE,
    ...
    def _build(self, display_name, description, applied_tos, tags):
        return {'display_name': display_name,
                'description': description,
                'stateful': True,
                'section_type': consts.SecurityConstants.LAYER3,
    ...
    def create_empty(self, display_name, description,
                     applied_tos, tags,
                     operation=consts.SecurityConstants.INSERT_BOTTOM,
                     other_section=None):
    ...
    def get_nsgroup_reference(self, nsgroup_id):
        return {'target_id': nsgroup_id,
                'target_type': consts.SecurityConstants.NSGROUP}

    def get_ip_cidr_reference(self, ip_cidr_block, ip_protocol):
        target_type = (consts.SecurityConstants.IPV4ADDRESS
                       if ip_protocol == consts.SecurityConstants.IPV4
                       else consts.SecurityConstants.IPV6ADDRESS)
    ...
    def get_rule_dict(
        ...
        destination=None,
        direction=consts.SecurityConstants.IN_OUT,
        ip_protocol=consts.SecurityConstants.IPV4_IPV6,
        service=None, action=consts.SecurityConstants.ALLOW,
        logged=False):
        return {'display_name': display_name,
    ...
    def _get_fw_rule_from_sg_rule(self, sg_rule, nsgroup_id, rmt_nsgroup_id,
        ...
        if sg_rule.get(consts.SecurityConstants.LOCAL_IP_PREFIX):
            local_ip_prefix = self.get_ip_cidr_reference(
                sg_rule[consts.SecurityConstants.LOCAL_IP_PREFIX],
                ip_protocol)
        else:
    ...
    def init_default(self, name, description, nested_groups,
        ...
        dhcp_client_rule_in = self.get_rule_dict(
            'DHCP Reply', direction=consts.SecurityConstants.IN,
            service=dhcp_client)
        ...
        dhcp_client_rule_out = self.get_rule_dict(
            'DHCP Request', direction=consts.SecurityConstants.OUT,
            service=dhcp_server)
