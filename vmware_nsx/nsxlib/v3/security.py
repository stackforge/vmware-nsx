    ...
    def add_nsgroup(self, nsgroup_id):
        for container in self._get_available_container(nsgroup_id):
            try:
                ...
                LOG.warning(_LW("Container group %s is full, trying the next "
                                "container.."), container, nsgroup_id)
        else:
    ...
    def remove_nsgroup(self, nsgroup_id):
        for container in self._get_available_container(nsgroup_id):
            try:
                ...
                LOG.warning(_LW("NSGroup %s was expected to be found in "
                                "container %s, but wasn't. Looking in the "
                                "next container.."), nsgroup_id, container)
                continue
        else:
