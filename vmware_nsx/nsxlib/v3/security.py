...
class NSGroupContainerManager(object):
    ...
    def _init_container_groups(self):
        # Construct the containers dict -
        # {0: <container-1>,.., n-1: <container-n>}
        containers = {
            self._get_container_index_from_name(nsgroup): nsgroup['id']
        ...
        if absent_containers:
            LOG.warning(
                ...
                {'num_present': len(containers),
                 'num_absent': len(absent_containers)})
    ...
    def _get_container_index_from_name(self, container):
        return int(container['display_name'][-1]) - 1
    ...
    def _hash_uuid(self, uuid_):
        return hash(uuid.UUID(uuid_))

    def _suggest_container(self, uuid):
        # Suggests a container to use, can be iterated to find alternative
        # containers in case that previous suggestions did not help.

        index = self._hash_uuid(uuid) % self.size
    ...
    def add_nsgroup(self, nsgroup_id):
        for container in self._suggest_container(nsgroup_id):
            try:
