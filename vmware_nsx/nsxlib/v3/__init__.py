...
class NsxLib(security.Security):
    ...
    def update_resource_with_retry(self, resource, payload):
        ...
        def update_resource_int(client, resource, revised_payload):
            return client.update(resource, revised_payload)

        revised_payload = self.client.get(resource)
        for key_name in payload.keys():
            revised_payload[key_name] = payload[key_name]
        return update_resource_int(self.client, resource, revised_payload)
    ...
    def delete_logical_switch(self, lswitch_id):
        ...
        @utils.retry_upon_exception(exceptions.StaleRevision,
                                    max_attempts=self.max_attempts)
        def delete_logical_switch_int(client, lswitch_id):
            resource = ('logical-switches/%s?detach=true&cascade=true' %
                        lswitch_id)
            client.delete(resource)

        return delete_logical_switch_int(self.client, lswitch_id)
    ...
    def update_logical_switch(self, lswitch_id, name=None, admin_state=None,
        ...
        @utils.retry_upon_exception(exceptions.StaleRevision,
                                    max_attempts=self.max_attempts)
        def update_logical_switch_int(client, resource, lswitch):
            return client.update(resource, lswitch)
        ...
        if tags is not None:
            lswitch['tags'] = tags
        return update_logical_switch_int(self.client, resource, lswitch)
