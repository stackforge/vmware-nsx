...
class NSX3Client(JSONRESTClient):
    ...
    def __init__(self, connection, url_prefix=None,
        ...
        if client_obj:
            self._nsx_api_managers = client_obj._nsx_api_managers or []
            self._max_attempts = client_obj._max_attempts
        else:
            self._nsx_api_managers = nsx_api_managers or []
    ...
    def _raise_error(self, status_code, operation, result_msg):
        ...
        error = ERRORS.get(status_code, DEFAULT_ERROR)
        raise error(manager=self._nsx_api_managers,
                    operation=operation,
                    details=result_msg)
