...
class NSXRequestsHTTPProvider(AbstractHTTPProvider):
    ...
    def new_connection(self, cluster_api, provider):
        ...
        if not cluster_api.insecure and cluster_api.ca_file:
...
class ClusteredAPI(object):
    ...
    def __init__(self, providers,
        ...
        conns.waitall()
        LOG.debug("Done initializing API endpoints. "
                  "API cluster heath: %s" % self.health)

    def _update_keepalive(self, old_state, new_state, endpoint):
        # TODO: backoff on revalidate
        if 'keepalive' in endpoint.metadata:
            endpoint.metadata.get('keepalive').cancel()
    ...
    def health(self):
        ...
        up = 0
        for endpoint in self._endpoints.values():
            if endpoint.state == ClusteredAPI.EndpointState.DOWN:
                down += 1
            elif endpoint.state == ClusteredAPI.EndpointState.UP:
                up += 1
    ...
    @property
    def cluster_id(self):
        return [str(ep) for ep in self._endpoints.values()]
    ...
    def _proxy(self, proxy_for, uri, *args, **kwargs):
        ...
        with self.endpoint_connection() as conn_data:
            ...
            try:
                ...
                return self._proxy(proxy_for, *args, **kwargs)


class NSXClusteredAPI(ClusteredAPI):

    def __init__(self, http_provider=None):
        # TODO: consolidate opts / groups
        self.username = cfg.CONF.nsx_v3.nsx_user
        self.password = cfg.CONF.nsx_v3.nsx_password
        self.retries = cfg.CONF.retries
        self.insecure = cfg.CONF.nsx_v3.insecure
        self.ca_file = cfg.CONF.nsx_v3.ca_file
        # TODO: conf prop
        #self.conns_per_pool = cfg.CONF.concurrent_connections
        self.conns_per_pool = 500
        self.http_timeout = cfg.CONF.http_timeout
        self.conn_idle_timeout = cfg.CONF.conn_idle_timeout
        self.redirects = cfg.CONF.redirects
        ...
        super(NSXClusteredAPI, self).__init__(
            ...
            keepalive_interval=cfg.CONF.conn_idle_timeout)
    ...
    def _build_conf_providers(self):
        ...
        # TODO: converge on host conf prop
        conf_urls = cfg.CONF.nsx_controllers[:]
        urls = []
        providers = []
        if cfg.CONF.nsx_v3.nsx_manager:
            conf_urls.append(cfg.CONF.nsx_v3.nsx_manager)
...
__DEFAULT_API_CLUSTER__ = None


def _get_fn_bridge_cluster():
    global __DEFAULT_API_CLUSTER__
    if __DEFAULT_API_CLUSTER__ is None:
        __DEFAULT_API_CLUSTER__ = NSXClusteredAPI()
    return __DEFAULT_API_CLUSTER__


def _set_fn_bridge_cluster(cluster):
    global __DEFAULT_API_CLUSTER__
    old = __DEFAULT_API_CLUSTER__
    __DEFAULT_API_CLUSTER__ = cluster
    return old


def _get_client(client):
    return client or NSX3Client(_get_fn_bridge_cluster())
