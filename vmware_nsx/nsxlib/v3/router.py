...
class RouterLib(object):
    ...
    def __init__(self, _nsx_client):
        self.router_client = nsx_resources.LogicalRouter(self, _nsx_client)
        self.router_port_client = nsx_resources.LogicalRouterPort(
            self, _nsx_client)
    ...
    def validate_tier0(self, tier0_groups_dict, tier0_uuid):
        ...
        err_msg = None
        try:
            lrouter = self.router_client.get(tier0_uuid)
    ...
    def add_router_link_port(self, tier1_uuid, tier0_uuid, edge_members):
        ...
        tier0_link_port = self.router_port_client.create(
        ...
        self.router_port_client.create(
    ...
    def remove_router_link_port(self, tier1_uuid, tier0_uuid):
        try:
            tier1_link_port = (
                self.router_port_client.get_tier1_logical_router_link_port(
                    tier1_uuid))
        except nsx_exc.ResourceNotFound:
        ...
        self.router_port_client.delete(tier1_link_port_id)
        self.router_port_client.delete(tier0_link_port_id)
    ...
    def update_router_edge_cluster(self, nsx_router_id, edge_cluster_uuid):
        return self.router_client.update(nsx_router_id,
                                         edge_cluster_id=edge_cluster_uuid)
    
    
    def create_logical_router_intf_port_by_ls_id(self, logical_router_id,
                                                 ls_id,
>>>>>>> 3ef7f72... (WIP)nsx v3 router refactor
                                                 logical_switch_port_id,
                                                 address_groups):
        try:
            port = self.router_port_client.get_logical_router_port_by_ls_id(
                ls_id)
        except nsx_exc.ResourceNotFound:
            return self.router_port_client.create(logical_router_id,
                                                  ROUTER_INTF_PORT_NAME,
                                                  nsxlib.LROUTERPORT_DOWNLINK,
                                                  logical_switch_port_id,
                                                  address_groups)
        else:
            return self.router_port_client.update(
