...
class LogicalDhcpServer(AbstractRESTResource):
    ...
    def create(self, dhcp_profile_id, server_ip, prefix_length,
               dns_servers=None, domain_name=None, gateway_ip=None,
               lease_time=None, options=None, tags=None):
    ...
    def update(self, uuid, dhcp_profile_id=None, server_ip=None,
               prefix_length=None, dns_servers=None, domain_name=None,
               gateway_ip=None, lease_time=None, options=None, tags=None):
        ...
        if prefix_length:
            body['ipv4_dhcp_server']['prefix_length'] = prefix_length
        if dns_servers:
        ...
        if lease_time:
            body['ipv4_dhcp_server']['lease_time'] = lease_time
        if options:
    ...
    def create_binding(self, server_uuid, mac, ip, hostname, options=None):
        body = {'mac_address': mac, 'ip_address': ip, 'host_name': hostname}
        if options:
