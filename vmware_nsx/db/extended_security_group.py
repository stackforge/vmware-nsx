...
class ExtendedSecurityGroupPropertiesMixin(object):
    ...
    def _check_invalid_security_groups_specified(self, context, port):
        if validators.is_attr_set(port.get(ext_sg.SECURITYGROUPS)):
            for sg in port.get(ext_sg.SECURITYGROUPS, []):
                # We do not accept provider security-groups as part of
                # the port's non-provider security-groups list.
        ...
        if validators.is_attr_set(
            ...
            # Also check all specified provider groups are indeed provider.
            for sg_id in port.get(provider_sg.PROVIDER_SECURITYGROUPS, []):
                self._check_provider_security_group_exists(context, sg_id)
    ...
    def _get_provider_security_groups_on_port(self, context, port):
        ...
        requested_provider_sgs = p.get(provider_sg.PROVIDER_SECURITYGROUPS)
