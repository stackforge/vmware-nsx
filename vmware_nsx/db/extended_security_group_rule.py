...
class ExtendedSecurityGroupRuleMixin(object):
    ...
    def _process_security_group_rule_properties(self, context,
                                                rule_res, rule_req):
        rule_res[ext_loip.LOCAL_IP_PREFIX] = None
        if not attr.is_attr_set(rule_req.get(ext_loip.LOCAL_IP_PREFIX)):
            return

        with context.session.begin(subtransactions=True):
            properties = nsxv_models.NsxvExtendedSecurityGroupRuleProperties(
                rule_id=rule_res['id'],
                local_ip_prefix=rule_req[ext_loip.LOCAL_IP_PREFIX])
            context.session.add(properties)
        rule_res[ext_loip.LOCAL_IP_PREFIX] = rule_req[ext_loip.LOCAL_IP_PREFIX]
