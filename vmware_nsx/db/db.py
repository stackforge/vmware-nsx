...
def add_project_plugin_mapping(session, id, project_id, plugin):
    with session.begin(subtransactions=True):
        binding = nsx_models.NsxProjectPluginMapping(
            id=id, project_id=project_id, plugin=plugin)
        session.add(binding)
    return binding


def get_project_plugin_mapping(session, id):
    try:
...
def get_project_plugin_mapping_by_proj(session, project_id):
    try:
        return session.query(nsx_models.NsxProjectPluginMapping).filter_by(
