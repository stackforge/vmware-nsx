...
def update_neutron_nsx_dhcp_binding(session, port_id, binding_id, mac, ip):
    with session.begin(subtransactions=True):
        (session.query(nsx_models.NeutronNsxDhcpBinding).
         filter_by(port_id=port_id, nsx_binding_id=binding_id).update(
             {nsx_models.NeutronNsxDhcpBinding.mac: mac,
              nsx_models.NeutronNsxDhcpBinding.ip: ip}))
