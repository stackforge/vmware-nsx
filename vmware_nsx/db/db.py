...
def add_neutron_vrf_binding(session, router_id, vrf_id):
    with session.begin(subtransactions=True):
        binding = nsx_models.NeutronRouterVrfBinding(
...
def get_neutron_vrf_id(session, router_id):
    try:
        binding = session.query(nsx_models.NeutronRouterVrfBinding).filter_by(
...
def delete_neutron_vrf_binding(session, router_id):
    return session.query(nsx_models.NeutronRouterVrfBinding).filter_by(
