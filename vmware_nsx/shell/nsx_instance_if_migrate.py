...
def get_opts():
    ...
    for mandatory_key in ['username', 'password', 'project', 'auth-url']:
        if opts.get(mandatory_key) is None:
            LOG.debug("%s must be specified!", mandatory_key)
            usage()
...
def iface_migrate(neutron, instance_name, iface, nsx_switch):
    ...
    if instance_mac is None:
        LOG.debug("Couldn't find MAC address for instance %s", instance_name)
        return

    ports = neutron.list_ports(fields=['id'], mac_address=instance_mac)
    if len(ports['ports']) != 1:
        LOG.debug('For instance %(vm)s, invalid ports received from neutron: '
    ...
    if tap_dev is None:
        LOG.debug("For instance %(vm)s, couldn't find tap device for "
...
def instance_migrate(libvirt_conn, neutron, instance, machine_type,
    ...
    if instance_name is None:
        LOG.debug("Couldn't find instance name in XML")
        return

    instance_uuid = xmltag_text_get(root, 'uuid')
    if instance_uuid is None:
        LOG.debug("Couldn't find UUID for instance %s", instance_name)
        return

    # Validate that os is supported by hypervisor
    os_tag = root.find('os')
    if os_tag is None:
        LOG.debug("Couldn't find OS tag for instance %s", instance_name)
        return
    type_tag = os_tag.find('type')
    if not is_valid_os_data(libvirt_conn, type_tag.text, type_tag.get('arch'),
                            type_tag.get('machine')):
        LOG.debug("Instance %s OS data is invalid or not supported by "
    ...
    LOG.debug('Migrated instance %(vm)s (%(uuid)s) successfully!',
...
def main():
    ...
    if auth is None:
        LOG.debug('Failed to authenticate with keystone')
        exit(1)

    sess = session.Session(auth=auth)
    if sess is None:
        LOG.debug('Failed to create keystone session')
        exit(1)

    neutron = client.Client(session=sess)
    if neutron is None:
        LOG.debug('Failed to create neutron session')
        exit(1)
    ...
    for instance in instances:
        try:
            ...
            LOG.debug('Failed to migrate instance with exception %s', e)
