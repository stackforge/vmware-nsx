...
def get_opts():
    ...
    for mandatory_key in ['username', 'password', 'project', 'auth-url']:
        if opts.get(mandatory_key) is None:
            print mandatory_key, "must be specified!"
            usage()

    return opts


def instance_uuid_get(root):
    uuid_tag = root.find('uuid')
    if uuid_tag is not None:
        return uuid_tag.text
...
def iface_migrate(neutron, instance_uuid, iface, nsx_switch):
    ...
    if instance_mac is None:
        LOG.error("Couldn't find MAC address for instance %s",
                  instance_uuid)
        return

    ports = neutron.list_ports(fields=['id'], mac_address=instance_mac)
    if len(ports['ports']) != 1:
        LOG.error('Invalid ports received from neutron: %s', ports)
        return
    ...
    if tap_dev is None:
        LOG.error("Couldn't find tap device for interface")
...
def is_valid_os_data(libvirt_conn, os_type, os_arch, os_machine):
    ...
    for guest_tag in caps_root.findall('guest'):
        if(guest_tag.find('os_type').text == os_type
...
def instance_migrate(libvirt_conn, neutron, instance, machine_type,
    ...
    instance_uuid = instance_uuid_get(root)
    if instance_uuid is None:
        LOG.error("Couldn't find UUID for instance %s",
                  root.find('name').text)
        return
    ...
    os_tag = root.find('os')
    if os_tag is None:
        LOG.error("Couldn't find OS tag for instance %s",
                  root.find('name').text)
        return
    ...
    if not is_valid_os_data(libvirt_conn, type_tag.text, type_tag.get('arch'),
        ...
        LOG.error("Instance %s OS data is invalid or not supported by "
                  "hypervisor", root.find('name').text)
        return
    ...
    for iface in ifaces:
        iface_migrate(neutron, instance_uuid, iface, nsx_switch)

    instance.undefine()
    libvirt_conn.defineXML(et.tostring(root))
    LOG.info('Migrated instance %s successfully!', instance_uuid)
