...
def get_networks_dict():
    ...
    for obj in root.iter('object'):
        if obj.find('objectTypeName').text in network_types:
            moref = obj.find('objectId').text
            if moref not in networks:
                networks[moref] = []
            networks[moref].append(obj.find('name').text)
    return networks
...
def list_missing_networks(resource, event, trigger, **kwargs):
    ...
    backend_networks = get_networks_dict()
    missing_networks = []
    ...
    for entry in mappings:
        ...
        if nsx_id not in backend_networks.keys():
            ...
            found = False
            for netname in backend_networks[nsx_id]:
                if netname.startswith(dvs_id):
                    found = True
            if not found:
                missing_networks.append({'neutron_id': entry['neutron_id'],
                                         'moref': nsx_id,
