...
class NeutronSecurityGroupDB(utils.NeutronDbClient,
    ...
    def _get_logical_ports_in_security_group(self, security_group_id):
        ...
        return [p['port_id'] for p in query]
...
def fix_security_groups(resource, event, trigger, **kwargs):
    ...
    for sg_id, sg in sgs_with_missing_nsx_group.items():
        ...
        plugin._process_security_group_create_backend_resources(context_,
                                                                secgroup)
        for lp in neutron_sg._get_logical_ports_in_security_group(sg_id):
            vnic_index = plugin._get_port_vnic_index(context_, lp)
            plugin._add_member_to_security_group(sg['nsx-securitygroup-id'],
                                                 vnic_index)
