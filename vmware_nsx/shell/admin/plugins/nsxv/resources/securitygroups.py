...
class NeutronSecurityGroupDB(utils.NeutronDbClient,
    ...
    def delete_security_group_section_mapping(self, sg_id):
        fw_mapping = self.context.session.query(
            ...
            nsxv_models.NsxvSecurityGroupSectionMapping.neutron_id == sg_id
        ).one_or_none()
        if fw_mapping:
    ...
    def _get_vnics_in_security_group(self, security_group_id):
        query = self.context.session.query(
            securitygroups_db.SecurityGroupPortBinding).filter(
                securitygroups_db.SecurityGroupPortBinding.security_group_id
                == security_group_id).all()
        return [(p['port_id'], p['device_id']) for p in query]
...
def fix_security_groups(resource, event, trigger, **kwargs):
    ...
    for sg_id, sg in sgs_with_missing_nsx_group.items():
        ...
        for p_id, device_id in neutron_sg._get_ports_in_security_group(sg_id):
            vnic_id = plugin._get_port_vnic_id(
                plugin._get_port_vnic_index(context_, p_id), device_id)
