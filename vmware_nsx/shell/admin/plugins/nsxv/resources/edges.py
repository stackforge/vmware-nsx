...
def change_edge_appliance_appliances(properties):
    """Update the appliances data of an edge, according to the current
    ...
    try:
        nsxv.change_edge_appliance_appliances(edge_id, request)
    except nsxv_exceptions.ResourceNotFound as e:
...
def nsx_update_edge(resource, event, trigger, **kwargs):
    ...
    if properties.get('highavailability'):
        ...
        change_edge_appliance_appliances(properties)
    else:
