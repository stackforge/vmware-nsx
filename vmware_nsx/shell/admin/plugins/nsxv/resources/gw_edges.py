...
def _validate_asn(asn):
    if not netutils.is_valid_port(asn):
        msg = "Invalid AS number, expecting an integer value (0 - 65535)."
        LOG.error(msg)
        return False
    return True
...
def create_redis_rule(resource, event, trigger, **kwargs):
    ...
    required_params = ('gw-edge-ids', 'learner-protocol', 'learn-from', 'action')
    properties = admin_utils.parse_multi_keyval_opt(kwargs.get('property', []))
    if not properties or set(required_params) <= set(properties.keys()):
        LOG.error(usage)
        return
...
def add_bgp_neighbour(resource, event, trigger, **kwargs):
    ...
    headers = ['edge_id', 'ip_address', 'remote_as',
               'hold_down_time', 'keep_alive_timer']
