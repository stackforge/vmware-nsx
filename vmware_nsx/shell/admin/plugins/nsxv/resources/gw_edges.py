...
def _extract_interface_info(info):
    ...
    return portgroup, ip_address, prefixlen
...
def _assemble_gw_edge(name, external_iface_info, internal_iface_info):
    ...
    portgroup, ip_address, prefixlen = (
        _extract_interface_info(external_iface_info))
    vnic_external = nsxv._assemble_edge_vnic(constants.EXTERNAL_VNIC_NAME,
                                             constants.EXTERNAL_VNIC_INDEX,
                                             ip_address, prefixlen,
                                             type="uplink")

    portgroup, gateway_ip, prefixlen = (
        _extract_interface_info(internal_iface_info))
    vnic_internal = nsxv._assemble_edge_vnic(constants.INTERNAL_VNIC_NAME,
                                             constants.INTERNAL_VNIC_INDEX,
...
def create_bgp_gw(resource, event, trigger, **kwargs):
    ...
    speaker = bgpdb.get_bgp_speaker(context, bgp_speaker_id)
...
def create_redis_rule(resource, event, trigger, **kwargs):
    ...
    nsxv.add_bgp_redistribution_rules(properties['edge-id'], prefixes, [rule])

    res = {'edge_id': properties['edge-id'],
           'prefix': prefix_name if prefix_name else 'ANY',
           'learner-protocol': properties['learner_protocol'],
           'learn-from': ', '.join(learn_from),
...
def delete_redis_rule(resource, event, trigger, **kwargs):
    ...
    nsxv.remove_bgp_redistribution_rules(properties['edge-id'], prefixes)
