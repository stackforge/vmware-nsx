...
def get_certificate_manager(**kwargs):
    ...
    if storage_driver_type == 'nsx-db':
        storage_driver = v3_utils.DbCertificateStorageDriver(
            context.get_admin_context())
    elif storage_driver_type == 'none':
        storage_driver = v3_utils.DummyCertificateStorageDriver()
    ...
    return client_cert.ClientCertificateManager(
            v3_utils.NSX_OPENSTACK_IDENTITY,
            nsx_trust,
            storage_driver)
...
def generate_cert(resource, event, trigger, **kwargs):
    ...
    cert_manager.generate(subject={}, valid_for_days=3650)
