...
def test_service(nsx_policy):
    ...
    # DEBUG ADIT - update of a service entry in currently failing
...
def test_communication_maps(nsx_policy, domain_id, group_ids, profile_id):
    ...
    # create another entry, and make sure it is added after
    ...
    try:
        ...
        LOG.error("ERROR comm_map.create crashed for %(name)s : %(e)s",
                  {'name': map_name2, 'e': e})
    new_seq = comm_map2['sequence_number']
    if new_seq != seq_num + 1:
...
def policy_obj_cleanup(nsx_policy, ep_id, domain_id, group_ids, service_id,
    ...
    if map_id:
        nsx_policy.deployment_map.delete(map_id)
    ...
    # DEBUG ADIT deleting the communication profile is failing
