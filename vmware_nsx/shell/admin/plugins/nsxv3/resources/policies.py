...
def test_domains(nsx_policy):
    ...
    try:
        ...
        LOG.error("ERROR domain.list crashed: %s", e)
...
def test_groups(nsx_policy, domain_id):
    ...
    try:
        ...
        LOG.error("ERROR group.list crashed: %s", e)
    ...
    try:
        ...
        LOG.error("ERROR group.get_by_name crashed for %s: %s", group_name, e)
...
def test_service(nsx_policy):
    # Create a service
    # DEBUG ADIT create with type doesn't work now
    ...
    try:
        ...
        LOG.error("ERROR service.create crashed for %s.\n"
                  "trying to create without an entry", service_name)
        # try to create without entry
        service = nsx_policy.service.create_without_entry(
            service_name,
            description='My service')
...
def test_contract(nsx_policy, service_id):
    ...
    try:
        ...
        LOG.error("ERROR contract.get_by_name crashed: %s", e)

    # update contract description:
    updated_contract = nsx_policy.contract.update(contract_id,
                                                  description='updated')
...
def policy_obj_cleanup(nsx_policy, domain_id, group_id, service_id,
                       contract_id):
    # delete the contract
...
def test_policy_api(resource, event, trigger, **kwargs):
    ...
    group_id = test_groups(nsx_policy, domain_id)
    ...
    # CLEANUP:
    LOG.info("Cleanup")
    policy_obj_cleanup(nsx_policy, domain_id, group_id, service_id,
                       contract_id)
