...
def test_service(nsx_policy):
    ...
    updated_service = nsx_policy.service.update(service_id,
                                                protocol='udp',
                                                dest_ports=[555])
    if updated_service['service_entries'][0]['l4_protocol'] != 'UDP':
...
def test_contract(nsx_policy, service_id):
    ...
    updated_contract = nsx_policy.contract.update(
        ...
        services=[service_id],
        action='allow')
    if updated_contract['contract_entries'][0]['action'] != 'ALLOW':
        LOG.error("ERROR contract.update entry didn't work for %s", contract_name)
