...
def nsx_update_dhcp_bindings(resource, event, trigger, **kwargs):
    ...
    for port in ports:
        ...
        if device_owner == const.DEVICE_OWNER_DHCP:
            ...
            server_data = nsx_utils.build_dhcp_server_config(
                network, subnet, port, 'NSX Neutron plugin upgrade')
            dhcp_server = nsx_client.create_logical_dhcp_server(**server_data)
            attachment = {'attachment_type': nsx_constants.ATTACHMENT_DHCP,
                          'id': dhcp_server['id']}
            lswitch_id, lport_id = neutron_client.get_lswitch_and_lport_id(
                port['id'])
            nsx_client.update_logical_switch_port(
                lport_id, {'attachment': attachment})
        ...
        for (mac, ip) in bindings:
            ...
            options = {'option121': {'static_routes': [
                {'network': '%s/32' % dhcp.METADATA_DEFAULT_IP,
                 'next_hop': ip}]}}
            nsx_client.create_static_binding(
