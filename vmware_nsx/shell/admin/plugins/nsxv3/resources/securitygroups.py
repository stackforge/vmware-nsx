...
class NeutronSecurityGroupApi(sg_db.SecurityGroupDbMixin,
    ...
    def get_security_groups(self):
        return self.get_security_groups(self.context)
...
neutron_db = admin_utils.NeutronDbClient()
...
def _update_ports_dynamic_criteria_tags():
    ...
    for port in neutron_db.get_ports():
        ...
        _, lport_id = neutron_db.get_nsx_switch_and_port_id(port['id'])
        criteria_tags = security.get_lport_tags_for_security_groups(secgroups)
        port_client.update(lport_id, port['id'], tags_update=criteria_tags)
...
@admin_utils.output_header
def nsx_update_security_groups(self):
...
registry.subscribe(nsx_update_security_groups,
                   constants.FIREWALL_NSX_GROUPS,
                   constants.Operations.UPDATE.value)
