...
#TODO(asarfaty): reuse the nsx-v3 certificate code
...
def get_nsx_trust_management(**kwargs):
    ...
    nsx_client = utils.get_nsxv3_client(username, password, True, plugin_conf=cfg.CONF.nsx_p)
    nsx_trust = trust_management.NsxLibTrustManagement(nsx_client, {})
    return nsx_trust


def get_certificate_manager(**kwargs):
    storage_driver_type = cfg.CONF.nsx_p.nsx_client_cert_storage.lower()
...
def verify_client_cert_on():
    if cfg.CONF.nsx_p.nsx_use_client_auth:
        return True
...
def generate_cert(resource, event, trigger, **kwargs):
    ...
    if cfg.CONF.nsx_p.nsx_client_cert_storage.lower() == "none":
...
def get_cert_filename(**kwargs):
    filename = cfg.CONF.nsx_p.nsx_client_cert_file
    if kwargs.get('property'):
        properties = admin_utils.parse_multi_keyval_opt(kwargs['property'])
