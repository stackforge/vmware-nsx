...
def handle_router_metadata_access(plugin, context, router_id, interface=None):
    ...
    try:
        if ports:
            if interface:
                if ((not cfg.CONF.NSX.metadata_on_demand or
                     _find_dhcp_disabled_subnet(plugin, ctx_elevated, ports))
                    and not _find_metadata_port(plugin, ctx_elevated, ports)):
                ...
                cfg.CONF.NSX.metadata_on_demand and
