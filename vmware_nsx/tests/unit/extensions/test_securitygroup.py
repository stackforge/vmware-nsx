...
class TestNSGroupContainerManager(nsxlib_testcase.NsxLibTestCase):
    ...
    @mock.patch.object(firewall, 'remove_nsgroup_member')
    @mock.patch.object(firewall, 'add_nsgroup_member')
    def test_nsgroup_container_is_full(self,
                                       add_member_mock,
                                       remove_member_mock):
        ...
        # Trying to add nsgroup to container at index 2 will raise
        # NSGroupIsFull exception, we expect that the nsgroup will be added to
        # container at index 3.
        ...
        # Since the nsgroup was added to the container at index 3, it will fail
        # to remove it from the container at index 2, and then will try to
        # remove it from the container at index 3.
    ...
    @mock.patch.object(firewall, 'remove_nsgroup_member')
    @mock.patch.object(firewall, 'add_nsgroup_member')
    def initialize_with_absent_containers(self,
                                          add_member_mock,
                                          remove_member_mock):
        size = 3
        cont_manager = security.NSGroupContainerManager(size)
        # list_nsgroups will return containers 1 and 3, but not container 2.
        with mock.patch.object(firewall,
            ...
            # containers 1 and 3 and create container 2.
            cont_manager = security.NSGroupContainerManager(size)
            self.assertEqual({1: NSG_IDS[0],
                              2: NSG_IDS[3],
                              3: NSG_IDS[2]},
                             cont_manager._containers)
