...
class NsxV3VrfTestCase(test_plugin.NsxV3PluginTestCaseMixin,
    ...
    def test_router_add_gateway_with_vrf(self):
        with mock.patch(
            ...
            return_value=_uuid()):
            with mock.patch(
                'vmware_nsx.nsxlib.v3.router.RouterLib.add_router_link_port'
            ) as add_router_link_port:
                    ...
                    with self._create_l3_ext_network() as network:
                        with self.subnet(network=network) as subnet:
                            ...
                            self._add_external_gateway_to_router(
                                ...
                                vrf_id=vrf_id)
                            add_router_link_port.assert_called_once_with(
                                mock.ANY, mock.ANY, mock.ANY, vrf_id)
                            body = self._show('routers',
                                              router['router']['id'])
