...
class TestExclusiveRouterTestCase(L3NatTest, L3NatTestCaseBase,
    ...
    def test_router_create_with_default_edge_size(self):
        data = {'router': {
                    'tenant_id': 'whatever',
                    'name': 'test_router',
                    'router_type': 'exclusive'}}
        cfg.CONF.set_override('edge_appliance_size', 'xlarge', group='nsxv')
        router_req = self.new_create_request('routers', data, self.fmt)
        res = router_req.get_response(self.ext_api)
        router = self.deserialize(self.fmt, res)
    ...
    def test_router_create_fails_when_bad_edge_size_specified(self):
        data = {'router': {
                    ...
                    'name': 'test_router',
                    'router_type': 'exclusive'}}
        cfg.CONF.set_override('edge_appliance_size', 'foobar', group='nsxv')
        router_req = self.new_create_request('routers', data, self.fmt)
        res = router_req.get_response(self.ext_api)
        router = self.deserialize(self.fmt, res)
