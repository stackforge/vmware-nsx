...
class TestPortsV2(NsxVPluginV2TestCase,
    ...
    def test_list_ports_filtered_by_security_groups(self):
        ...
        with self.port() as port1, self.port() as port2:
            ...
            temp_sg_p1 = {'security_group': {'tenant_id': 'some_tenant',
                                          'name': '', 'description': 's'}}
            temp_sg_p2 = {'security_group': {'tenant_id': 'some_tenant',
                                             'name': '', 'description': 's'}}
            sg_dbMixin = sg_db.SecurityGroupDbMixin()
            sg_p1 = sg_dbMixin.create_security_group(ctx, temp_sg_p1)
            sg_p2 = sg_dbMixin.create_security_group(ctx, temp_sg_p2)
            sg_dbMixin._delete_port_security_group_bindings(
                ctx, port2['port']['id'])
            sg_dbMixin._create_port_security_group_binding(
                ctx, port1['port']['id'], sg_p1['id'])
            sg_dbMixin._create_port_security_group_binding(
                ctx, port2['port']['id'], sg_p2['id'])
            port1['port']['security_groups'][0] = sg_p1['id']
            port2['port']['security_groups'][0] = sg_p2['id']
            query_params = "security_groups=%s" % port1['port']['security_groups'][0]
            ports_data = self._list('ports', query_params=query_params)
            self.assertEqual(port1['port']['id'], ports_data['ports'][0]['id'])
            #self.assertEqual(1, len(ports_data['ports']))

            query_params = "security_groups=%s" % port2['port']['security_groups'][0]
            ports_data = self._list('ports', query_params=query_params)
