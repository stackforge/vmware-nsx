...
class TestNetworksV2(test_plugin.TestNetworksV2, NsxVPluginV2TestCase):
    ...
    def test_create_vlan_network_with_multiple_dvs_not_found_failure(self):
        ...
        with mock.patch.object(
            p, '_create_vlan_network_at_backend',
            # Return one successful netmoref and fail to find
            # the second DVS on backend as side effect.
            side_effect=[_uuid(),
                         nsxv_exc.NsxResourceNotFound(res_id='dvs-2',
                                                      res_name='dvs_id')]):
            with mock.patch.object(
                p, '_delete_backend_network') as delete_net_call:
                self.assertRaises(nsxv_exc.NsxResourceNotFound,
                                  ...
                                  context.get_admin_context(),
                                  network)
                # One successfully created port group should be rolled back
                # on the failure of second port group creation.
                self.assertEqual(1, delete_net_call.call_count)
