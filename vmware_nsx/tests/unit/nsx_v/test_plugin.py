...
class L3NatTestCaseBase(test_l3_plugin.L3NatTestCaseMixin):
    ...
    def test_subnet_dhcp_metadata_with_update(self):
        cfg.CONF.set_override('dhcp_enable_isolated_metadata',
                              True, group='nsxv')
    ...
    def test_subnet_dhcp_metadata_with_router(self):
        cfg.CONF.set_override('dhcp_enable_isolated_metadata',
                              True, group='nsxv')
        self.plugin_instance.metadata_proxy_handler = mock.Mock()
        self.plugin_instance._update_dhcp_edge_service = mock.Mock()
        with self.subnet(cidr="10.0.0.0/24", enable_dhcp=True) as s1,\
                self.router() as r1:
            subnet_id = s1['subnet']['id']
            is_dhcp_meta = self.plugin_instance.is_dhcp_metadata(
                context.get_admin_context(), subnet_id)
            self.assertTrue(is_dhcp_meta)
            assert not self.plugin_instance._update_dhcp_edge_service.called
            self._router_interface_action('add', r1['router']['id'],
                                          s1['subnet']['id'], None)
            assert self.plugin_instance._update_dhcp_edge_service.called
            self.plugin_instance._update_dhcp_edge_service.reset_mock()
            is_dhcp_meta = self.plugin_instance.is_dhcp_metadata(
                context.get_admin_context(), subnet_id)
            self.assertFalse(is_dhcp_meta)
    ...
    def test_subnet_dhcp_force_metadata_with_router(self):
        cfg.CONF.set_override('dhcp_enable_isolated_metadata',
                              False, group='nsxv')
        cfg.CONF.set_override('dhcp_force_metadata',
                              True, group='nsxv')
        self.plugin_instance.metadata_proxy_handler = mock.Mock()
        self.plugin_instance._update_dhcp_edge_service = mock.Mock()
        with self.subnet(cidr="10.0.0.0/24", enable_dhcp=True) as s1,\
