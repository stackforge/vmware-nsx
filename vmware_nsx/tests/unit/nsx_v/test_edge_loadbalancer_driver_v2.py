...
from vmware_nsx.plugins.nsx_v.drivers import (
    edge_loadbalancer_driver_v2 as edge_lbv2)
...
class BaseTestEdgeLbaasV2(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        edge_lbv2.EdgeLoadbalancerBaseManager._lbv2_driver = self.lbv2_driver
        edge_lbv2.EdgeLoadbalancerBaseManager._core_plugin = self.core_plugin
...
class TestEdgeLbaasV2Listener(BaseTestEdgeLbaasV2):
    def setUp(self):
        ...
        lb = lb_models.LoadBalancer(LB_ID, LB_TENANT_ID, 'lb-name', '',
                                    'some-subnet', 'port-id', LB_VIP)
        self.listener = lb_models.Listener(LISTENER_ID, LB_TENANT_ID,
                                           'l-name', '', None, LB_ID,
                                           'HTTP',
                                           protocol_port=80, loadbalancer=lb)
