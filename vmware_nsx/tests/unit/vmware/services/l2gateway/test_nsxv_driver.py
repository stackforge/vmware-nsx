...
class TestL2gatewayDriver(base.BaseTestCase):
        ...
        fake_l2gw_dict = {"l2_gateway":
                          {"tenant_id": "fake_tenant_id",
                           ...
                           "devices": [{"interfaces":
                                        ...
                                        "device_name": "fake_dev"}]}}
        with mock.patch.object(l2gateway_db.L2GatewayMixin, '_admin_check'):
            self.plugin._nsxv().vcns.validate_network.return_value = False
            self.assertRaises(n_exc.InvalidInput,
                              self.plugin.create_l2_gateway,
                              self.context, fake_l2gw_dict)

    def test_create_gw_edge_failure(self):
    ...
    @mock.patch('vmware_nsx.neutron.services.l2gateway.'
                'nsx_v_driver.NsxvL2GatewayDriver._validate_device_list')
    @mock.patch('vmware_nsx.neutron.services.l2gateway.'
                'nsx_v_driver.NsxvL2GatewayDriver._validate_interface_list')
    @mock.patch('vmware_nsx.neutron.services.l2gateway.'
                'nsx_v_driver.NsxvL2GatewayDriver._create_l2_gateway_edge')
    @mock.patch('networking_l2gw.db.l2gateway.l2gateway_db.'
    ...
    @mock.patch('networking_l2gw.db.l2gateway.l2gateway_db.'
                'L2GatewayMixin.create_l2_gateway')
    def test_create_l2_gateway(self, create_l2gw, _create_l2gw_edge,
                               val_inter, val_dev, _admin_check):
        fake_l2gw_dict = {"l2_gateway":
                          {"tenant_id": "fake_teannt_id",
                           "name": "fake_l2gw",
    ...
    @mock.patch('vmware_nsx.neutron.services.l2gateway.'
                'nsx_v_driver.NsxvL2GatewayDriver._get_device')
    def test_delete_l2_gateway_connection(self, get_devices, del_conn,
    ...
    @mock.patch('vmware_nsx.neutron.services.l2gateway.'
                'nsx_v_driver.NsxvL2GatewayDriver._get_device')
    @mock.patch('vmware_nsx.neutron.plugins.vmware.dbexts.'
                'nsxv_db.get_nsxv_router_binding_by_edge')
    @mock.patch('networking_l2gw.db.l2gateway.l2gateway_db.'
                'L2GatewayMixin.delete_l2_gateway')
    def test_delete_l2_gateway(self, del_l2gw, get_nsxv_router,
