...
class TestL3NatTestCase(L3NatTest,
    ...
    def _mock_del_snat_rules(self):
        return mock.patch("vmware_nsxlib.v3.router.RouterLib."
                          "delete_gw_snat_rules")
    ...
    def _create_subnet_and_assert_snat_rules(self, subnetpool_id,
        ...
        with self.network() as net:
            ...
            with self._mock_add_snat_rule() as add_nat,\
                self._mock_del_snat_rules() as delete_nat:
                # Add the interface
                self._router_interface_action(
                    'add',
                    router_id,
    ...
    def _test_3leg_router_address_scope_change(self, change_gw=False,
        ...
        with self.address_scope(name='as1') as as1, \
            ...
            with self._mock_add_snat_rule() as add_nat, \
                self._mock_del_snat_rules() as del_nat:
                ...
                if change_gw:
                    ...
                    # Both previous rules should be deleted (which is done
                    # in single delete_all call), and one new rule for
                    # subnet2 should be added
