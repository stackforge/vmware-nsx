...
class PrepareObjectForMigration(object):
    ...
    def prepare_security_group_rule(self, sg_rule):
        self.fix_description(sg_rule)
        return self.drop_fields(sg_rule, self.drop_sg_rule_fields)

    def prepare_security_group(self, sg):
        self.fix_description(sg)
        return self.drop_fields(sg, self.drop_sg_fields)

    def prepare_router(self, rtr):
        self.fix_description(rtr)
        return self.drop_fields(rtr, self.drop_router_fields)

    def prepare_subnetpool(self, pool):
    ...
    def prepare_subnet(self, subnet):
    ...
    def prepare_port(self, port, remove_qos=False):
