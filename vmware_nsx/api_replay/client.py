    ...
    def migrate_qos_rule(self, dest_policy, source_rule):
        ...
        try:
            if rule_type == 'bandwidth_limit':
                ...
                LOG.info(_LI("QoS rule type %s is not supported for policy "
                             "%s"), rule_type, pol_id)
            LOG.info(_LI("created QoS policy %s rule %s"), pol_id, rule)
        except Exception as e:
            LOG.error(_LE("Failed to create QoS rule for policy %s: %s"),
                      pol_id, e)
        ...
        for pol in source_qos_pols:
            ...
            # dest server doesn't have the group so we create it here.
                ...
                try:
                    ...
                    LOG.error(_LE("Failed to create QoS policy %s: %s"),
                              pol['id'], e)
                    continue
                else:
        ...
        for sg in source_sec_groups:
            ...
            # dest server doesn't have the group so we create it here.
                ...
                try:
                    ...
                    LOG.error(_LE("Failed to create security group (%s): %s"),
                              new_sg, e)
                ...
                if self.have_id(port['id'], dest_ports) is False:
                    ...
                    if (port['device_owner'] == 'network:router_interface' and
                        ...
                        try:
                            ...
                            LOG.info(_LI("Uplinked router %s to network %s"),
                    ...
                    try:
                        ...
                        # NOTE(arosen): this occurs here if you run the
                        # script multiple times as we don't track this.
                        LOG.error(_LE("Failed to create port (%s) : %s"),
                                  port, e)
                    else:
                        LOG.info(_LI("Created port %s with IPs: %s"),
                                 created_port['id'],
    ...
    def migrate_floatingips(self):
        ...
        for source_fip in source_fips:
            ...
            try:
                ...
                LOG.error(_LI("Failed to create floating ip (%s) : %s"),
                          source_fip, e)
