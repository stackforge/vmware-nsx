...
class NsxVPluginV2(agents_db.AgentDbMixin,
    ...
    def create_router(self, context, router, allow_metadata=True):
        ...
        if (r.get('router_size') != attr.ATTR_NOT_SPECIFIED:
            if r.get('router_type') == "shared"):
                msg = (_("Cannot specify router-size for shared router"))
                raise n_exc.BadRequest(resource="router", msg=msg)
            elif (r.get('router_type') == "exclusive" and
                  r.get('distributed'):
                msg = (_("Cannot specify router-size for exclusive "
                         "distributed router"))
                raise n_exc.BadRequest(resource="router", msg=msg)
        elif (r.get('router_size') == attr.ATTR_NOT_SPECIFIED and
              r.get('router_type') == "exclusive" and not r.get('distributed')):
