...
from vmware_nsx.plugins.nsx_v_drivers import lbaas_common as lb_common
from vmware_nsx.vshield.common import exceptions as nsxv_exc
...
class EdgeMemberManager(EdgeLoadbalancerBaseManager):
    ...
    def update(self, context, old_member, new_member):
        for listener in new_member.pool.listeners:
            for lb_id in listener.loadbalancers:
                lb_binding = nsxv_db.get_nsxv_lbaas_loadbalancer_binding(
                    context.session, lb_id)
                pool_binding = nsxv_db.get_nsxv_lbaas_pool_binding(
                    context.session, lb_id, listener.id, new_member.pool_id)
...
class EdgeHealthMonitorManager(EdgeLoadbalancerBaseManager):
    ...
    def create(self, context, hm):
        for listener in hm.pool.listeners:
            for lb_id in listener.loadbalancers:
                lb_binding = nsxv_db.get_nsxv_lbaas_loadbalancer_binding(
                    context.session, lb_id)
                pool_binding = nsxv_db.get_nsxv_lbaas_pool_binding(
                ...
                if hm_binding:
                    ...
                    try:
                        h = self.vcns.create_health_monitor(
                            edge_id, edge_monitor)[0]
                        edge_mon_id = lb_common.extract_resource_id(
                            h['location'])

                        nsxv_db.add_nsxv_lbaas_monitor_binding(
                            context.session, lb_id, listener.id, hm.pool.id,
                            hm.id, edge_id, edge_mon_id)
                        ...
                        with excutils.save_and_reraise_exception():
                            self.lb_driver.load_balancer.failed_completion(
                                context, hm)
                            LOG.error(
                                _LE('Failed to create health monitor on edge:'
                                    ' %s'), edge_id)
        ...
        self.lb_driver.load_balancer.successful_completion(context, hm)

    @log_helpers.log_method_call
    def update(self, context, old_hm, new_hm):
        for listener in new_hm.pool.listeners:
            for lb_id in listener.loadbalancers:
                lb_binding = nsxv_db.get_nsxv_lbaas_loadbalancer_binding(
                    context.session, lb_id)
                ...
                hm_binding = nsxv_db.get_nsxv_lbaas_monitor_binding(
                    context.session, lb_id, listener.id, new_hm.pool.id,
                    new_hm.id, edge_id)
        ...
        self.lb_driver.load_balancer.successful_completion(context, new_hm)
                ...
                try:
                    ...
                    with excutils.save_and_reraise_exception():
                        self.lb_driver.load_balancer.failed_completion(
                            context, hm)
                        LOG.error(
                            _LE('Failed to delete monitor mapping on edge: '
                                '%s'), edge_id)
