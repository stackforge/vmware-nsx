...
class EdgeFirewallDriver(object):
    ...
    def __init__(self):
        ...
        self.ping_application_ids = None
    ...
    def get_ping_application_ids(self):
        # check cached list first
        # (if backend version changes, neutron should be restarted)
        if self.ping_application_ids:
            return self.ping_application_ids

        self.ping_application_ids = self.get_application_ids(
                ['ICMP Echo', 'IPv6-ICMP Echo'])
        if not self.ping_application_ids:
