...
class EdgeDynamicRoutingDriver(object):
    ...
    def add_bgp_speaker_config(self, edge_id, global_router_id,
        ...
        nsx_bgp_obj = self._convert_bgp_routing_config(bgp_speaker, bgp_peers,
                                                       fips, subnets)
        try:
            self.vcns.update_dynamic_routing_service(edge_id,
                                                     routing_config)
            self.vcns.update_bgp_dynamic_routing(edge_id, nsx_bgp_obj)
    ...
    def add_bgp_peer_config(self, edge_id, bgp_peer):
        ...
        self.vcns.update_bgp_dynamic_routing(edge_id, edge_bgp_info)
    ...
    def add_bgp_redistribution_rule(self, context, edge_id, subnet):
        ...
        if global_dr_config.get('ipPrefixes'):
            global_dr_config['ipPrefixes']['ipPrefixes'].append(new_ip_prefix)
        else:
            global_dr_config['ipPrefixes'] = {'ipPrefixes': [new_ip_prefix]}
        self.vcns.update_dynamic_routing_service(edge_id, global_dr_config)

        _, edge_bgp_info = self.vcns.get_bgp_routing_config(edge_id)
        bgp_redis_rules = edge_bgp_info['redistribution']
        for r in bgp_redis_rules:
            # Checking if the advertise_tenant_network is set to true
            # if yes, create a new rule for the subnet and allow advertise
            # if no, return
    ...
    def delete_bgp_redistribution_rule(self, context, edge_id, subnet):
        ...
        bgp_redis_rules = [r for r in bgp_redis_rules
                           if r.get('prefixName') != 'cidr-%s' % subnet['id']]
        edge_bgp_info['redistribution'] = bgp_redis_rules
        self.vcns.update_bgp_dynamic_routing(edge_id, edge_bgp_info)
