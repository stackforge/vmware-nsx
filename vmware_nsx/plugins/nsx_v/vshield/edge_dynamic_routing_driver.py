...
class EdgeDynamicRoutingDriver(object):
    ...
    def _construct_redistribution_rules(self, bgp_speaker, fips=None,
                                        subnets=None, redis_rules=None,
                                        action=CREATE_OP):
        """
        :rtype: {
            'enabled': True,
        ...
        """
        if redis_rules:
            return_value = redis_rules
        else:
            return_value = DEFAULT_HEADER
        if bgp_speaker.get('advertise_tenant_networks') and subnets:
            for subnet in subnets:
                ...
                if action == CREATE_OP or action == ADD_OP:
                    return_value['rules']['rules'].append(redis_rule)
                else:
                    return_value['rules']['rules'].remove(redis_rule)
    ...
    def add_bgp_redistribution_rule(self, context, edge_id,
        ...
        bgp_speaker = bgp_db.BgpDbMixin().get_bgp_speaker(context,
                                                          bgp_speaker_id)
        _, edge_bgp_info = self.vcns.get_bgp_routing_config(edge_id)
        orig_bgp_redis_rules = edge_bgp_info['redistribution']
        new_bgp_redis_rules = self._construct_redistribution_rules(
                bgp_speaker, fips=[], subnets=[subnet],
                redis_rules=orig_bgp_redis_rules, action=ADD_OP)
        edge_bgp_info['redistribution'] = new_bgp_redis_rules
        self.vcns.enable_bgp_dynamic_routing(edge_id, edge_bgp_info)

    def delete_bgp_redistribution_rule(self, context, edge_id,
                                       bgp_speaker_id, subnet):
        ...
        orig_bgp_redis_rules = edge_bgp_info['redistribution']
        new_bgp_redis_rules = self._construct_redistribution_rules(
            bgp_speaker, [], [subnet], orig_bgp_redis_rules, DELETE_OP)
        edge_bgp_info['redistribution'] = new_bgp_redis_rules
        self.vcns.enable_bgp_dynamic_routing(edge_id, edge_bgp_info)
