...
class EdgeDynamicRoutingDriver(object):
    ...
    def _update_bgp_routing_config(self, edge_id, **kwargs):
        ...
        curr_rules.extend(kwargs.get('rules_to_add', []))
    ...
    def _update_edge_static_routes(self, edge_id, default_routes):
        routes = self._get_edge_static_routes(edge_id)
        routes['staticRoutes']['staticRoutes'].append(default_routes)
    ...
    def add_bgp_speaker_config(self, edge_id, prot_router_id, local_as,
        ...
        if ecmp:
            self._update_edge_static_routes(edge_id, default_routes)
