...
class EdgeManager(object):
    ...
    def _add_dhcp_option(self, static_config, opt):
        ...
        if opt_name in self.nsxv_plugin._supported_dhcp_options:
            key = self.nsxv_plugin._supported_dhcp_options[opt_name]
            if opt_name == 'classless-static-route':
                ...
                if 'option150' not in static_config['dhcpOptions']:
                    static_config['dhcpOptions']['option150'] = {
                        'servers': []}
                opt150 = static_config['dhcpOptions']['option150']['servers']
                opt150.append(opt_val)
            else:
