...
class NsxVMetadataProxyHandler(object):
    ...
    def _get_internal_net_by_az(self, context):
        # Get all internal networks from the DB, and look for the one with
        # the right AZ
        ...
        for net in net_list:
            net_obj = self.nsxv_plugin.get_network(context, net['network_id'])
            az_hints = net_obj.get('availability_zone_hints')
            az_name = az_hints[0] if az_hints else 'default'
            if az_name == self.az.name:
                return net_list[0]['network_id']
