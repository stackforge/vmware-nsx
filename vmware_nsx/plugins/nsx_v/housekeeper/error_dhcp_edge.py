...
class ErrorDhcpEdgeJob(base_job.BaseJob):
        ...
        for edge_id in edge_dict.keys():
            ...
            with locking.LockManager.get_lock(edge_id):
                ...
                self._validate_edge_subinterfaces(
                    context, backend_vnics, vnic_dict, if_changed)
                self._add_missing_subinterfaces(
                    context, vnic_binds, backend_vnics, if_changed)


    def _validate_edge_subinterfaces(
            self, context, backend_vnics, vnic_dict, if_changed):
    ...
    def _add_missing_subinterfaces(
            self, context, vnic_binds, backend_vnics, if_changed):
            ...
            for vnic in backend_vnics:
                if vnic['index'] == vnic_bind['vnic_index']:
                    for sub_if in (
                        ...
                        if ((sub_if['tunnelId'] ==
                                 vnic_bind['tunnel_index'])
                            and (sub_if.get('name') !=
                                     vnic_bind['network_id'])):
                            if_changed[vnic['index']] = True
                            self._recreate_vnic_subinterface(
                                context, sub_if, vnic_bind['network_id'])
