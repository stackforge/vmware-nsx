...
class RouterSharedDriver(router_driver.RouterBaseDriver):
    ...
    def update_routes(self, context, router_id, nexthop):
        ...
        if edge_id:
            ...
            if is_conflict:
                with locking.LockManager.get_lock(str(edge_id)):
                    self._notify_before_router_displacement(context, router_db)
                    self._remove_router_services_on_edge(context, router_id)
                    self._unbind_router_on_edge(context, router_id)
                self._bind_router_on_available_edge(
                    context, router_id, router_db.admin_state_up)
                ...
                with locking.LockManager.get_lock(str(new_edge_id)):
                    self._add_router_services_on_available_edge(context,
                                                                router_id)
                    self._notify_after_router_placement(context, router_db)
            else:
                with locking.LockManager.get_lock(str(edge_id)):
                    router_ids = self.edge_manager.get_routers_on_same_edge(
                        context, router_id)
    ...
    def _notify_after_router_placement(self, context, router):
    ...
    def _notify_before_router_displacement(self, context, router, gw_ip=None):
    ...
    def _update_gw_info(self, context, org_gw_info, new_gw_info, router):
        ...
        with locking.LockManager.get_lock(str(edge_id)):
            ...
            if new_ext_net_id and new_ext_net_id != org_ext_net_id:
                ...
                if is_migrated:
                    self._notify_before_router_displacement(context,
                                                            router, orgaddr)
        ...
        if is_migrated:
            ...
            with locking.LockManager.get_lock(str(edge_id)):
                self._add_router_services_on_available_edge(context,
                                                            router_id)
                self._notify_after_router_placement(context, router)
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        edge_id = edge_utils.get_router_edge_id(context, router_id)
        router_db = self.plugin._get_router(context, router_id)
        p, s = super(nsx_v.NsxVPluginV2, self.plugin)._validate_interface_info(
            interface_info)
        if s:
