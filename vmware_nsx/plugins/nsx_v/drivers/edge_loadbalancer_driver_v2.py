...
class EdgeLoadBalancerManager(EdgeLoadbalancerBaseManager):
    ...
    def stats(self, context, lb):
        ...
        vse_stats = lb_stats[1].get('virtualServer', [])

        stats = {'bytes_in': 0,
                 'bytes_out': 0,
                 'active_connections': 0,
                 'total_connections': 0}
        for vse in vse_stats:
            stats['bytes_in'] += vse.get('bytesIn', 0)
            stats['bytes_out'] += vse.get('bytesOut', 0)
            stats['active_connections'] += vse.get('curSessions', 0)
            stats['total_connections'] += vse.get('totalSessions')
...
class EdgePoolManager(EdgeLoadbalancerBaseManager):
    ...
    def update(self, context, old_pool, new_pool):
        edge_pool = {
            ...
            'algorithm': BALANCE_MAP.get(new_pool.lb_algorithm, 'round-robin'),
            'transparent': False
        }
