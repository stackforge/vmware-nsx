...
class PoolManagerBase(object):
    ...
    def _a_a(self, context):
        ll = nsxv_db.get_nsxv_router_bindings(context.session)
        return json.dumps({l['router_id']:
                    {'e_id': l['edge_id'], 'e_s': l['status']} for l in ll})
    ...
    def reuse_member(self, context, owner, id):
        ...
        """
        print 'X1->%s' % self._a_a(context)
        self._registry.mark_for_creation(context, id)
        print 'X2->%s' % self._a_a(context)
        self._init_member(id)
        self._registry.reuse_member(context, owner, id)
        print 'X3->%s' % self._a_a(context)
        self.check_thresholds(context)
        print 'X4->%s' % self._a_a(context)
...
class NsxvEdgeRegistry(PoolRegistryBase):
    ...
    def reuse_member(self, context, owner, id=None):
        nsxv_db.delete_nsxv_router_binding(context.session, owner)
        nsxv_db.add_nsxv_router_binding(
            context.session, self.get_spare_id(), id, None,
            plugin_const.ACTIVE,
            appliance_size=self._elem_size, edge_type=self._elem_type,
            availability_zone=self._elem_zone)

    def _set_spare_state(self, context, id, state):
        binds = nsxv_db.get_nsxv_router_bindings_by_edge(context.session, id)
        if binds:
            if len(binds) == 1:
    ...
    def mark_for_creation(self, context, id):
        self._set_spare_state(context, id, plugin_const.PENDING_CREATE)

    def mark_for_deletion(self, context, id):
        self._set_spare_state(context, id, plugin_const.PENDING_DELETE)
