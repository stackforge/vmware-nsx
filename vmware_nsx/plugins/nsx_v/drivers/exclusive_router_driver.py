...
class RouterExclusiveDriver(router_driver.RouterBaseDriver):
    ...
    def _update_current_gw_port(self, context, router_id, router, ext_ips):
        port = router.gw_port
        orgaddr = port.plugin._get_port_fixed_ip_addr(port)

        port_data = {'fixed_ips': ext_ips}
        port = super(nsx_v.NsxVPluginV2, self).update_port(
            context, router.gw_port['id'], {'port': port_data})

        newaddr = port.plugin._get_port_fixed_ip_addr(port)
        subnet_mask = self.plugin._get_port_subnet_mask(context, port)

        self.update_router_interface_ip(context, router_id, port['id'],
                                        port['network_id'], orgaddr, newaddr,
                                        subnet_mask)
        return port
    ...
    def _update_gw_info(self, context, org_gw_info, new_gw_info, router):
        ...
        edge_id = self._get_router_edge_id(context, router['id'])
        with locking.LockManager.get_lock(edge_id):
            if new_ext_net_id != org_ext_net_id and orgnexthop:
                ...
                edge_utils.clear_gateway(self.nsx_v, context, router['id'])
            ...
            self.plugin._update_routes(context, router['id'], newnexthop)
