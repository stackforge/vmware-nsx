...
class ConfiguredAvailabilityZone(object):
    ...
    def __init__(self, config_line):
        if config_line and ':' in config_line:
            ...
            # Use the global configuration for edge_placement_random
            self.edge_placement_random = cfg.CONF.nsxv.edge_placement_random
        elif config_line:
            ...
            # Use the global config for edge_placement_random if not set
            self.edge_placement_random = az_info.get('edge_placement_random')
            if self.edge_placement_random is None:
                self.edge_placement_random = (
                    cfg.CONF.nsxv.edge_placement_random)
        else:
