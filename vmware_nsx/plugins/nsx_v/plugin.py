...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _process_security_group_create_backend_resources(self,
        ...
        if self._use_nsx_policies:
            # When using policies - no rules are created.
    ...
    def _validate_security_group_create(self, security_group):
        if self._use_nsx_policies:
            # must have a policy:
            ...
            if not self.nsx_v.vcns.validate_inventory(
                security_group[sg_policy.POLICY]):
                msg = (_('Policy %s was not found on the NSX') %
                         security_group[sg_policy.POLICY])
                raise n_exc.InvalidInput(error_message=msg)
        else:
    ...
    def create_security_group(self, context, security_group, default_sg=False):
        ...
        self._validate_security_group_create(sg_data)
