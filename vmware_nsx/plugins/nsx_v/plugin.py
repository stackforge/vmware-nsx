...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _validate_vnic_type(self, context, port_data, attrs,
    ...
    def create_port(self, context, port):
        ...
        with context.session.begin(subtransactions=True):
            ...
            if self._validate_vnic_type(context, port_data, attrs,
                                        has_security_groups,
                                        port_security):
                nsxv_db.add_nsxv_port_ext_attributes(
                    session=context.session,
                    port_id=port_data['id'],
                    vnic_type=pbin.VNIC_DIRECT)
    ...
    def _update_port(self, context, id, port, original_port, is_compute_port,
        ...
        with context.session.begin(subtransactions=True):
            ...
            if self._validate_vnic_type(context, ret_port, attrs,
                                        has_security_groups,
                                        has_port_security):
                LOG.error("====> TBD")
