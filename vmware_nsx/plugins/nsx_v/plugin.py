...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _add_interface_by_port(self, context, router, port_id, owner):
        ...
        router_driver = self._find_router_driver(context, router.id)
        try:
            #TBD(roeyc): port may have multiple fixed-ips on different subnets?
            return router_driver.add_router_interface(context, router,
                                                      subnets[0], port)
        except vsh_exc.VcnsApiException:
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to add interface '%s' on router %s",
                          port['fixed_ips'][0]['ip_address'], router.id)
                router_driver.remove_router_interface(context,
                                                      router, subnets[0])
        return port, subnets

    def _add_interface_by_subnet(self, context, router, subnet_id, owner):
        with locking.LockManager.get_lock('nsx-shared-router-pool'):
            port, subnets = super(NsxVPluginV2, self)._add_interface_by_subnet(
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error("Failed to add interface '%s' on router %s",
                          port['fixed_ips'][0]['ip_address'], router.id)
                router_driver.remove_router_interface(context, router,
                                                      subnets[0], port)
        return port, subnets
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        return super(NsxVPluginV2, self).add_router_interface(context,
                                                              router_id,
                                                              interface_info)

    def _remove_interface_by_port(self, context, router_id,
                                  port_id, subnet_id, owner):
        port, subnets = super(NsxVPluginV2, self)._remove_interface_by_port(
            context, router_id, port_id, subnet_id, owner)

        router_driver = self._find_router_driver(context, router_id)
        #TBD(roeyc): port may have multiple fixed-ips on different subnets?
        router_driver.remove_router_interface(context, router_id,
                                              subnets[0], port)
        return port, subnets

    def _remove_interface_by_subnet(self, context,
                                    router_id, subnet_id, owner):
        port, subnets = super(NsxVPluginV2, self)._remove_interface_by_subnet(
