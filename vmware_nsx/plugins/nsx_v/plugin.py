...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def default_section(self):
        if not self._default_section:
            self._default_section = self._create_cluster_default_fw_section()
    ...
    def _process_security_group_create_backend_resources(self,
        ...
        # When using policies, the default section should be created after the
        # first security group was added to a policy, so that the location of
        # the default section will be after the policy in the NSX firewall
        # This code makes sure of that since the section will be created
        # when accessed the first time.
        if self._use_nsx_policies and not self.default_section:
            LOG.error(_LE("Failed to create the default FW section"))
