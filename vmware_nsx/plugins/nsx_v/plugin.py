...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _allow_address_pairs_disable_spoofguard(self, session, port):
        if (cfg.CONF.nsxv.enable_multiple_ips_per_vm and
                cfg.CONF.nsxv.spoofguard_enabled):
            sg_policy = nsxv_db.get_spoofguard_policy_id(session, port['port'][
                'network_id'])
    ...
    def create_port(self, context, port):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            if self._check_update_has_allowed_address_pairs(port):
                ...
                self._allow_address_pairs_disable_spoofguard(context.session,
                                                             port)
