    ...
    def _validate_router_size(self, router):
        ...
        if r.get(ROUTER_SIZE) != attr.ATTR_NOT_SPECIFIED:
            ...
            if r.get('router_type') == nsxv_constants.EXCLUSIVE:
                if cfg.CONF.nsxv.edge_appliance_size not in VALID_EDGE_SIZES:
                    msg = (_("Invalid edge size specified in nsx.ini file. "
                             "Specified value: %(specified)s; "
                             "Valid values: %(valid)s") %
                           {'specified': cfg.CONF.nsxv.edge_appliance_size,
                            'valid': VALID_EDGE_SIZES})
                    raise n_exc.BadRequest(resource="router", msg=msg)
                else:
                    r[ROUTER_SIZE] = cfg.CONF.nsxv.edge_appliance_size
