...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _update_vlan_network_dvs_ids(self, network, new_physical_network):
        ...
        # DEBUG ADIT - add edges / vms to this new one??
    ...
    def update_network(self, context, id, network):
        ...
        if (net_attrs.get(providernet.PHYSICAL_NETWORK) and
            ...
            if updated_morefs:
                new_dvs = list(new_dvs_pg_mappings.keys())
        ...
        with context.session.begin(subtransactions=True):
            ...
            if updated_morefs:
                vlan_id = orig_net.get(providernet.SEGMENTATION_ID)
                # Save netmoref to dvs id mappings for VLAN network
                # type for future access.
                all_dvs = orig_net.get(providernet.PHYSICAL_NETWORK)
