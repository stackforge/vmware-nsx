...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def __init__(self):
        ...
        self.dummy_section = self._create_cluster_default_fw_section()
    ...
    def _create_cluster_default_fw_section(self):
        ...
        dummy_section_name = ("OS NSX Neutron plugin placeholder section "
                              "(Don't remove).")
        ...
        with locking.LockManager.get_lock('default-section-init'):
            dummy_section_id = self.nsx_v.vcns.get_section_id(
                dummy_section_name)
            if not dummy_section_id:
                dummy_section = self.nsx_sg_utils.get_section_with_rules(
                    dummy_section_name, [])
                h, c = self.nsx_v.vcns.create_section('ip', dummy_section)
                dummy_section_id = (
                    self.nsx_sg_utils.parse_and_get_section_id(c))
            ...
            if section_id:
                ...
                h, c = self.nsx_v.vcns.create_section(
                    'ip', section_req_body, insert_after=dummy_section_id)
        return dummy_section_id
    ...
    def get_port(self, context, id, fields=None):
        port = super(NsxVPluginV2, self).get_port(context, id)
        # REVISIT(roeyc): Since Port - Provider security-groups mapping and
        # port security-group mapping are saved in the same db table, provider
        # security-group will also be eagrly loaded to the port security-groups
        # list, we override this method to remove these provider
        # security-groups.
        provider_secgroups = set(port[provider_sg.PROVIDER_SECURITYGROUPS])
        secgroups = set(port[ext_sg.SECURITYGROUPS])
        port[ext_sg.SECURITYGROUPS] = list(secgroups - provider_secgroups)
    ...
    def _create_fw_section_for_security_group(self,
        ...
        if securitygroup[provider_sg.PROVIDER]:
            h, c = self.nsx_v.vcns.create_section(
                'ip', self.nsx_sg_utils.to_xml_string(section),
                insert_before=self.dummy_section)
        else:
            h, c = self.nsx_v.vcns.create_section(
                'ip', self.nsx_sg_utils.to_xml_string(section),
                insert_after=self.dummy_section)
