...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _count_no_sec_ports_for_device_id(self, context, device_id):
        ...
        count = 0
        for port in ports:
            # count only ports with vnic index and port-security disabled
            if (validators.is_attr_set(port.get(ext_vnic_idx.VNIC_INDEX)) and
                not port[psec.PORTSECURITY]):
                count += 1
        return count
    ...
    def _add_vm_to_exclude_list(self, context, device_id):
        if (self._dvs and
            self._count_no_sec_ports_for_device_id(
                ...
                # because the BB was already updated)
