...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def allow_multiple_addresses_configure_spoofguard(self, context, id,
        ...
        if (cfg.CONF.nsxv.spoofguard_enabled and
            ...
            try:
                ...
                msg = 'Unable to create spoofguard policy, error: %s, ' \
                      'net_morefs= %s, network_id= %s', e, net_morefs, orig_net
                        ...
                        try:
                            ...
                            for ap in port[addr_apidef.ADDRESS_PAIRS]:
                                if len(ap['ip_address'].split('/')) > 1:
                                    LOG.error(
                                        'Cannot enale port-security on '
                                        'network %s since port_id %s '
                                        'specifies CIDRS in the allowed '
                                        'address pairs', orig_net, port)
                                    self.nsx_v.vcns. \
    ...
    def _validate_address_pairs(self, attrs, db_port, context):
    ...
    def create_port(self, context, port):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            if self._check_update_has_allowed_address_pairs(port):
                ...
                self._validate_address_pairs(attrs, neutron_db, context)
            else:
    ...
    def _update_port(self, context, id, port, original_port, is_compute_port,
        ...
        if addr_apidef.ADDRESS_PAIRS in attrs:
            self._validate_address_pairs(attrs, original_port, context)
