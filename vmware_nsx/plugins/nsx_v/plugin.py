...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _is_overlapping_reserved_subnets(self, subnet):
        ...
        For the V plugin we have a limitation that we should not use the
        following ranges : 169.254.128.0/17 and 169.254.1.0/24
        """
        reserved_subnets = ["169.254.128.0/17", "169.254.1.0/24"]
        ...
        if data['cidr'] not in (attr.ATTR_NOT_SPECIFIED, None):
            ...
            # Go over each of the reserved subnets, and check each for intersection
    ...
    def create_subnet(self, context, subnet):
        ...
        if subnet['subnet']['enable_dhcp']:
            ...
            if self._is_overlapping_reserved_subnets(subnet):
                err_msg = _("No support for those subnets")
                raise n_exc.InvalidInput(error_message=err_msg)
