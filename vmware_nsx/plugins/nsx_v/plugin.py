            ...
            if (self._count_no_sec_ports_for_device_id(
                ...
                if vm_moref is not None:
                    try:
                        ...
                        LOG.info("Add VM %s to exclude list on behalf of port "
                                 "%s: added to list", device_id, port_id)
                    except n_exc.BadRequest:
                        LOG.error(_LE("Failed to add vm %(device)s "
                                      "moref %(moref)s to exclusion list"),
                                  {'device': device_id, 'moref': vm_moref})
            else:
                LOG.info("Add VM %s to exclude list on behalf of port %s: "
                         "already in list", device_id, port_id)
            ...
            if (self._count_no_sec_ports_for_device_id(
                ...
                if vm_moref is not None:
                    try:
                        ...
                        LOG.info("Remove VM %s from exclude list on behalf of "
                                 "port %s: removed from list",
                                 device_id, port_id)
                    except n_exc.BadRequest:
                ...
                LOG.info("Remove VM %s from exclude list on behalf of port"
                         " %s: other ports still in list", device_id, port_id)
        ...
        with locking.LockManager.get_lock('port-update-%s' % id):
            ...
            if is_compute_port and device_id:
                ...
                with locking.LockManager.get_lock(
                    'port-update-device-%s' % device_id):
                    return self._update_port(context, id, port, original_port)
            else:
                return self._update_port(context, id, port, original_port)

    def _update_port(self, context, id, port, original_port):
        attrs = port[attr.PORT]
        port_data = port['port']
