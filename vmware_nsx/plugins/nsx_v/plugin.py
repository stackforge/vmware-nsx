...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _fix_cluster_default_fw_section_order(self, context):
        # When using NSX policies, and adding the first security group to one
        # of them, the section of the policy is created below the OS cluster
        # section.
        # Since we cannot change the order of the clusters via the REST api,
        # we should recreate this section.
        if not self._use_nsx_policies:
            return

        # Check that there is only 1 default security group now
        if (self.get_security_groups_count(context) == 1):
            # delete current OS default section
            section_uri = "%s/%s/%s" % (vcns.FIREWALL_PREFIX,
                                        vcns.FIREWALL_L3_SEC_TYPE,
    ...
    def create_security_group(self, context, security_group, default_sg=False):
        ...
        # If it's the first sg created with a policy, it is possible that the
        # order of backend sections is wrong, since the policy section is
        # created immediately before the default section.
        # in this case we will re-create the OS-cluster section
        if self._use_nsx_policies:
            self._fix_cluster_default_fw_section_order(context)
        return new_sg
    ...
    def _update_nsx_security_group_policies(self, new_policy, old_policy,
                                            nsx_sg_id):
        # validate that the new policies exists
