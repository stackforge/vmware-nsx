...
class NsxVPluginV2(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _update_router_gw_info(self, context, router_id, info,
        ...
        # for multiple external subnets support, we need to set gw port first
        # on subnet which has gateway. If can't get one subnet with gateway or
        # allocate one available ip from subnet, we would just enter normal
        # logic and admin should exactly know what he did.
        if not ext_ips and network_id:
            ...
            if fixed_subnet:
                for subnet in subnets:
                    ...
                    try:
                        ...
                        pass
                LOG.warning(_LW("Cannot get one subnet with gateway to "
                                "allocate one available gw ip"))
        router_driver._update_router_gw_info(
            context, router_id, info,
            is_routes_update=is_routes_update)
