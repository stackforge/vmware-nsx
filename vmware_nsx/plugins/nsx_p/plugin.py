...
# TODO(asarfaty): decide on categories & precedence of maps
NSX_P_DEFAULT_SECTION_CATEGORY = policy_constants.CATEGORY_APPLICATION
NSX_P_REGULAR_SECTION_CATEGORY = policy_constants.CATEGORY_APPLICATION
NSX_P_PROVIDER_SECTION_CATEGORY = policy_constants.CATEGORY_ENVIRONMENT
...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def update_security_group(self, context, sg_id, security_group):
        ...
        if sg_logging.LOGGING in security_group['security_group']:
            logged = (security_group['security_group'][sg_logging.LOGGING] or
                      cfg.CONF.nsx_p.log_security_groups_allowed_traffic)
            # TODO(asarfaty) create an api to update all entries at once
            LOG.debug("Updating all the rules of SG %s with logging=%s",
                      sg_id, logged)
