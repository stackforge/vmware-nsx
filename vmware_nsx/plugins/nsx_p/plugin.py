...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _prepare_default_rules(self):
        ...
        try:
            ...
            self.nsxpolicy.comm_map.get(domain_id, NSX_P_DEFAULT_SECTION)
        except nsx_lib_exc.ResourceNotFound:
            # Create a new section
            try:
                # TODO(asarfaty): is that the right category?
                self.nsxpolicy.comm_map.create_or_overwrite_map_only(
                    NSX_P_DEFAULT_SECTION, domain_id,
            ...
            # The default rules should be equivalent to:
            ...
            #
            # TODO(asarfaty): Check if can add all rules in 1 backend callback
    ...
    def _create_security_group_backend_resources(self, secgroup, domain_id):
        ...
        conditions = [
            self.nsxpolicy.group.get_condition(
                cond_val=sg_id,
                cond_key=policy_constants.CONDITION_KEY_TAG),
            self.nsxpolicy.group.get_condition(
                cond_val=NSX_P_SECURITY_GROUP_TAG,
                cond_key=policy_constants.CONDITION_KEY_NAME)]
        expression = self.nsxpolicy.group.get_nested_condition(
    ...
    def _create_security_group_backend_rule(self, domain_id, map_id, sg_rule):
        ...
        # TODO(asarfaty): use seq-num from the rules order?
        # TODO(asarfaty): use random seq-num to avoid concurrency issues?
        self.nsxpolicy.comm_map.create_entry(
            nsx_name, domain_id, map_id, entry_id=sg_rule['id'],
            description=sg_rule.get('description'),
            service_ids=None,  # temporary
            action=policy_constants.ACTION_ALLOW,
            source_groups=[this_group_id],  # temporary
            dest_groups=None  # temporary
        )
    ...
    def update_security_group(self, context, id, security_group):
        # update the neutron security gruop
        with db_api.context_manager.writer.using(context):
    ...
    def delete_security_group(self, context, id):
        ...
        try:
            self.nsxpolicy.comm_map.delete(domain_id, id)
            self.nsxpolicy.group.delete(domain_id, id)
        except Exception as e:
    ...
    def delete_security_group_rule(self, context, id):
        ...
        try:
            self.nsxpolicy.comm_map.delete_entry(domain_id, map_id, id)
        except Exception as e:
