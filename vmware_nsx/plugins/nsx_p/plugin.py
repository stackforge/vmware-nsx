...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _update_router_gw_info(self, context, router_id, info):
        ...
        if actions['add_service_router']:
            edge_cluster = self.nsxpolicy.tier0.get_edge_cluster(
                new_tier0_uuid)
            if edge_cluster:
                self.nsxpolicy.tier1.set_edge_cluster(router_id, edge_cluster)

        if actions['remove_router_link_port'] or actions['add_router_link_port']:
            # GW was changed
            # TODO(asarfaty): this does not work: update ignores None values
            # And for some reason it also deleted the router display name...
            #Note(asarfaty): ading the router name even though it was not
            ...
            # DEBUG ADIT - set transport zone

        # TODO(asarfaty): handle enable/disable snat, router adv flags, etc.

        if actions['remove_service_router']:
            # remove the edge cluster
            #TODO(asarfaty): setting to None still does not work
            self.nsxpolicy.tier1.set_edge_cluster(router_id, None)
