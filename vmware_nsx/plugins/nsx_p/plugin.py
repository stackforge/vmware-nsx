...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _init_default_config(self):
        ...
        if not self.default_tier0_router:
            # If tier0 is not provided, it will be auto-configured in case
            # backend has only one tier0 router
            tier0s = self.nsxpolicy.tier0.list()
            if len(tier0s) == 1:
                self.default_tier0_router = tier0s[0]['id']
    ...
    def __build_address_bindings(self, port):
        address_bindings = []
    ...
    def _create_port_at_the_backend(self, context, port_data):
        ...
        self.nsxpolicy.segment_port.create(
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        try:
            # TODO(annak): Multiple routers per network?
            self.nsxpolicy.segment.update(network_id,
                                          tier1=router_id)
        except Exception as ex:
            with excutils.save_and_reraise_exception():
