...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _create_security_group_backend_resources(self, secgroup, domain_id):
        ...
        #TODO(asarfaty): adding a condition on the name (=scope) fails
        conditions = [
            self.nsxpolicy.group.build_condition(
                ...
                cond_member_type=policy_constants.CONDITION_MEMBER_PORT)]
        #TODO(asarfaty): adding a condition on the name (=scope) fails
        #conditions.append(self.nsxpolicy.group.build_condition(
        #    cond_val=NSX_P_SECURITY_GROUP_TAG,
        #    cond_key=policy_constants.CONDITION_KEY_NAME,
        #    cond_member_type=policy_constants.CONDITION_MEMBER_PORT))
        #Note(asarfaty): leaving this 'if' until solving the conditions issue
        if len(conditions) > 1:
