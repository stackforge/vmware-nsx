...
NO_SEG_SECURITY_PROFILE_UUID = policy_defs.SegmentSecurityProfileDef.DEFAULT_PROFILE
...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _init_segment_profiles(self):
        ...
        # No Port security spoofguard profile (default NSX profile. just verify it exists)
        try:
            self.nsxpolicy.spoofguard_profile.get(NO_SPOOFGUARD_PROFILE_UUID)
        except nsx_lib_exc.ResourceNotFound:
            msg = (_("Cannot find spoofguard profile %s") % NO_SPOOFGUARD_PROFILE_UUID)
            raise nsx_exc.NsxPluginException(err_msg=msg)

        # Mac discovery profile (find it or create)
        try:
            self.nsxpolicy.mac_discovery_profile.get(MAC_DISCOVERY_PROFILE_UUID)
        except nsx_lib_exc.ResourceNotFound:
        ...
        # No Port security segment-security profile (default NSX profile. just verify it exists)
        try:
            self.nsxpolicy.segment_security_profile.get(NO_SEG_SECURITY_PROFILE_UUID)
        except nsx_lib_exc.ResourceNotFound:
