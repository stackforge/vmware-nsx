...
from vmware_nsxlib.v3 import constants as nsx_lib_v3_consts
...
NSX_P_GLOBAL_DOMAIN_ID = 'os_domain'
...
class NsxPolicyPlugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _prepare_default_rules(self):
        ...
        with locking.LockManager.get_lock('nsx_p_prepare_default_rules'):
            # Return if the domain was already created
            try:
                self.nsxpolicy.domain.get(NSX_P_GLOBAL_DOMAIN_ID)
            except nsx_lib_exc.ResourceNotFound:
                # prevent logger from logging this exception
                sys.exc_clear()
                LOG.info("Going to create default domain, group & communication map")
            else:
                return

            # Create a global domain to catch all the neutron ports
            # (This will not fail if domain already exists)
            self._create_project_domain(NSX_P_GLOBAL_DOMAIN_ID)

            # Create the default group membership criteria to match all neutron
            # ports by tag
            conditions = [self.nsxpolicy.group.build_condition(
                cond_val=NSX_P_SECURITY_GROUP_TAG,
                cond_key=policy_constants.CONDITION_KEY_NAME,
                cond_member_type=policy_constants.CONDITION_MEMBER_VM)]
            # Create the default OpenStack group
            # (This will not fail if group already exists)
            try:
                self.nsxpolicy.group.create_or_overwrite_with_conditions(
                    name=NSX_P_GLOBAL_DOMAIN_ID,
                    domain_id=NSX_P_GLOBAL_DOMAIN_ID,
                    group_id=NSX_P_GLOBAL_DOMAIN_ID,
                    description=NSX_P_DEFAULT_GROUP_DESC,
                    conditions=conditions)
            except Exception as e:
                msg = (_("Failed to create NSX default group: %(e)s") % {'e': e})
                raise nsx_exc.NsxPluginException(err_msg=msg)
            ...
            try:
                ...
                # Create a new section with the default rules
                # TODO(asarfaty) add 'logged' to communication entry and use it
                rule_id = 1
                dhcp_client_rule = self.nsxpolicy.comm_map.build_entry(
                    'DHCP Reply', NSX_P_GLOBAL_DOMAIN_ID, NSX_P_DEFAULT_SECTION,
                    rule_id, sequence_number=rule_id, service_ids=['DHCP-Client'],
                    action=policy_constants.ACTION_DENY,
                    source_groups=None,
                    dest_groups=[NSX_P_GLOBAL_DOMAIN_ID],
                    direction=nsxlib_consts.IN)
                rule_id += 1
                dhcp_server_rule = self.nsxpolicy.comm_map.build_entry(
                    'DHCP Request', NSX_P_GLOBAL_DOMAIN_ID, NSX_P_DEFAULT_SECTION,
                    rule_id, sequence_number=rule_id, service_ids=['DHCP-Server'],
                    action=policy_constants.ACTION_DENY,
                    source_groups=[NSX_P_GLOBAL_DOMAIN_ID],
                    dest_groups=None,
                    direction=nsxlib_consts.OUT)
                rule_id += 1
                block_rule = self.nsxpolicy.comm_map.build_entry(
                    'Block All', NSX_P_GLOBAL_DOMAIN_ID, NSX_P_DEFAULT_SECTION,
    ...
    def _create_security_group_backend_resources(self, secgroup, domain_id):
        ...
        nsx_name = utils.get_name_and_uuid(secgroup['name'] or 'securitygroup',
                                           sg_id)
        # Create the groups membership criteria for ports by tag and value
        conditions = [
            self.nsxpolicy.group.build_condition(
                cond_val=sg_id,
            ...
            self.nsxpolicy.group.build_condition(
                ...
                cond_key=policy_constants.CONDITION_KEY_NAME,
                cond_member_type=policy_constants.CONDITION_MEMBER_VM)]
        expression = self.nsxpolicy.group.build_nested_condition(
            operator=policy_constants.CONDITION_OP_AND,
            conditions=conditions)
        # Create the group
        try:
            group = self.nsxpolicy.group.create_or_overwrite_with_conditions(
                nsx_name, domain_id, group_id=sg_id,
                description=secgroup.get('description'),
                conditions=[expression], tags=tags)
            group_id = group['id']
        except Exception as e:
        ...
        try:
            comm_map = self.nsxpolicy.comm_map.create_or_overwrite_map_only(
            ...
            msg = (_("Failed to create NSX communication map for SG %(sg)s: "
                     "%(e)s") % {'sg': sg_id, 'e': e})
            self.nsxpolicy.group.delete(domain_id, group_id)
            raise nsx_exc.NsxPluginException(err_msg=msg)

        return map_id, group_id

    def _get_l4_protocol_name(self, protocol_number):
        #TODO(asarfaty): reuse the nsxlib code
        if protocol_number is None:
            return
        protocol_number = nsx_lib_v3_consts.IP_PROTOCOL_MAP.get(
        ...
        if protocol_number == 6:
            return nsxlib_consts.TCP
        elif protocol_number == 17:
            return nsxlib_consts.UDP
        elif protocol_number == 1:
            return nsxlib_consts.ICMPV4
        else:
            return protocol_number

    def _get_rule_service(self, sg_rule):
        srv = None
        #TODO(asarfaty): IPv6 support? sg_rule['ethertype'].upper()
        l4_protocol = nsxlib_utils.get_l4_protocol_name(sg_rule['protocol'])
        srv_name = 'service for rule %s' % sg_rule['id']
        ...
        if l4_protocol in [nsxlib_consts.TCP, nsxlib_consts.UDP]:
            ...
            nsxlib_utils.validate_dhcp_params(
            ...
            srv = self.nsxpolicy.icmp_service.create_or_overwrite(
                ...
                icmp_type=icmp_type,
                icmp_code=icmp_code)
        # TODO(asarfaty): Support other protocols (IPProtocolServiceEntry)
        if srv:
            return srv['id']
    ...
    def _create_security_group_backend_rule(self, domain_id, map_id, sg_rule):
        ...
        nsx_name = sg_rule['id']
        # TODO(asarfaty): use actual rule source/dest/service/direction here
        # use random seq-num to avoid concurrency issues
        seq_num = random.randint(1, 10000000)
        direction = nsxlib_consts.IN if sg_rule.get('direction') == 'ingress' else nsxlib_consts.OUT
        source = None
        destination = this_group_id
        if direction == nsxlib_consts.OUT:
            source, destination = destination, source
        service = self._get_rule_service(sg_rule)
    ...
    def create_security_group(self, context, security_group, default_sg=False):
        ...
        try:
            ...
            map_id, group_id = self._create_security_group_backend_resources(
                secgroup, project_id)

            sg_rules = secgroup_db['security_group_rules']
            for sg_rule in sg_rules:
                self._create_security_group_backend_rule(
                    project_id, map_id, sg_rule)
        except Exception as e:
            with excutils.save_and_reraise_exception():
    ...
    def update_security_group(self, context, sg_id, security_group):
        ...
        try:
            ...
            LOG.warning("Failed to update SG %s NSX resources: %s",
                        sg_id, e)
            # Go on with the update anyway (it's just the name & description)
    ...
    def delete_security_group(self, context, sg_id):
        ...
        try:
            # TODO(asarfaty): Do we need "Force" here?
    ...
    def create_security_group_rule_bulk(self, context, security_group_rules):
        ...
        map_id = example_rule['security_group_id']
    ...
    def delete_security_group_rule(self, context, rule_id):
        ...
        map_id = rule_db['security_group_id']
        domain_id = rule_db['tenant_id']

        # Delete the rule itself
        try:
            self.nsxpolicy.comm_map.delete_entry(domain_id, map_id, rule_id)
        except Exception as e:
