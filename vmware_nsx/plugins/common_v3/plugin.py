...
class NsxPluginV3Base(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _get_update_router_gw_actions(
        ...
        # DEBUG ADIT - change this logic in the other patch:  <<<----
        # Get the current status from the backend, and add only if new service needs it
        # And same for remove - remove only if no service need it
        # FW as a service without GW - doesn't need SR
        actions['add_service_router'] = (((real_new_enable_snat and
                                           not real_org_enable_snat) or
                                          (real_new_enable_snat and not
                                          orgaddr and newaddr)
                                          ) and not (fw_exist or lb_exist) or
                                          # Adding GW now allows the FW creation so SR is needed
                                          (not orgaddr and newaddr and fw_exist))
        actions['remove_service_router'] = (
            # removing NAT or removing GW, and no service need the SR
            ((not real_new_enable_snat and real_org_enable_snat) or
