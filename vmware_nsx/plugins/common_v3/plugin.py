...
class NsxPluginV3Base(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _validate_subnet_ip_version(self, subnet, orig_subnet=None):
        if subnet.get('ip_version') == 4:
            # No dhcp restrictions for V4
            return

        if (not validators.is_attr_set(subnet.get('ip_version')) and
            orig_subnet and orig_subnet.get('ip_version') == 4):
            # Ip version is 4 and hasn't changed
            return
    ...
    def _update_subnet(self, context, subnet_id, subnet):
        ...
        self._validate_subnet_ip_version(subnet['subnet'], orig_subnet)
        if self._has_native_dhcp_metadata():
