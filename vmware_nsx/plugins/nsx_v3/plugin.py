...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def update_subnet(self, context, subnet_id, subnet):
        ...
        if (cfg.CONF.nsx_v3.native_dhcp_metadata and
            ...
            kwargs = {}
            for neutron_key, nsx_key in [('dns_nameservers', 'dns_servers'),
                                         ('gateway_ip', 'gateway_ip')]:
                if neutron_key in subnet['subnet']:
                    value = subnet['subnet'][neutron_key]
                    if value != orig_subnet[neutron_key]:
                        kwargs[nsx_key] = value
            if kwargs:
                dhcp_service = nsx_db.get_nsx_service_binding(
