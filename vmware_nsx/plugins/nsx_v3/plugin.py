...
class NsxV3Plugin(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _validate_no_qos(self, net_data):
        err_msg = None
        if ('qos_policy_id' in net_data and
            net_data['qos_policy_id'] is not None):
    ...
    def _get_qos_profile(self, context, policy_id):
        qos_profile_id = nsx_db.get_switch_profile_by_qos_policy(
            context.session, policy_id)
        qos_profile = nsxlib.get_qos_switching_profile(qos_profile_id)
        if qos_profile:
    ...
    def _create_port_at_the_backend(self, context, port_data,
        ...
        qos_policy_id = None
        if ('qos_policy_id' in port_data and
            port_data['qos_policy_id'] is not None):
            qos_policy_id = port_data['qos_policy_id']
            qos_profile_id = self._get_qos_profile(context, qos_policy_id)
    ...
    def _assert_on_external_net_with_qos(self, port_data):
        ...
        # on external networks.
        qos_policy = port_data.get('qos_policy_id')
    ...
    def _get_port_qos_ids(self, context, updated_port):
        ...
        policy_id = None
        profile_id = None
        if ('qos_policy_id' in updated_port):
            policy_id = updated_port['qos_policy_id']
        else:
            # Look for the previous QoS policy
            policy_id = qos_utils.get_port_policy_id(
                context, updated_port['id'])
        if policy_id is not None:
            profile_id = self._get_qos_profile(context, policy_id)
