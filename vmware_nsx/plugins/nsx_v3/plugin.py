...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _ext_extend_port_dict(self, result, portdb):
        ...
        self._extend_get_port_dict_binding(ctx, portdb, result)
    ...
    def _extend_get_port_dict_binding(self, context, portdb, result):
        self._extend_port_dict_binding(context, result)

        # add the qos policy id from the DB
        if 'id' in portdb:
            result[qos_consts.QOS_POLICY_ID] = (
                qos_com_utils.get_port_policy_id(context, portdb['id']))
