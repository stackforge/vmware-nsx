...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _init_native_dhcp(self):
        try:
            dhcp_profile = nsx_resources.DhcpProfile(self._nsx_client).get(
                cfg.CONF.nsx_v3.dhcp_profile_uuid)
            self._dhcp_profile_id = dhcp_profile['id']
    ...
    def _enable_native_dhcp(self, context, network, subnet):
        ...
        host_routes = [{'destination_subnet': subnet['cidr'],
                        'router': '0.0.0.0'}]
        for hr in subnet['host_routes']:
            if hr.destination == const.IPv4_ANY:
                if not gateway_ip:
                    gateway_ip = hr.nexthop
            else:
                host_routes.append({'destination_subnet': hr.destination,
                                    'router': hr.nexthop})
        if gateway_ip:
            host_routes.append({'destination_subnet': const.IPv4_ANY,
                                'router': gateway_ip})
        ...
        try:
            dhcp_server = self._dhcp_server.create(
                self._dhcp_profile_id, server_ip, dns_servers, domain_name,
                gateway_ip, lease_time, options, tags)
    ...
    def _disable_native_dhcp(self, context, network):
        ...
        dhcp_service_id = nsx_db.get_nsx_service_id(
            context.session, network['id'], nsx_constants.SERVICE_DHCP)
        try:
            self._dhcp_server.delete(dhcp_service_id)
            LOG.info(_LI("Deleted logical DHCP server %(server)s for network "
                         "%(network)s"), {'server': dhcp_service_id,
                                          'network': network['id']})
    ...
    def update_subnet(self, context, subnet_id, subnet):
        ...
        if cfg.CONF.nsx_v3.native_dhcp_metadata:
            ...
            if kwargs:
                ...
                try:
                    self._dhcp_server.update(dhcp_service_id, **kwargs)
                except nsx_exc.ManagerError:
                    with excutils.save_and_reraise_exception():
    ...
    def _add_dhcp_binding_on_server(self, context, dhcp_service_id, subnet_id,
        ...
        try:
            ...
            binding = self._dhcp_server.create_binding(
                ...
                netaddr.IPNetwork(subnet.cidr).prefixlen, hostname, options)
            ...
            nsx_db.add_neutron_nsx_dhcp_binding(
                context.session, port['id'], subnet_id, dhcp_service_id,
                binding['id'])
        except nsx_exc.ManagerError:
            with excutils.save_and_reraise_exception():
    ...
    def _add_dhcp_binding(self, context, port):
        ...
        for fixed_ip in self._filter_ipv4_dhcp_fixed_ips(
            ...
            self._add_dhcp_binding_on_server(
                context, dhcp_service_id, fixed_ip['subnet_id'],
    ...
    def _update_dhcp_binding(self, context, old_port, new_port):
        ...
        if old_port["device_owner"] == const.DEVICE_OWNER_DHCP:
            ...
            dhcp_service_id = nsx_db.get_nsx_service_id(
                ...
                nsx_constants.SERVICE_DHCP)
            try:
                self._dhcp_server.update(dhcp_service_id,
                                         server_ip=new_ips[subnets[0]])
            except nsx_exc.ManagerError:
                with excutils.save_and_reraise_exception():
                    LOG.error(_LE("Unable to update IP %(ip)s on logical DHCP "
                                  "server %(server)s"),
                              {'ip': new_ips[subnets[0]],
                               'server': dhcp_service_id})
        else:
            # Update DHCP binding.
            ...
            for binding in bindings:
                if binding['subnet_id'] in subnet_ip_to_delete:
                    self._delete_dhcp_binding_from_server(context, binding)
                elif binding['subnet_id'] in subnet_ip_to_update:
                    self._update_dhcp_binding_on_server(
                        context, binding, new_ips[binding['subnet_id']])
            if subnet_ip_to_add:
                dhcp_service_id = nsx_db.get_nsx_service_id(
                ...
                for subnet_id in subnet_ip_to_add:
                    self._add_dhcp_binding_on_server(
                        context, dhcp_service_id, subnet_id,
                        new_ips[subnet_id], new_port)
