...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _enable_native_dhcp(self, context, network, subnet):
        ...
        prefix_length = str(netaddr.IPNetwork(subnet['cidr']).prefixlen)
        ...
        try:
            dhcp_server = self._dhcp_server.create(
                self._dhcp_profile_id, server_ip, prefix_length, dns_servers,
                domain_name, gateway_ip, lease_time, options, tags)
    ...
    def _disable_native_dhcp(self, context, network):
        ...
        if not dhcp_service:
            LOG.error(_LE("Unable to find DHCP service network %s"),
                      network['id'])
            return
    ...
    def _add_dhcp_binding_on_server(self, context, dhcp_service_id, subnet_id,
        ...
        try:
            ...
            binding = self._dhcp_server.create_binding(
                dhcp_service_id, port['mac_address'], ip, hostname, options)
