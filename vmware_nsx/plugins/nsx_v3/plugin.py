...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _create_network_at_the_backend(self, context, net_data, az,
        ...
        if self._is_ens_tz(physical_net):
            ...
            self._ensure_override_ens_with_portsecurity(physical_net, net_data)
    ...
    def _assert_on_ens_with_qos(self, tz_id, net_data):
        ...
        if validators.is_attr_set(qos_id) and tz_id:
            err_msg = _("Cannot configure QOS on ENS networks")
            raise n_exc.InvalidInput(error_message=err_msg)

    def _ensure_override_ens_with_portsecurity(self, tz_id, net_data):
        if tz_id:
    ...
    def update_network(self, context, id, network):
        ...
        if extern_net:
            ...
            self._assert_on_ens_with_qos(self._get_net_tz(context, id),
                                         net_data)
            self._ensure_override_ens_with_portsecurity(
                self._get_net_tz(context, id), net_data)
    ...
    def create_port(self, context, port, l2gw_port_check=False):
        ...
        if is_ens_tz_port and cfg.CONF.nsx_v3.disable_port_security_for_ens:
