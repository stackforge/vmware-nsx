...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _init_dhcp_notifications(self):
        """Init the DHCP agents notification

        This will be used:
        1. for portgroup networks
        2. when using non native dhcp
        """
        ...
        # unbind the regular events, and bind new ones that will notify only
        # of relevant objects
        notify_api = self.agent_notifiers.get(const.AGENT_TYPE_DHCP)
        ...
        for resource in subscribed_resources:
            ...
            for event in subscribed_events:
                registry.subscribe(
                    self._portgroup_send_dhcp_notification,
                    resource, event)

    def _portgroup_send_dhcp_notification(self, resource, event, trigger,
                                          context, **kwargs):
        # Call the dhcp agent notifier only for portgroup networks
        ...
        if resource == resources.NETWORK:
            net_id = kwargs[resource].get('id')
        else:
            net_id = kwargs[resource].get('network_id')

        if not self._network_is_portgroup_net(context, net_id):
            LOG.debug("Skipping notification for %(event)s %(resource)s",
                      {'event': event, 'resource': resource})
            return

        notify_api = core_plugin.agent_notifiers.get(const.AGENT_TYPE_DHCP)
        if notify_api:
            LOG.debug("notifier called for %(event)s %(resource)s",
                      {'event': event, 'resource': resource})
    ...
    def _create_network_at_the_backend(self, context, net_data, az,
        ...
        if provider_data['net_type'] == utils.NsxV3NetworkTypes.PORTGROUP:
            ...
            if net_data.get(psec.PORTSECURITY):
                LOG.warning("Disabling port security for portgroup network %s",
                            net_data['id'])
                net_data[psec.PORTSECURITY] = False
                net_data['security_groups'] = []
    ...
    def create_port(self, context, port, l2gw_port_check=False):
        ...
        if is_portgroup_net:
            ...
            if port_data.get(psec.PORTSECURITY):
                LOG.warning("Disabling port security for port of portgroup "
                            "network %s", port_data['network_id'])
                port_data[psec.PORTSECURITY] = False
                port_data['security_groups'] = []
        ...
        with db_api.context_manager.writer.using(context):
            ...
            if is_portgroup_net:
                if qos_selected:
                    ...
                    raise n_exc.InvalidInput(error_message=err_msg)
                if port_data.get(psec.PORTSECURITY):
                    LOG.warning("Disabling port security for port of "
                                "portgroup network %s",
                                port_data['network_id'])
                    port_data[psec.PORTSECURITY] = False
                    port_data['security_groups'] = []
