                    ...
                    try:
                        ...
                        msg = (_("Bad value %(val)s for DHCP option "
                                 "%(name)s") % {'name': opt_name,
                                                'val': opt_val})
                        raise n_exc.InvalidInput(error_message=msg)
            elif not self._dhcp_server.get_code(opt_name):
                msg = (_("DHCP option %s is not supported") % opt_name)
                raise n_exc.InvalidInput(error_message=msg)

    def _get_dhcp_option121(self, ip):
        return {'option121': {'static_routes': [
            {'network': '%s' % cfg.CONF.nsx_v3.native_metadata_route,
             'next_hop': ip}]}}

    def _get_extra_dhcp_options(self, options, extra_dhcp_opts):
        if extra_dhcp_opts:
            other_opts = []
            for opt in extra_dhcp_opts:
                opt_name = opt.get('opt_name')
                if opt.get('opt_value') is not None:
                    opt_val = opt['opt_value']
                    # None value means - delete this option
                    if opt_name == 'classless-static-route':
                        ...
                        other_opts.append({
                            'code': self._dhcp_server.get_code(opt_name),
                            'values': [opt_val]})
            if other_opts:
                options['others'] = other_opts

    def _get_dhcp_options(self, ip, extra_dhcp_opts):
        # Always add option121. Extra options only if configured on port
        options = self._get_dhcp_option121(ip)
        if extra_dhcp_opts:
            self._get_extra_dhcp_options(options, extra_dhcp_opts)
        return options
