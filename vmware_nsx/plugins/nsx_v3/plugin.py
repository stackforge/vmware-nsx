...
class NsxV3Plugin(agentschedulers_db.AZDhcpAgentSchedulerDbMixin,
    ...
    def _ensure_override_ens_with_portsecurity(self, net_data, net_id):
        if cfg.CONF.nsx_v3.disable_port_security_for_ens:
            if net_data[psec.PORTSECURITY]:
                LOG.warning("Disabling port security for network %s", net_id)
                net_data[psec.PORTSECURITY] = False
    ...
    def update_network(self, context, id, network):
        ...
        if extern_net:
            ...
            if self._get_net_tz(context, id):
                self._assert_on_ens_with_qos(net_data)
                self._ensure_override_ens_with_portsecurity(net_data, id)
        ...
        with db_api.context_manager.writer.using(context):
            ...
            if is_ens_tz_port:
                if qos_selected:
                    err_msg = _("Cannot configure QOS on ENS networks")
                    raise n_exc.InvalidInput(error_message=err_msg)
                if original_port['security_groups'] or psec.PORTSECURITY:
                    err_msg = _("Cannot configure Port security"
                                " on ENS networks")
                    raise n_exc.InvalidInput(error_message=err_msg)
