...
class NsxV3Plugin(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _validate_multiple_subnets_diff_routers(self, context, interface_info):
        ...
        if len(router_id_set) >= 1:
            err_msg = _("Subnets of network %s cannot be attached to "
                        "multiple routers, already attached to router "
                        "%s") % (net_id, router_ids[0])
            raise n_exc.InvalidInput(error_message=err_msg)

    def add_router_interface(self, context, router_id, interface_info):
        # disallow multiple subnets belong to same network being attached
        # to different routers
        self._validate_multiple_subnets_diff_routers(context, interface_info)
