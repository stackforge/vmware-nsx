...
class ErrorDhcpServerJob(base_job.BaseJob):

    def __init__(self, readonly):
        super(ErrorDhcpServerJob, self).__init__(
            readonly, cfg.CONF.nsxv3.housekeeping_readonly_jobs)

    def get_project_plugin(self, plugin):
        return plugin.get_plugin_by_type(projectpluginmap.NsxPlugins.NSX_T)

    def get_name(self):
        return 'error_dhcp_server'

    def get_description(self):
        return 'Detect orphaned DHCP server'

    def run(self, context):
        super(ErrorDhcpServerJob, self).run(context)
        ...
        for server in orphaned_servers:
            LOG.warning("Housekeeping: DHCP server %s (neutron network: %s)",
                server['id'], server['neutron_net_id'])
            if not self.readonly:
                success, error = v3_utils.delete_orphaned_dhcp_server(
                    context, self.plugin.nsxlib, server)
                if success:
                    LOG.info("Housekeeping: Removed.")
                else:
