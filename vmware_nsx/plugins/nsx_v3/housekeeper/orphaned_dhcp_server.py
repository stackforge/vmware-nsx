    ...
    def run(self, context):
        ...
        if not orphaned_servers:
            ...
            info = base_job.housekeeper_log(
                msg, info, is_error=False)
            return {'error_count': 0, 'error_info': msg}

        msg = "Found %s orphaned DHCP servers:" % len(orphaned_servers)
        ...
        for server in orphaned_servers:
            ...
            if not self.readonly:
                ...
                if success:
                    info = base_job.housekeeper_log("Removed", info)
                else:
                    msg = "Failed to remove: %s." % error
                    info = base_job.housekeeper_log("Removed", info)
