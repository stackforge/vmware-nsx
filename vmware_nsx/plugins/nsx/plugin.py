...
class NsxKunePlugin(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _get_plugin_from_net_id(self, context, net_id):
        ...
        network = super(NsxKunePlugin, self).get_network(context, net_id)
        # caller method name
        action = sys._getframe(1).f_code.co_name
...
def is_kune_core_plugin():
    core_plugin = cfg.CONF.core_plugin
    if (core_plugin.endswith('NsxKunePlugin') or
        core_plugin.endswith('vmware_kune')):
        return True
