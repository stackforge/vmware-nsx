...
class NsxTVPlugin(addr_pair_db.AllowedAddressPairsMixin,
    ...
    def _get_plugin_from_net_id(self, context, net_id):
        ...
        network = super(NsxTVPlugin, self)._get_network(context, net_id)
        # caller method name
        #action = sys._getframe(1).f_code.co_name
    ...
    def get_ports(self, context, filters=None, fields=None,
        ...
        with db_api.context_manager.reader.using(context):
            ports = (
                super(NsxTVPlugin, self).get_ports(
                    context, filters, fields, sorts,
                    limit, marker, page_reverse))
            # Add port extensions
            for port in ports:
    ...
    def _ext_extend_network_dict(result, netdb):
        ...
        p = plugin._get_plugin_from_net_id(ctx, netdb['id'])
    ...
    def _ext_extend_port_dict(result, portdb):
        ...
        # get the core plugin as this is a static method with no 'self'
        plugin = directory.get_plugin()
        p = plugin._get_plugin_from_net_id(ctx, portdb['network_id'])
        with db_api.context_manager.writer.using(ctx):
    ...
    def _ext_extend_subnet_dict(result, subnetdb):
        ...
        # get the core plugin as this is a static method with no 'self'
        plugin = directory.get_plugin()
        p = plugin._get_plugin_from_net_id(ctx, subnetdb['network_id'])
        with db_api.context_manager.writer.using(ctx):
    ...
    def create_project_plugin_map(self, context, project_plugin_map):
        # TODO(asarfaty): Should be allowed for admin only
        # TODO(asarfaty): Validate project id exists
        data = project_plugin_map['project_plugin_map']
        if nsx_db.get_project_plugin_mapping_by_proj(
            context.session, data['project_id']):
    ...
    def get_project_plugin_maps(self, context, filters=None, fields=None,
        ...
        return [data.__dict__ for data in mappings]
    ...
    def _get_plugin_from_project(self, context, project_id):
        ...
        If not there - add an entry with the default v3 plugin
        """
        plugin_type = 'nsx-v3'
        mapping = nsx_db.get_project_plugin_mapping_by_proj(
