...
class NsxPluginBase(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _get_ports_by_sec_group(self, context, filters=None):
        security_groups = filters.pop("security_groups", None)
        if security_groups:
            bindings = (
                super(NsxPluginBase, self)._get_port_security_group_bindings(
                    context, filters={'security_group_id': security_groups}))
            if 'id' in filters:
