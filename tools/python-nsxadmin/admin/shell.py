...
def _validate_op_choice(choice, nsx_plugin):
    if nsx_plugin == 'nsxv':
        supported_resource_ops = nsxv_resources[cfg.CONF.resource].supported_ops
        if choice not in supported_resource_ops:
            LOG.error(_LE('Supported list of operations for the NSX-V resource '
                          '%s'), supported_resource_ops)
            sys.exit(1)

    elif nsx_plugin == 'nsxv3':
        supported_resource_ops = nsxv3_resources[cfg.CONF.resource].supported_ops
        if choice not in supported_resource_ops:
