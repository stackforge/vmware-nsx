...
def count_dhcp_bindings_per_edge():
    return nsxv_db.get_nsxv_dhcp_bindings_count_per_edge(
        neutron_db.context.session)


def neutron_get_dhcp_bindings_count_per_edge():
    """List # of dhcp bindings per NSXv Edge from Neutron DB"""
    output = []
    # get_dhcp_bindigs_per_edge() returns:
    # [('edge-61', '8192'), ['edge-62', '60')]
    for binding in count_dhcp_bindings_per_edge():
        output.append({'edgeId': binding[0], 'count': binding[1]})
    return output
...
def nsx_get_static_bindings_by_edge(edge_id):
    ...
    sbindings = nsx_dhcp_bindings[1].get('staticBindings').get('staticBindings')

    for binding in sbindings:
        nsx_dhcp_static_bindings.add(
            (edge_id, binding.get('macAddress').lower(), binding.get('bindingId').lower()))
...
def neutron_get_static_bindings_by_edge(edge_id):
    ...
    for binding in nsxv_db.get_dhcp_static_bindings_by_edge(
        ...
        neutron_db_dhcp_bindings.add(
            (binding.edge_id, binding.mac_address.lower(), binding.binding_id.lower()))
...
def list_missing_dhcp_bindings(resource, event, trigger, **kwargs):
    ...
    Missing DHCP bindings are those exist in Neutron DB but are not present
    on corresponding NSXv Edge.
    """
    neutron_dhcp_bindings = neutron_get_dhcp_bindings_count_per_edge()

    for binding in neutron_dhcp_bindings:
        LOG.info(_LI("=" * 60))
        LOG.info(_LI("For edge: %s"), binding.get('edgeId'))
