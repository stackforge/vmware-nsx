...
if [[ $Q_PLUGIN == 'vmware_nsx_v' ]]; then
    ...
    if [[ "$1" == "stack" && "$2" == "post-config" ]]; then
        ...
        if is_service_enabled q-svc || is_service_enabled neutron-api; then
            ...
            NSX_MANAGER=($NSX_MANAGER)
            unset IFS
            $PYTHON $dir/tools/nsxp_cleanup.py --mgr-ip $NSX_MANAGER --user $NSX_USER --password $NSX_PASSWORD --db-connection $db_connection
        fi
    elif [[ "$1" == 'clean' ]]; then
        if is_service_enabled q-svc || is_service_enabled neutron-api; then
            $PYTHON $dir/tools/nsxp_cleanup.py --mgr-ip $NSX_MANAGER --user $NSX_USER --password $NSX_PASSWORD
        fi
    fi
fi
