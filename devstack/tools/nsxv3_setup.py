...
def pre_stack_setup(testbed_json, branch, node):
    ...
    nova_cmd = ('sudo rm -rf /opt/stack/nova' if branch == 'kermit' else
                'cd /opt/stack/nova; git checkout %s; git pull -r' % branch)
    upstream_devstack = ('cd ~/devstack; git checkout %s; git pull -r;' %
                         branch)
...
def config_ctrl_local_conf(testbed_json, branch):
    ...
    os_branch = 'stable\/liberty' if branch == 'stable/liberty' else branch
