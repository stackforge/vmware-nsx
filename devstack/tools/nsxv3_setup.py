...
NOVA_FETCH = ("git fetch https://review.openstack.org/openstack/nova "
              "refs/changes/08/238208/1 && git cherry-pick FETCH_HEAD")
...
def pre_stack_setup(testbed_json, branch, node):
    ...
    Steps:
        ...
        - $ mkdir /opt/stack
        - $ cd /opt/stack
        - $ git clone https://github.com/openstack/nova -b stable/liberty
        - $ cd nova
    ...
    cmds = [
        ...
        'mkdir /opt/stack',
        ('cd /opt/stack; '
         'git clone https://github.com/openstack/nova -b %s' % branch),
        ('' if branch == 'master' else 'cd /opt/stack/nova; %s' %
         NOVA_FETCH),
        'sudo rm -rf /usr/local/lib/python2.7/dist-packages/*',
        'cd ~/devstack; git fetch; git checkout %s' % branch,
