...
class NSXClient(object):
    ...
    def get_nsx_domains(self):
        domains = self.nsxlib.domain.list()
    ...
    def cleanup_security_groups(self, domain_id):
        ...
        print("Number of OS Communication maps to be deleted: %s" % len(maps))
        for m in maps:
            self.nsxlib.comm_map.delete(domain_id, m['id'])
        print("Number of OS Groups to be deleted: %s" % len(groups))
        for grp in groups:
    ...
    def cleanup_all(self):
        ...
        Global cleanup steps:
            ...
            - Segments and ports
        """
        domains = self.get_nsx_domains()
        for domain_id in domains:
            if domain_id == policy_constants.DEFAULT_DOMAIN:
                continue
            print("Cleaning up opensatck resources from domain %s" % domain_id)
            self.cleanup_security_groups(domain_id)

        print("Cleaning up opensatck global resources")
