...
class NSXClient(object):
    ...
    def update_logical_port_attachment(self, lports):
        ...
        the VIF attachment on the ports firdst.
        """
        for p in lports:
            p['attachment'] = None
    ...
    def cleanup_os_logical_ports(self):
        ...
        for p in os_lports:
            ...
            if response.status_code == 200:
                print "Successfuly deleted logical port %s" % p['id']
            else:
                print "ERROR: Failed to delete lport %s, response code" % (
                    p['id'], response.status_code)

    def get_os_resources(self, resources):
        """
        Get tall logical resources created by OpenStack
        """
    ...
    def cleanup_os_logical_switches(self):
        ...
        for ls in lswitches:
            ...
            response = self.delete(endpoint=endpoint)
            if response.status_code == 200:
                print "Succeffly deleted logical switch %s-%s" % (
