...
class NeutronNsxDB(object):
    ...
    def query_all(self, column, model):
        return [r[column] for r in self.session.query(model).all()]
...
class VSMClient(object):
    ...
    def query_all_logical_switches(self):
        ...
        if self.neutron_db:
            ...
            lswitches = [ls for ls in lswitches if ls['id'] in db_lswitches]
    ...
    def query_all_firewall_sections(self):
        ...
        if self.neutron_db:
            db_sections = self.neutron_db.query_all_firewll_sections()
    ...
    def query_all_spoofguard_policies(self):
        ...
        if self.neutron_db:
            ...
            policies = [p for p in policies if p['id'] in db_policies]
        return policies
