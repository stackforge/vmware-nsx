    ...
    def delete_logical_switch(self, name):
        ...
        if ls is not None:
            ...
            if response.status_code == 200:
                LOG.info('Successfully deleted logical switch %s' % name)
            else:
                LOG.error('Delete ls %s failed with reponse code %s' %
