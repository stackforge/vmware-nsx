...
class NSXV3Client(object):
    ...
    def get_nsx_resource_by_name(self, nsx_resources, nsx_name):
        """
        Get the NSX component created from OpenStack by name. The name should
        be converted from os_name to nsx_name.
        ...
        if len(nsx_resource) == 0:
            LOG.error("Backend nsx resource %s NOT found!" % nsx_name)
            return
        if len(nsx_resource) > 1:
            LOG.error("More than 1 nsx resources found: %s!" % nsx_resource)
            return
        else:
            LOG.debug("Found nsgroup: %s" % nsx_resource[0])
    ...
    def get_logical_switch(self, os_name, os_uuid):
        ...
        if not os_name or not os_uuid:
            ...
            return
        nsx_name = os_name + os_uuid[:5] + "..." + os_uuid[-5:]
    ...
    def get_firewall_section(self, os_name, os_uuid):
        ...
        """
        if not os_name or not os_uuid:
            LOG.ERROR("Name and uuid of OS security group should be present "
                      "in order to query backend FW section created")
            return
    ...
    def get_ns_group(self, os_name, os_uuid):
        ...
        if not os_name or not os_uuid:
            LOG.ERROR("Name and uuid of OS security group should be present "
                      "in order to query backend nsgroup created")
            return
        nsx_name = os_name + " - " + os_uuid
