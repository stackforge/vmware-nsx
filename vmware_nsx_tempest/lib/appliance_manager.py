...
class ApplianceManager(manager.NetworkScenarioTest):
    ...
    def create_subnet(self, subnet_name, network, router_client=None,
            router_id=None, **kwargs):
        subnet_name_ = constants.APPLIANCE_NAME_STARTS_WITH + subnet_name
        subnet = self._create_subnet(network, namestart=subnet_name_, **kwargs)
        self.topology_subnets[subnet_name] = subnet
        if router_id:
            if not router_client:
                router_client = self.routers_client
            router_client.add_router_interface(router_id,
                subnet_id=subnet["id"])
            self.addCleanup(test_utils.call_and_ignore_notfound_exc,
