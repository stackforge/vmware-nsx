...
class ApplianceManager(manager.NetworkScenarioTest):
    ...
    def create_router(self, router_name,  routers_client=None, **kwargs):
        if not routers_client:
            router_client = self.routers_client
        router_name = constants.APPLIANCE_NAME_STARTS_WITH + router_name
        router = self._create_router(
            namestart=router_name, **kwargs)
        public_network_info = {"external_gateway_info": dict(
            network_id=self.topology_public_network_id)}
        router_client.update_router(router['id'], **public_network_info)
        self.topology_routers[router_name] = router
        return router

    def create_network(self, network_name, networks_client=None,
                       tenant_id=None, port_security_enabled=True,
                       **kwargs):
        if not networks_client:
            networks_client = self.networks_client
        if not tenant_id:
            tenant_id = networks_client.tenant_id
        network_name = constants.APPLIANCE_NAME_STARTS_WITH + network_name
        name = data_utils.rand_name(network_name)
    ...
    def create_subnet(
            self, subnet_name, network, router_client=None, router_id=None,
            **kwargs):
        subnet_name = constants.APPLIANCE_NAME_STARTS_WITH + subnet_name
        subnet = self._create_subnet(network, namestart=subnet_name,
                                   **kwargs)
    ...
    def create_instance(self, server_name, networks, security_groups=None,
        ...
        server_name = constants.APPLIANCE_NAME_STARTS_WITH + server_name
        networks_ = []
        for net in networks:
            net_ = {"uuid": net["id"]}
            networks_.append(net_)
        server = self.create_server(
            name=server_name, networks=networks_, clients=clients, **kwargs)
