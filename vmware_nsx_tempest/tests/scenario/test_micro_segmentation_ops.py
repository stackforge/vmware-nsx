...
class TestMicroSegmentationOps(feature_manager.FeatureManager):
    ...
    def define_security_groups(self):
        self.web_sg = self.create_empty_security_group(namestart="web_sg_")
        self.app_sg = self.create_empty_security_group(namestart="app_sg_")
    ...
    def deploy_micro_segmentation_topology(self):
        router_1 = self.create_router("router_1")
        # Web network
        network_web = self.create_network("network_web")
        subnet_web = self.create_subnet("subnet_web", [network_web],
                                        router_id=router_1["id"])
        server_web_1 = self.create_instance(
            "server_web_1", network_web,
            security_groups=[{'name': self.web_sg['name']}])
        server_web_2 = self.create_instance(
            "server_web_2", network_web,
            security_groups=[{'name': self.web_sg['name']}])
        import time
        time.sleep(120)
        # App network
        network_app = self.create_network("network_app")
        subnet_app = self.create_subnet("subnet_app", network_app,
                                        router_id=router_1["id"])
        server_app_1 = self.create_instance("server_app_1", network_app,
            security_groups=[{'name': self.app_sg['name']}])
        server_app_2 = self.create_instance("server_app_2", network_app,
            security_groups=[{'name': self.app_sg['name']}])
    ...
    def test_micro_segmentation_ops(self):
        ...
        self.check_cross_network_connectivity(
            self.topology_networks["network_1_web"],
            self.servers_details["server_3_app"].floating_ip,
            self.servers_details["server_3_app"].server)
        self.check_cross_network_connectivity(
            self.topology_networks["network_2_app"],
            self.servers_details["server_1_web"].floating_ip,
            self.servers_details["server_1_web"].server)
