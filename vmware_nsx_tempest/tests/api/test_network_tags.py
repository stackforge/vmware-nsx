...
class BaseTagsTest(base.BaseNetworkTest):
    ...
    def skip_checks(cls):
        ...
        if OS_RELEASE_NUMBER < MITAKA_RELEASE_NUMBER:
            msg = "The tags feature begins at Mitaka release."
...
class NetworkTagFilterTest(BaseTagsTest):
    ...
    def resource_setup(cls):
        ...
        cls.a_b_c = ['a', 'ab', 'abc'] # each one tag to only one network
        cls.not_tagged_tags = ['talking-head', 'spinning-tail']
        cls._tags = (['east', 'gold', 'production', 'k1'],
                     ['west', 'silver', 'development'],
                     ['north', 'brown', 'development', 'abc'],
                     ['south', 'brown', 'testing', 'a'],
                     ['west', 'gold', 'production', 'ab', 'k2'],
                     ['east', 'silver', 'testing'],
                     ['north', 'gold', 'production', 'k3'],
    ...
    def test_tags_not_tags_any(self):
        """finding networks having tags and not-tags-any."""
        tags = ['gold', 'production']
        not_tags = ['east']
    ...
    def test_tags_not_tags(self):
        """finding networks having tags and not-tags."""
        tags = ['gold', 'production']
        not_tags = ['west']
        m_net_list = not_x_and_y(not_tags, self.GG,
                                 x_and_y(tags, self.GG))
