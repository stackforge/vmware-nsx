...
class BaseQosTest(base.BaseAdminNetworkTest):
    ...
    Manages qos resources creation and deletion.
    Manages network/port creation and deletion as network cannot be
...
class QosPolicyTest(BaseQosTest):
    ...
    def test_policy_association_with_admin_network(self):
        ...
        self._disassociate_network(network['id'], self.adm_mgr)
    ...
    def test_policy_association_with_tenant_network(self):
        ...
        self._disassociate_network(network['id'], self.pri_mgr)
    ...
    def test_policy_association_with_network_nonexistent_policy(self):
        """Can not create network with policy that is not exist."""
        self.assertRaises(
            exceptions.NotFound,
            self.create_network,
            'test network',
    ...
    def _disassociate_port(self, port_id, client_mgr=None):
        client_mgr = client_mgr if client_mgr else self.adm_mgr
        self.update_port(port_id, qos_policy_id=None, client_mgr=client_mgr)
    ...
    def test_policy_association_with_port_shared_policy(self):
        ...
        retrieved_port = self.show_port(port['id'], client_mgr=self.pri_mgr)
        self.assertEqual(
            policy['id'], retrieved_port['qos_policy_id'])

        self._disassociate_port(port['id'], client_mgr=self.pri_mgr)
    ...
    def test_policy_update_association_with_port_shared_policy(self):
        ...
        policy = self.create_qos_policy(name='test-policy',
                                        ...
                                        shared=True)
        network = self.create_shared_network('test network')
        port = self.create_port(network, client_mgr=self.pri_mgr)
...
class QosBandwidthLimitRuleTest(BaseQosTest):
    ...
    def test_rule_update(self):
        ...
        qos_client.update_bandwidth_limit_rule(
            rule['id'], policy['id'], max_kbps=200, max_burst_kbps=1337)
