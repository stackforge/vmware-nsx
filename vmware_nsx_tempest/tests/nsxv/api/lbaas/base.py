...
class BaseTestCase(base.BaseNetworkTest):
    ...
    def resource_setup(cls):
        ...
        cls.load_balancers_client = (
            load_balancers_client.LoadBalancersClient(*client_args))
        cls.listeners_client = (
            listeners_client.ListenersClient(*client_args))
        cls.pools_client = pools_client.PoolsClient(*client_args)
        cls.members_client = members_client.MembersClient(*client_args)
        cls.health_monitors_client = (
            health_monitors_client.HealthMonitorsClient(*client_args))
    ...
    def setup_lbaas_core_network(cls):
        ...
        router_cfg = dict(router_name=network_name, router_type='exclusive')
    ...
    def resource_cleanup(cls):
        ...
        for lb_id in cls._lbs_to_delete:
            try:
                lb = cls.load_balancers_client.show_load_balancer_status_tree(
...
class BaseAdminTestCase(BaseTestCase):
    ...
    def resource_setup(cls):
        ...
        cls.listeners_client = (
            listeners_client.ListenersClient(*client_args))
        cls.pools_client = (
            pools_client.PoolsClient(*client_args))
        cls.members_client = (
            members_client.MembersClient(*client_args))
        cls.health_monitors_client = (
            health_monitors_client.HealthMonitorsClient(*client_args))
