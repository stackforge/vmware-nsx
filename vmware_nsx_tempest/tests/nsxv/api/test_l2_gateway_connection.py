...
class L2GatewayConnectionTest(base.BaseAdminNetworkTest):
    ...
    @classmethod
    def _get_ipaddress_from_tempest_conf(cls, ip_version=4):
    ...
    def _get_segmentation_id(self, _l2gw, d_idx=0, i_idx=0):
        _dev = _l2gw['devices'][d_idx]
        _seg = _dev['interfaces'][i_idx].get('segmentation_id', [])
        return sorted(_seg)

    def _get_interfaces(self, _l2gw, d_idx=0):
        _dev = _l2gw['devices'][d_idx]
        return sorted(_dev)

    def _pop_segmentation_id(self, _l2gw, d_idx=0, i_idx=0):
        _dev = _l2gw['devices'][d_idx]
        _seg = _dev['interfaces'][i_idx].pop('segmentation_id', [])
        return sorted(_seg)

    def _create_l2gw_switch(self, _name, _devices):
    ...
    def _create_l2gw_connection(self, _l2gw, network_id=None, **kwargs):
    ...
    def _suld_l2gw_connection(self, _res_new):
        _res_show = self.l2gwc_client.show_l2_gateway_connection(
            _res_new['id'])[L2GWC_RID]
