...
class TenantDeleteTest(base.BaseAdminNetworkTest):
    ...
    @classmethod
    def skip_checks(cls):
        super(TenantDeleteTest, cls).skip_checks()
        ...
        if neutron_version[0] < '4.1.2':
            msg = "Please update neutron verion, run pip install upgrade\
                pip then pip install neutron-pythonclient upgrade"
            raise cls.skipException(msg)

    @classmethod
    def setup_clients(cls):
        super(TenantDeleteTest, cls).setup_clients()

    @classmethod
    def resource_setup(cls):
        super(TenantDeleteTest, cls).resource_setup()
    ...
    def test_tenant_delete_purge_using_non_admin(self):
        nets = {}
        ...
        admin_tenant_id = os.popen(
            "keystone --insecure tenant-list | grep admin | awk '{print $2}'")\
            .read()
        purge_output =\
            os.popen('neutron --insecure purge %s --tenant-id=%s' %
    ...
    def test_tenant_delete_no_resource_for_deletion(self):
        name = data_utils.rand_name('tenant-delete-')
    ...
    def test_tenant_delete_with_all_resorces_deleted(self):
        name = data_utils.rand_name('tenant-delete-')
    ...
    def test_tenant_delete_with_some_resources_left(self):
