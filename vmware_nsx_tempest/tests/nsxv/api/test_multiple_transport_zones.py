...
CONF = config.CONF
MAX_MTZ = getattr(CONF.nsxv, 'max_mtz', 0) or 3


class MultipleTransportZonesTest(base.BaseAdminNetworkTest):
    """Validate NSX-v plugin can support multiple transport zone.

    The test environment must at least has 1 additional TZ created.
    The default number of TZs, include the default TZ is 3.
    ...
    def resource_setup(cls):
        ...
        cls.nsxv_scope_ids = cls.get_all_scope_ids(inc_default_scope=True)
    ...
    def create_tenant_network_subnet(cls, name_prefix='mtz-project'):
        ...
        subnet = resp['subnet'] if 'subnet' in resp else resp
        return (network['id'], (None, network, subnet))

    @classmethod
    def get_all_scope_ids(cls, inc_default_scope=False):
        """get all scope ids."""
        scopes = cls.vsm.get_all_vdn_scopes()
        scope_id_list = [x['objectId'] for x in scopes]
        if inc_default_scope:
            return scope_id_list
        try:
            ix = scope_id_list.index(CONF.nsxv.vdn_scope_id)
            scope_id_list.pop(ix)
        except Exception:
            pass
        return scope_id_list
    ...
    def create_network_subnet(self, scope_id, cidr=None, cidr_offset=0):
        ...
        network = resp['network'] if 'network' in resp else resp
        net_id = network['id']
        # NOTE: after creation, physical_network changed TZ's dvs moid.
        # For example:
        #    network['provider:physical_network'] = u'dvs-2070'
        resp = self.create_subnet(network,
                                  name=network_name,
                                  cidr=cidr,
                                  cidr_offset=cidr_offset)
        subnet = resp['subnet'] if 'subnet' in resp else resp
        s_subnet = self.show_subnet(subnet['id'])['subnet']
        resp = self.show_network(net_id)
        s_network = resp['network'] if 'network' in resp else resp
        net_subnets = s_network['subnets']
        self.assertIn(subnet['id'], net_subnets)
        lsw_list = self.vsm.get_all_logical_switches(scope_id)
        lsws = [x for x in lsw_list if x['name'] == net_id]
        msg = ("network=%s is not configied by specified vdn_scope_id=%s"
               % (net_id, scope_id))
        self.assertTrue(len(lsws) == 1, msg=msg)
        self.addCleanup(self._try_delete_resource,
                        self.delete_network, net_id)
        return (net_id, s_network, s_subnet)
    ...
    def check_update_subnet(self, subnet):
        ...
        s_subnet = resp['subnet'] if 'subnet' in resp else resp
        self.assertEqual(new_name, s_subnet['name'])

    def create_show_update_mtz_network_subnet(self, s_id):
        net_id, network, subnet = self.create_network_subnet(s_id)
        self.check_update_network(network)
        self.check_update_subnet(subnet)
        return net_id

    def create_router(self, router_type, name=None, **kwargs):
        router_client = self.admin_client
        rtr_name = name or data_utils.rand_name('mtz-')
        create_kwargs = dict(name=rtr_name, external_gateway_info={
        ...
        self.assertEqual(router['name'], rtr_name)
        return (router_client, router)

    def do_create_router_and_add_interfaces(self, router_type, nets):
        (router_client, router) = self.create_router(router_type)
        if router_type == 'exclusive':
            # remove it? router_types tested it, and only apply to exclusive
        ...
        for net_id, (s_id, network, subnet) in six.iteritems(nets):
            # register for cleanup before actually add interface
            # so interfaces added can be removed and router be deleted.
            self.addCleanup(
    ...
    def do_unset_router(self, router, nets):
        router_client = self.admin_client
    ...
    def t_router_with_multiple_mtz_networks(self, router_type):
        """router attached with multiple TZs."""
        scope_id_list = self.get_all_scope_ids(inc_default_scope=True)
        nets = {}
        for step in range(0, MAX_MTZ):
            s_id = scope_id_list[step % len(scope_id_list)]
            net_id, network, subnet = self.create_network_subnet(
                s_id, cidr_offset=(step + 2))
            nets[net_id] = (s_id, network, subnet)
        router = self.do_create_router_and_add_interfaces(router_type, nets)
        self.do_unset_router(router, nets)

    def t_router_with_network_and_mtz_networks(self, router_type):
    ...
    def test_mtz_network(self):
        scope_id_list = self.get_all_scope_ids(inc_default_scope=False)
        s_id = scope_id_list[0]
        self.create_show_update_mtz_network_subnet(s_id)
    ...
    def test_shared_router_with_multiple_mtz_networks(self):
        ...
        self.t_router_with_multiple_mtz_networks(
            router_type='shared')
    ...
    def test_shared_router_with_network_and_mtz_networks(self):
        """router attached with multiple TZs and one tenant network."""
        self.t_router_with_network_and_mtz_networks(
            router_type='shared')
