...
class MultipleTransportZonesTest(base.BaseAdminNetworkTest):
    ...
    def resource_setup(cls):
        ...
        cls.nsxv_scope_ids = cls.get_all_scope_id_list(inc_default_scope=True)
    ...
    @classmethod
    def get_all_scope_id_list(cls, inc_default_scope=False):
    ...
    def t_router_with_multiple_mtz_networks(self, router_type):
        ...
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=True)
        nets = {}
    ...
    def test_mtz_network(self):
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=False)
    ...
    def test_mtz_overlap_network(self):
        """overlap subnets with the same TZ"""
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=True)
        s_id = scope_id_list[0]
        nets = {}
        for cidr_step in range(1, self.MAX_MTZ):
            net_id, network, subnet = self.create_network_subnet(s_id)
            nets[net_id] = (s_id, network, subnet)
        self.remove_networks(nets)

    @test.idempotent_id('6ecf67fc-4396-41d9-9d84-9d8c936dcb8f')
    def test_multiple_mtz_overlap_network(self):
        """overlap subnets from multiple TZs."""
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=True)
        nets = {}
        cidr_step = 0
    ...
    def test_mtz_non_overlap_network(self):
        """non-overlap subnets from one TZ."""
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=False)
        s_id = scope_id_list[0]
        nets = {}
        for cidr_step in range(0, self.MAX_MTZ):
    ...
    def test_multiple_mtz_non_overlap_network(self):
        """non-overlap subnets from multiple TZs."""
        scope_id_list = self.get_all_scope_id_list(inc_default_scope=True)
        nets = {}
        for cidr_step in range(0, self.MAX_MTZ):
            s_id = scope_id_list[cidr_step % len(scope_id_list)]
