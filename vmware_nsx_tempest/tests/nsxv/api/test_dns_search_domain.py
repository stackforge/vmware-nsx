...
class DnsSearchDomainTest(base.BaseAdminNetworkTest):
    ...
    def resource_setup(cls):
        ...
        cls.search_domain_exist = CONF.network.search_domain_exist
        cls.search_domain_not_exist = CONF.network.search_domain_not_exist
        network_name = data_utils.rand_name('dns-search')
        resp = cls.create_network(client=cls.networks_client,
                                  name=network_name)
        cls.t_network = resp.get('network', resp)
        # addCleanup() only available at instance, not at class
        resp = cls.create_subnet(cls.t_network,
                                 name=network_name,
                                 client=cls.subnets_client,
                                 dns_search_domain=cls.search_domain_exist)
        cls.t_subnet = resp.get('subnet', resp)
    ...
    def create_networks(self, network_name):
        ...
        resp = self.create_subnet(network,
                                  ...
                                  dns_search_domain=self.search_domain_exist)
    ...
    def test_dns_search_domain_crud_operations(self):
        ...
        network_name = data_utils.rand_name('search-domain')
