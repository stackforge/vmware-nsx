...
class FWaaSTestJSON(base.BaseNetworkTest):
    ...
    @test.attr(type='nsxv')
    @test.idempotent_id('901dae30-b148-43d9-ac86-09777aeaba20')
    ...
    @test.attr(type='nsxv')
    @test.idempotent_id('471ebc13-8e3b-4aca-85b8-747935bf0559')
    ...
    @test.attr(type='nsxv')
    @test.idempotent_id('0bdc9670-17b8-4dd5-80c8-dc6e956fc6ef')
    def test_create_multiple_firewall_rules_check_at_backend(self):
        ...
        for key in edges:
            if firewall_topo['router']['name'] in key['name']:
                ...
                break
        rules = self.vsm.get_edge_firewall_rules(edge_id)
        self.assertGreaterEqual(len(rules), 19,
                                "Firewall Rules are greater than 19")
