...
class RouterSizeBaseTest(base.BaseRouterTest):
    ...
    def resource_cleanup(cls):
        routers = cls.routers_client.list_routers()['routers']
        """
        for router in routers:
            test_utils.call_and_ignore_notfound_exc(
                cls.router_client.update_router,
                    router['id'],
                    external_gateway_info=None)
            test_utils.call_and_ignore_notfound_exc(
                cls.router_client.delete_router,
                    router['id'])
        """
        super(RouterSizeBaseTest, cls).resource_cleanup()
    ...
    def change_router_size(self, router, new_router_size):
        router_size = router.get('router_size')
