...
from tempest.lib.services.network import RoutersClient
...
class RouterSizeBaseTest(base.BaseRouterTest):
    """Base class to test router can be created with different sizes:

    NSX-v allows exclusive router to be created with one of ROUTER_SIZE
    and after VIO-3.0 it can update its router_size after created.

    tempest internally use urllib3 and by default it will retry very 60
    seconds. However this retry mechanism cause bug#1716696.

    A better solution is to change request's retry-time so it will not
    cause neutronto keep creating routers while router was not created
    in time.

    The methods should be used for change retry-time are:

       create_exclusive_router_md1 & change_router_size_md1

       The retry-time is http_timeout in request.__init__() and is
       defined by CONF.nsxv.create_router_http_timeout.

    Other methods are kept here for reference and to experience
    how this problem can occur.
    """
    ...
    def setup_clients(cls):
        ...
        # overwrite routers_client parameters
    ...
    def setUp(self):
        ...
        self.router_sizes_client = RoutersClient(
    ...
    def create_exclusive_router(self, router_size):
        ...
        rtr_cfg = dict(
            router_name=name, admin_state_up=False,
            external_network_id=CONF.network.public_network_id,
            router_type='exclusive',
            router_size=router_size)
        router = self.create_router(**rtr_cfg)
        self.assertEqual(router['name'], name)
        self.check_router_nsx_name(router, router_size)
        return router

    def create_exclusive_router_md1(self, router_size):
        """Use this to reduce the possibility of keeping create routers."""
    ...
    def create_exclusive_router_md2(self, router_size):
        name = data_utils.rand_name('rtr-%s' % router_size)
        LOG.debug("create router with size=%s", router_size)
        rtr_cfg = dict(
            router_name=name, admin_state_up=False,
        ...
        router = self.routers_client.update_router(
            ...
            external_gateway_info={
                'network_id': CONF.network.public_network_id})
        router = self.routers_client.show_router(router_id)['router']
        return router
    ...
    def change_router_size_md1(self, router, new_router_size):
        LOG.debug("update router to size=%s", new_router_size)
    ...
    def do_create_update_delete_router_with_size(self,
                                                 ...
                                                 del_waitfor=10.0,
                                                 del_interval=1.5):
        router = self.create_exclusive_router_md1(router_size)
    ...
    def do_router_size_change_test(self, router_size, new_router_size_list):
        ...
        for new_router_size in new_router_size_list:
            self.change_router_size_md1(router, new_router_size)
...
class QuadlargRouterTest(RouterSizeBaseTest):
    @test.attr(type='nsxv')
