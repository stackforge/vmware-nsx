...
class TestMultipleTransportZonesBasicOps(dmgr.TopoDeployScenarioManager):
    ...
    def do_create_router_and_add_interfaces(self, router_type, nets):
        router = self.create_router(router_type)
    ...
    def do_servers_connectivity(self, servers):
    ...
    def wait_for_servers_become_active(self, servers):
        ...
        net_id_list = servers.keys()
        while (len(net_id_list) > 0):
            for net_id in net_id_list:
                nsv = self.servers[net_id]
                serv = nsv['server']
                ss = servers_client.show_server(serv['id'])
                if ss['status'] == 'ACTIVE':
                    net_id_list.remove(net_id)
            time.sleep(5)

    def run_mtz_basic_ops(self, router_type):
        self.servers = {}
        router, nets = self.t_router_with_network_and_mtz_networks(
            router_type)
        for net_id in six.iterkeys(nets):
            ...
            self.servers[net_id] = dict(server=svr, s_id=s_id,
                                        ...
                                        servers_client=servers_client)
        # wait for all servers become ACTIVE
        # assign floatingip to the last one
        self.do_servers_connectivity(self.servers)
