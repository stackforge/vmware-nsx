...
class TestLBaaSV2BasicOps(dmgr.TopoDeployScenarioManager):

    """This test checks basic load balancer V2 operation.

    The following is the scenario outline:
    1. Create an instance
    2. SSH to the instance and start two servers
    3. Create a load balancer with 1 listener, 1 pools, 1 healthmonitor
       and two members and with ROUND_ROBIN algorithm
       associate loadbalancer's vip_address with a floating ip
    ...
    @classmethod
    def skip_checks(cls):
        super(TestLBaaSV2BasicOps, cls).skip_checks()
        cfg = CONF.network
    ...
    def resource_setup(cls):
        super(TestLBaaSV2BasicOps, cls).resource_setup()
    ...
    def setup_credentials(cls):
        ...
        super(TestLBaaSV2BasicOps, cls).setup_credentials()

    def setUp(self):
        super(TestLBaaSV2BasicOps, self).setUp()
    ...
    def tearDown(self):
        ...
        # Investigating subnet can not be removed because exceptions.Conflict:
        # Unable to complete operation on network <network uuid>.
        # There are one or more ports still in use on the network.
        self.router.delete()
        super(TestLBaaSV2BasicOps, self).tearDown()
    ...
    def create_lbaasv2_networks(self):
    ...
    def start_web_servers(self):
        ...
        for server in [self.server1, self.server2]:
            ...
            self.start_server(server, server_fip, server['name'])
    ...
    def start_server(self, server, server_fip, server_name):
        """start sever's web server which return its server_name."""
    ...
    def create_project_lbaas(self):
        ...
        self.pool = self.pools_client .create_pool(
            ...
            lb_algorithm="ROUND_ROBIN", protocol=self.protocol_type,
            name=lb_name)['pool']
        self.wait_for_load_balancer_status(lb_id)
    ...
    def test_load_balancer_basic_ops(self):
        self.create_lbaasv2_networks()
