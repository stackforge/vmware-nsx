...
class HeatSmokeTest(base.BaseOrchestrationTest,
    ...
    def read_template(cls, name, ext='yaml'):
        ...
        dirpath = '/opt/stack/vmware-nsx/vmware_nsx_tempest/tests/'
        filepath = os.path.join(dirpath, *loc)
    ...
    def resource_setup(cls):
        ...
        try:
            ...
            LOG.exception(_LE("File neutron_smoke.yaml not found %(rsp)s") %
                          {'rsp': e})
    ...
    def test_created_resources(self):
        ...
        for resource in self.resources:
            msg = 'rescource %s not create succuessfully' \
    ...
    def test_topo1_same_network_connectivity_(self):
        ...
        LOG.info(_LI(" floating ip :%(rsp)s and private ip list : %(rsp1)s") %
    ...
    def test_topo1_cross_external_connectivity(self):
        ...
        LOG.info(_LI("floating ip :%(rsp)s and external ip : %(rsp1)s") %
                 {"rsp": topo1_server1_floatingip, "rsp1": address_list})
        self._check_server_connectivity(topo1_server1_floatingip, address_list,
                                        should_connect=True)
