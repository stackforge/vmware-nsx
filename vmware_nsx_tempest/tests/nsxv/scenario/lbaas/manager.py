...
class ScenarioTest(test.BaseTestCase):
    ...
    def create_floating_ip(self, thing, pool_name=None):
        """Creates a floating IP and associates to a server
...
class NetworkScenarioTest(ScenarioTest):
    ...
    def _create_subnet(self, network, client=None, namestart='subnet-smoke',
                       **kwargs):
        """Create a subnet for the given network within

        the cidr block configured for tenant networks.
        """
        if not client:
            client = self.subnets_client

        def cidr_in_use(cidr, tenant_id):
            """check subnet with cidr already exist in tenant or not. """
    ...
    def create_floating_ip(self, thing, external_network_id=None,
        ...
        """Creates a floating IP and associates to a resource/port_id

        using Neutron client
        """
        if not external_network_id:
            external_network_id = CONF.network.public_network_id
        if not client:
            client = self.floating_ips__client
        if not port_id:
    ...
    def _create_security_group(self, client=None, tenant_id=None,
        ...
        secgroup = self._create_empty_security_group(
            namestart=namestart, client=client,
            security_groups_client=client, tenant_id=tenant_id)
    ...
    def _create_loginable_secgroup_rule(self, client=None, secgroup=None):
        """These rules are intended to permit inbound ssh and icmp traffic

        from all sources, so no group_id is provided.
        Setting a group_id would only permit traffic from ports
        belonging to the same security group.
        """
    ...
    def create_networks(self, client=None, tenant_id=None,
        ...
        """
        # to provide args to network and subnet use the following dict's
        network_dict = kwargs.pop('network_dict', {})
        subnet_dict = kwargs.pop('subent_dict', {})
        if CONF.baremetal.driver_enabled:
            ...
            subnet_dict.update(dict(network=network, client=client))
            # use explicit check because empty list is a valid option
            if dns_nameservers is not None:
                subnet_dict['dns_nameservers'] = dns_nameservers
            subnet = self._create_subnet(**subnet_dict)
...
class BaremetalScenarioTest(ScenarioTest):
    ...
    def _node_state_timeout(self, node_id, state_attr,
        ...
        if not test.call_until_true(check_state, timeout, interval):
    ...
    def wait_node(self, instance_id):
        ...
        assoc_timeout = CONF.baremetal.association_timeout
        if not test.call_until_true(_get_node, assoc_timeout, 1):
...
class SwiftScenarioTest(ScenarioTest):
    ...
    def list_and_check_container_objects(self, container_name,
        ...
        """List objects for a given container_client

        and assert which are present and which are not.
        """
