...
class TopoDeployScenarioManager(manager.NetworkScenarioTest):
    ...
    def create_server(self, name=None, image=None, flavor=None,
        ...
        cr_msg_t = "Creating a server (name: %s, image: %s,"
    ...
    def waitfor_host_connected(self, host_ip, ping_timeout=5, msg=None):
        ...
        if msg and type(msg) in (str, unicode):
            LOG.debug("waitfor_host_connected ip=%s! %s", host_ip, msg)
        t0 = time.time()
        t1 = time.time() + WAITTIME_FOR_CONNECTIVITY
        LOG.debug("VM-IP[%s] %s: %s.", host_ip, PING_START, t0)
        while (time.time() < t1):
            ...
            if server_pingable:
                LOG.debug("VM-IP[%s] %s: %s (%s).",
                          host_ip, PING_DONE, time.time(),
                          (time.time() - t0))
                break
            LOG.debug("VM-IP[%s] %s, redo after %s seconds.",
                      host_ip, PING_INSESSION,
                      WAITTIME_AFTER_ASSOC_FLOATINGIP)
        if not server_pingable:
            LOG.info("VM-IP[%s] %s: %s (%s).",
                     host_ip, PING_TIMEOUT, time.time(),
                     (time.time() - t0))
    ...
    for dest in dest_list:
        for iptype in iptype_list:
            ...
            if not dest['reachable']:
                n_not_reachable += 1
                LOG.info("VM[%s] can-not-reach-dest[%s %s]",
                         host['ipaddr'], dest['helper'], dest['ipaddr'])
            else:
                LOG.info("VM[%s] can-reach-dest[%s %s]",
                         host['ipaddr'], dest['helper'], dest['ipaddr'])
...
def is_reachable(ssh_client, dest_ip, time_out=60.0, ping_timeout=5.0):
    for now in run_till_timeout(time_out, ping_timeout):
        ...
        if reachable:
            return True
        LOG.debug("DEST[%s] NOT-REACHABLE, retry in %s seconds.",
                  dest_ip, time_out)
    return False
...
def isnot_reachable(ssh_client, dest_ip, time_out=60.0, ping_timeout=5.0,
    ...
    for now in run_till_timeout(time_out, ping_timeout):
        ...
        if not reachable:
            return True
        LOG.debug("DEST[%s] IS-REACHABLE, retry in %s seconds.",
                  dest_ip, time_out)
    return False
