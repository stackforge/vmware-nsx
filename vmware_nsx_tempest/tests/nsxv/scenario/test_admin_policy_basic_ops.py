...
class TestAdminPolicyBasicOps(dmgr.TopoDeployScenarioManager):
        ...
        for dest_ip in ip_list:
            ...
            msg = (self.exc_msg +
                   ("\n  *FAILURE* VM[%s] shouldn't able to PING %s[%s]" %
                    (host_id, ip_type, dest_ip)))
            # if not not_reachable: import pdb; pdb.set_trace()
            self.assertTrue(not_reachable, msg)
            self.log_exc_msg(
                ("  VM[%s] is not able to PING %s[%s]" %
                 (host_id, ip_type, dest_ip)))

    def ican_reach_ip(self, ip_addr, ping_timeout=2):
        ip_type = 'floating-ip'
        for x in range(15):
    ...
    def icannot_reach_ip(self, ip_addr, ping_timeout=5):
        ...
        msg = (self.exc_msg +
               ("\n  *FAILURE* Tempest should not PING %s[%s]" %
                (ip_type, ip_addr)))
        # if reachable: import pdb; pdb.set_trace()
    ...
    def run_basic_scenario(self, router_type):
        ...
        self.log_exc_msg("Tempest can ping & ssh vm-nasa-ames-1's floatingip")
        ...
        self.log_exc_msg("Tempest can ping & ssh vm-nasa-jpl-3's floatingip")
        ...
        self.log_exc_msg("vm-nasa-ames-1 can ping vm-nasa-jpl-1's floatingip")
        self.host_can_reach_ips(
            'nasa-ames-1', ames_1_ssh, 'floating-ip',
            [self.fip_nasa_jpl_3['floating_ip_address']])
        self.log_exc_msg("vm-nasa-jpl-3 can ping vm-nasa-ames-3's floatingip")
        self.host_can_reach_ips(
            'nasa-jpl-3', jpl_3_ssh, 'floating-ip',
            [self.fip_nasa_ames_1['floating_ip_address']])

        ### tenant jpl:policy_BB, tenant ames:policy_AA
        jpl_private_ips = [y['fixed_ip_address']
                           for x, y in six.iteritems(self.server_ips)
                           if x > '2']
        ames_private_ips = [y['fixed_ip_address']
                            for x, y in six.iteritems(self.server_ips)
                            if x < '3']
        # from vm-nasa-ames-1 can ping all other private-ips
        self.log_exc_msg(
            ("Update tenant:jpl to use policy-B[%s]" % self.policy_BB))
        ...
        self.host_cannot_reach_ips('nasa-ames-1', ames_1_ssh,
                                   'private-ip', jpl_private_ips)
        self.host_can_reach_ips('nasa-ames-1', ames_1_ssh,
                                'private-ip', ames_private_ips)

        ### both tenants with policy_BB
        self.log_exc_msg(
            ("Update tenant:ames to use policy-B[%s]" % self.policy_BB))
