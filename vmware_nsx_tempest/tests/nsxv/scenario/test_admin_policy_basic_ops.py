...
class TestAdminPolicyBasicOps(dmgr.TopoDeployScenarioManager):
    ...
    Test topology:
        ...
        4.*2 Admin change tenant:jpl's policy to policy_BB
        5. Tenant jpl's VMs are not pingable, ssh still OK
           Tenant ames's MVs, both ping and ssh are OK
        6.*3 Admin change tenant:ames's policy to policy_BB
    ...
    def host_can_reach_ips(self, host_id, host_ssh, ip_type, ip_list):
        for dest_ip in ip_list:
            reachable = dmgr.is_reachable(host_ssh, dest_ip, time_out=90)
            msg = (self.exc_msg +
    ...
    def run_admin_policy_op_scenario(self, router_type):
        ...
        # from vm-nasa-ames-1 can ping all other private-ips
        self.log_exc_msg(
            ("Update tenant:jpl to use policy_BB[%s]" % self.policy_BB))
        # admin update jpl to policy_BB
        ...
        self.log_exc_msg(
            ("Update tenant:ames to use policy_BB[%s]" % self.policy_BB))
