...
class TestAdminPolicyBasicOps(dmgr.TopoDeployScenarioManager):
    ...
    def run_basic_scenario(self, router_type):
        ...
        # all private-ips are reachable
        private_ips = [y['fixed_ip_address']
                       for x,y in self.server_ips.items()]
        # from vm-nasa-ames-1 can ping all other private-ips
        self.host_can_reach_ips(ames_1_ssh, private_ips)
        # from vm-nasa-jpl_3 can ping all other private-ips
        self.host_can_reach_ips(jpl_3_ssh, private_ips)
        # can ping both tanants' floating-ips
        self.host_can_reach_ips(
            ames_1_ssh, [self.fip_nasa_jpl_3['floating_ip_address']])
        self.host_can_reach_ips(
            jpl_3_ssh, [self.fip_nasa_ames_1['floating_ip_address']])
