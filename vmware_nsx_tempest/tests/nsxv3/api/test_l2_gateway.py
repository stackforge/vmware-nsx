...
from tempest.services.network.json import base as bs
...
class L2GatewayClient(bs.BaseNetworkClient):
    """
...
class L2GatewayTest(base.BaseAdminNetworkTest):
    ...
    def setup_clients(cls):
        ...
        cls.l2gw_created = {}
        cls.l2gw_client = get_client(cls.os_adm)
    ...
    def resource_cleanup(cls):
        try:
            [cls.l2gw_client.delete_l2_gateway(id) for id in
             cls.l2gw_created.keys()]
        except Exception:
            LOG.warn(_LW("Failed to delete l2gw in clean up!!!"))
            pass

    @staticmethod
    def get_segmentation_id(_l2gw, d_idx=0, i_idx=0):
        _dev = _l2gw["devices"][d_idx]
        _seg = _dev["interfaces"][i_idx].get("segmentation_id", [])
        return sorted(_seg)

    @staticmethod
    def pop_segmentation_id(_l2gw, d_idx=0, i_idx=0):
        _dev = _l2gw["devices"][d_idx]
        _seg = _dev["interfaces"][i_idx].pop("segmentation_id", [])
        return sorted(_seg)

    @staticmethod
    def get_interfaces(_l2gw, d_idx=0):
        _dev = _l2gw["devices"][d_idx]
        return sorted(_dev)
    ...
    def operate_l2gw(self, name, devices, task):
        LOG.info(_LI("name: {}, devices: {}, task: {} "
                     .format(name, devices, task)))
        if task == "create":
            ...
            LOG.info(_LI("response : {}".format(rsp_l2gw)))
            self.assertEqual(name, rsp_l2gw["name"],
                             "l2gw name={} is not the same as requested={}"
                             .format(rsp_l2gw["name"], name))
        elif task == "delete":
