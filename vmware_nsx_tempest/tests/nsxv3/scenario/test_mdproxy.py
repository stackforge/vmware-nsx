...
class TestMDProxy(NetworkScenarioTest):
    """Test MDProxy.
    """
    ...
    def skip_checks(cls):
        ...
        """
        if not test.is_extension_enabled("native_mdproxy", "network"):
            msg = "native_mdproxy is not present in network-feature-enabled " \
                  ", skipping all the tests!!!"
            raise cls.skipException(msg)

    def verify_ssh(self, keypair, instance, port_id=None):
        created_f_ip = self.create_floating_ip(instance,
                                               port_id=port_id)
        self.fip = str(created_f_ip["floating_ip_address"])
        self.assertIsNotNone(self.fip)
        LOG.debug("self.fip: %r" % self.fip)
        # Check ssh
    ...
    def _get_port_id(self, network_id, subnet_id, instance):
        ...
        for port in self._list_ports():
            ...
            LOG.debug("port_fixed_ip: %r" % port_fixed_ip)
            if port["network_id"] == network_id \
                    and port["fixed_ips"][0]["subnet_id"] == subnet_id \
                    and "compute:" in port["device_owner"] \
                    and port_fixed_ip == instance_fixed_ip:
                port_id = port["id"]
                LOG.debug("port: %r" % port)
                LOG.debug("nid:%r subnid:%r pid:%r" % (port["network_id"],
                                                       port["fixed_ips"][0][
                                                           "subnet_id"],
                                                       port_id))
    ...
    def _verify_md(self, md_url, expected_value="", check_exist_only=False,
        ...
        def exec_cmd_and_verify_output():
            ...
            while exec_cmd_retry < 30:
                ...
                exec_cmd_retry += 1
                time.sleep(5)
                LOG.info("%r Retried in next 5 seconds" % exec_cmd_retry)
            if check_exist_only:
                return "Verification is successful!"
            msg = ("Failed while verifying metadata on server. Result "
                   "of command %r is NOT %r." % (cmd, expected_value))
            LOG.debug("Expectedvalue:%r Actual Vaule:%r Incase "
                      "Failure Message:%r" % (expected_value, result, msg))
            if sub_result:
    ...
    def verify_md_proxy_logical_ports_on_backend(self):
        ...
        for port_index in range(len(logical_ports)):
            if logical_ports[port_index]["attachment"]["attachment_type"] == \
                    "METADATA_PROXY":
                md_counter += 1
                ...
                self.assertEqual("UP", logical_ports[port_index][
                    "admin_state"], msg)
                self.assertIn("mdproxy-", logical_ports[port_index][
                    "display_name"], msg2)
                self.assertNotEqual(0, len(logical_ports[port_index][
                                               "tags"]), msg3)
    ...
    def test_mdproxy_ping(self):
        ...
        (network_id, subnet_id) = \
            self._create_net_subnet_router_interface(router)
        n1 = {"uuid": network_id}
        instance, keypair = self._create_server_on_networks([n1])
    ...
    def test_mdproxy_with_server_on_two_ls(self):
        router = self._create_router(
            router_name=data_utils.rand_name("router-MDProxy"),
            external_network_id=CONF.network.public_network_id)
        (network_id1, subnet_id1) = \
            self._create_net_subnet_router_interface(router)
        (network_id2, subnet_id2) = \
            self._create_net_subnet_router_interface(router)
    ...
    def test_mdproxy_isolated_network(self):
        ...
        n1 = {"uuid": network_id}
        self._create_server_on_networks([n1])
        self.verify_md_proxy_logical_ports_on_backend()
    ...
    def test_mdproxy_delete_when_ls_bounded(self):
        ...
        self.assertEqual(str(result["error_code"]), "10026")
    ...
    def test_mdproxy_with_multiple_ports_on_network(self):
        ...
        n1 = {"uuid": network_id}
        instance, keypair = self._create_server_on_networks([n1])
        instance2, keypair2 = self._create_server_on_networks([n1])
        port_id = self._get_port_id(network_id, subnet_id, instance)
        # Verify 1st instance.
        self.verify_ssh(keypair=keypair, instance=instance, port_id=port_id)
        self.verify_metadata_in_detail(instance=instance)
        ...
        self.verify_ssh(keypair=keypair2, instance=instance2,
                        port_id=port_id2)
