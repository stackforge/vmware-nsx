...
class TestMDProxy(manager.NetworkScenarioTest):
    ...
    def skip_checks(cls):
        ...
        """
        if not CONF.scenario.native_mdproxy:
            msg = "native_mdproxy is not enabled under scenario config" \
    ...
    def _verify_md(self, md_url, expected_value="", check_exist_only=False,
        ...
        def exec_cmd_and_verify_output():
            ...
            while exec_cmd_retried < \
                ...
                LOG.info(_LI("Tried %r times. Retry in next %r seconds") % (
                    exec_cmd_retried,
                    constants.INTERVAL_BETWEEN_EXEC_RETRY_ON_SSH))
            if check_exist_only:
