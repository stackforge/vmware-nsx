...
class TestProviderSecurityGroup(manager.NetworkScenarioTest):
    ...
    Test the following Provider security Group scenarios
        - Create default and PSG based server's and check connectivity
        - Create mulitple default and PSG based server's and check
          connectivity
        - Create mulitple default and PSG based server's and check
    ...
    def _check_network_reachable(self, ip_address, ssh_client):
        ...
        if "64 bytes from" in result:
            LOG.debug(_LE("Network pingable"))
    ...
    def _check_network_not_reachable(self, ip_address, ssh_client):
        ...
        try:
            ...
            LOG.debug(_LE("Unable to access %{dest}s via ping to "
                          "floating-ip %{src}s"))
            LOG.debug(_LE("Provider sec group rules are applied "
                          " so vm's are not accessible from vm "
                          " launched using default sec group"))
            pass

    def _test_connectivity_between_default_psg_server(self, network_topo1):
        server_name_default = data_utils.rand_name('server-default-sec-group')
        network = network_topo1['network']
        ...
        ssh_client = self.get_remote_client(ip_address_default_vm,
                                            private_key=private_key_default_vm)
        self._check_network_reachable(private_ip_address_psg_vm, ssh_client)
        self.create_security_group_rule(sg_id, cmgr=self.cmgr_adm,
                                        protocol='icmp')
        self._check_network_not_reachable(private_ip_address_psg_vm,
                                          ssh_client)

    def _test_ping_when_psg_enabled_disbaled_on_port(self, network_topo1):
        server_name_default = data_utils.rand_name('server-default-sec-group')
        network = network_topo1['network']
        server_default = self._create_server(server_name_default, network)
        tenant_id = network['tenant_id']
        ...
        port_id = self.get_port_id(network['id'],
                                   network_topo1['subnet']['id'], server_psg)
    ...
    def _test_build_up_topology_and_check_connectivity(self, network_topo):
        ...
        ssh_client =\
            self.get_remote_client(ip_address_default_vm_1,
                                   private_key=private_key_default_vm_1)
        self._check_network_reachable(private_ip_address_psg_vm_1, ssh_client)
        self._check_network_reachable(private_ip_address_psg_vm_2, ssh_client)
        self._check_network_reachable(private_ip_address_default_vm_2,
                                      ssh_client)
        self.create_security_group_rule(sg_id, cmgr=self.cmgr_adm,
                                        protocol='icmp')
        self._check_network_not_reachable(private_ip_address_psg_vm_1,
                                          ssh_client)
        self._check_network_not_reachable(private_ip_address_psg_vm_2,
                                          ssh_client)
        self._check_network_reachable(private_ip_address_default_vm_2,
                                      ssh_client)
