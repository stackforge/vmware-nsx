...
class TestRouterNoNATOps(manager.NetworkScenarioTest):
    ...
    def _test_router_nat_update(self, snat=True):
        ...
        self.assertTrue((len(nat_rules['results']) != 0) == snat)
        ...
        self.assertEqual(router_adv['advertise_nat_routes'], adv_msg % snat)
        self.assertEqual(router_adv['advertise_nsx_connected_routes'],
                         adv_msg % (not snat))
        ...
        # Check router advertisement is correctly set
        router_adv = self.nsx.get_logical_router_advertisement(nsx_router)
        self.assertFalse(router_adv['advertise_nat_routes'],
                         nat_msg % (not snat))
