    ...
    def _clean_network(self): 
        ...
        for port in ports:
            if (port['device_owner'].startswith('compute:None') and
                ...
                self.remove_from_list_by_index(self.ports, port)
        for subnet in subnets:
            if subnet['id'] in [s['id'] for s in self.subnets]:
                self.subnets_client.delete_subnet(subnet['id'])
                self.remove_from_list_by_index(self.subnets, subnet)
        for network in networks:
            if network['id'] in [n[id] for n in self.networks]:
                self.networks_client.delete_network(network['id'])
                self.remove_from_list_by_index(self.subnets, subnet)

    def _create_scale_local_dhcp_server(self, scale):
        ...
        scale_dhcp_servers = [ds for ds in all_dhcp_servers
